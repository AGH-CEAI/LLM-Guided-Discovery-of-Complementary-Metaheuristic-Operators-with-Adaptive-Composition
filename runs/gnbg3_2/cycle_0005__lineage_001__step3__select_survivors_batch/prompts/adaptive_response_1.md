...
- 6 variants win across 24 tasks with moderate spread (6, 6, 4, 3, 3, 1, 1 wins)
- The variants compute fundamentally different signals (geometric, spectral, entropy, rank-correlation, graph-centrality, temporal-regime) — these are NOT rescalings of each other
- No single variant dominates (>50% wins), but some win on similar tasks
- Strategy #2 (ENSEMBLE/VOTING) fits best: running all 6 strategies every generation and aggregating via weighted voting avoids cold-start, handles correlated-but-distinct signals, and provides built-in robustness
