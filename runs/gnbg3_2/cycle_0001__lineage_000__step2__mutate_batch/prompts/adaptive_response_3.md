Looking at the benchmark data, the three winning variants use fundamentally different mechanisms (k-NN spatial, hybrid with diversity-switching, eigendecomposition) and none dominates overall. The variants are NOT near-equivalent rescalings — they capture genuinely different aspects of the search landscape. This makes them ideal for **Mechanism #2 (Ensemble/Voting) with online weight adaptation** — combining their outputs is more robust than picking one, and the different mechanisms provide complementary failure modes.
