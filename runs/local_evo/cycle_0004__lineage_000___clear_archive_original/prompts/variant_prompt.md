Below is a metaheuristic optimizer. Propose 8 DISTINCT replacement implementations for `_clear_archive_original`.

Requirements:
- Keep the EXACT function signature: `def _clear_archive_original(self):`
- Each idea must use a fundamentally different strategy/operator
- Include well-known operators (exponential crossover, arithmetic blend, segment-based, eigenvector-rotated) AND creative / unconventional ones
...
Respond with exactly 8 ideas using the Idea N format.