...
class AdaptiveCovarianceGuidedEvolutionStrategy:
    """
    Adaptive version of CG-ES that automatically selects the best _add_to_archive
    strategy during optimization using Thompson Sampling multi-armed bandit.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        self.reward_history = []
        self.sliding_window_size = max(10, dim)
    ...
    def _update_elite_archive(self):
        ...
        # Update operator reward after archive update
        if self._last_operator_idx >= 0:
            reward = self._compute_operator_reward()
            self._update_operator_reward(self._last_operator_idx, reward)
        
    def _add_to_archive(self, new_elites):
        """Add new elite individuals to archive using adaptive operator selection."""
        ...
        if self.elite_archive is None or len(self.elite_archive) == 0:
            ...
            self._last_operator_idx = 0
            return
        
        # Select operator using Thompson Sampling
        operator_idx = self._select_operator_thompson()
        self._last_operator_idx = int(operator_idx)
        
        # Apply selected operator
        offspring = self._apply_operator(new_elites, operator_idx)
        
        # Add to archive
        self.elite_archive = np.vstack([self.elite_archive, offspring])
        
    def _apply_operator(self, new_elites, operator_idx):
        """Apply the selected operator to generate offspring."""
        if operator_idx == 0:
            return new_elites.copy()
        elif operator_idx == 1:
            return self._operator_exp_crossover(new_elites)
        elif operator_idx == 2:
            return self._operator_blx_alpha(new_elites)
        elif operator_idx == 3:
            return self._operator_segment_crossover(new_elites)
        elif operator_idx == 4:
            return self._operator_eigen_crossover(new_elites)
        elif operator_idx == 5:
            return self._operator_fitness_weighted(new_elites)
        elif operator_idx == 6:
            return self._operator_crowding(new_elites)
        else:
            return new_elites.copy()
    
    def _operator_exp_crossover(self, new_elites):
    ...
    def _operator_blx_alpha(self, new_elites):
        """BLX-α arithmetic blend crossover operator."""
        low, high = self.bounds
        n_new = len(new_elites)
        alpha = 0.5
    ...
    def _operator_segment_crossover(self, new_elites):
    ...
    def _operator_eigen_crossover(self, new_elites):
        """Eigenvector-rotated crossover operator."""
        low, high = self.bounds
        try:
        ...
        return np.clip(offspring, low, high)
    
    def _operator_fitness_weighted(self, new_elites):
        """Fitness-weighted probabilistic merge operator."""
        low, high = self.bounds
    ...
    def _operator_crowding(self, new_elites):
        """Crowding distance for diversity operator."""
        low, high = self.bounds
        ...
        selected_new = [c - archive_len for c in selected]
        return np.clip(new_elites[selected_new], low, high)
    
    def _select_operator_thompson(self):
        """Select operator using Thompson Sampling from Beta distribution."""
        # Clip to prevent numerical issues
        a = np.clip(self.alpha, 1e-10, 1e10)
        b = np.clip(self.beta, 1e-10, 1e10)
        
        # Thompson Sampling: sample from Beta distribution for each operator
        samples = self._rng.beta(a, b)
        
        # Avoid selecting same operator too frequently (UCB-style tiebreaker)
        if len(self.operator_history) > 0:
            recent_ops = self.operator_history[-min(len(self.operator_history), self.sliding_window_size):]
            for i in range(len(samples)):
                count = recent_ops.count(i)
                samples[i] -= count * 0.01
        
        return int(np.argmax(samples))
    
    def _compute_operator_reward(self):
        """Compute reward for the last used operator based on improvement metrics."""
        if self._last_archive_size >= len(self.elite_archive):
            return 0.0
        
        # Track improvement in best fitness
        ...
        if len(self.best_fitness_history) >= 2:
            fitness_change = self.best_fitness_history[-2] - self.best_fitness_history[-1]
            reward += float(np.sign(fitness_change)) * min(abs(fitness_change), 1.0)
        
        # Reward based on diversity improvement
        if self._last_archive_size > 0:
            old_diversity = self._compute_archive_diversity(self.elite_archive[:-1]) if self._last_archive_size > 1 else 0.0
            new_diversity = self._compute_archive_diversity(self.elite_archive) if len(self.elite_archive) > 1 else 0.0
            diversity_delta = new_diversity - old_diversity
            reward += 0.5 * float(np.sign(diversity_delta)) * min(abs(diversity_delta), 1.0)
        
        # Small positive reward for not stagnating
        if self.stagnation_counter < 10:
            reward += 0.1
        
        return float(np.clip(reward, -2.0, 2.0))
    
    def _compute_archive_diversity(self, archive):
        """Compute diversity of the archive using average pairwise distance."""
        if archive is None or len(archive) < 2:
            return 0.0
        archive = np.atleast_2d(archive)
        if len(archive) < 2:
            return 0.0
        centroid = np.mean(archive, axis=0)
        distances = np.sqrt(np.sum((archive - centroid)**2, axis=1))
    ...
    def _update_operator_reward(self, operator_idx, reward):
        ...
        if len(self.operator_history) > self.sliding_window_size:
            self.operator_history.pop(0)
            self.reward_history.pop(0)
        
        # Update Beta distribution parameters
        if reward > 0:
            self.alpha[operator_idx] += reward
        else:
            self.beta[operator_idx] += abs(reward)
        
        # Clip to prevent numerical issues
        self.alpha = np.clip(self.alpha, 1e-10, 1e10)
        self.beta = np.clip(self.beta, 1e-10, 1e10)
        
        # Update archive size tracking
        self._last_archive_size = len(self.elite_archive)
