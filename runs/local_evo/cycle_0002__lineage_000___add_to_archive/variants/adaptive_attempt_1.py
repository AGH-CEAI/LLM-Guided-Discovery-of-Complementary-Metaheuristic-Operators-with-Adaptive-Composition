...
class CovarianceGuidedEvolutionStrategy:
    ...
    An adaptive population-based optimizer that automatically selects
    the best _add_to_archive strategy using Thompson Sampling with
    sliding window credit assignment.
    
    Implements 7 crossover strategies and dynamically chooses the best
    based on observed rewards during optimization.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        self._operator_counts = np.zeros(self._num_operators, dtype=np.int64)
        self._current_operator = 0
        self._reward_window_size = 20
        self._operator_rewards = [deque(maxlen=self._reward_window_size) for _ in range(self._num_operators)]
        self._iteration = 0
        self._prev_best_fitness = np.inf
        self._running_best = np.inf
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            self._running_best = min(self._running_best, current_best)
    ...
    def _select_operator_thompson(self):
        ...
        samples = np.zeros(self._num_operators)
        for i in range(self._num_operators):
            alpha_i = max(1.0, float(self._alpha[i]))
            beta_i = max(1.0, float(self._beta[i]))
            samples[i] = self._rng.beta(alpha_i, beta_i)
    ...
    def _update_operator_reward(self, operator_idx, reward):
        """Update reward for selected operator using sliding window."""
        reward = float(np.clip(reward, -1e6, 1e6))
        self._operator_rewards[operator_idx].append(reward)
        self._operator_counts[operator_idx] += 1
        
        # Update Beta distribution parameters for Thompson Sampling
        if reward > 0:
            self._alpha[operator_idx] += 0.1 * reward
        else:
            self._beta[operator_idx] += 0.1 * abs(reward)
        
        # Clamp parameters to prevent numerical issues
        self._alpha[operator_idx] = float(np.clip(self._alpha[operator_idx], 0.1, 1e6))
        self._beta[operator_idx] = float(np.clip(self._beta[operator_idx], 0.1, 1e6))
    
    def _compute_reward(self):
        """Compute reward based on improvement in best fitness."""
        current_best = float(np.min(self.current_fitness))
        
        if np.isfinite(self._prev_best_fitness) and self._prev_best_fitness > 0:
            improvement = self._prev_best_fitness - current_best
            scale = max(abs(self._prev_best_fitness), 1e-6)
            reward = float(improvement / scale)
        else:
            reward = 0.0
    ...
    def _update_elite_archive(self):
        ...
        self._update_operator_reward(self._current_operator, reward)
        
        # Prune archive to maintain size
        self._prune_archive()
        
        self._iteration += 1
    
    # =========================================================================
    # VARIANT 1: Exponential Crossover (2 wins)
    # =========================================================================
    
    def _add_to_archive_exp_crossover(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 2: BLX-α Arithmetic Blend Crossover (6 wins)
    # =========================================================================
    
    def _add_to_archive_blx_alpha(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 3: Segment-based Crossover (2 wins)
    # =========================================================================
    
    def _add_to_archive_segment(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 4: Eigenvector-Rotated Crossover (1 win)
    # =========================================================================
    
    def _add_to_archive_eigen_rotated(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 5: Fitness-Weighted Probabilistic Merge (3 wins)
    # =========================================================================
    
    def _add_to_archive_fitness_weighted(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 6: Crowding Distance for Diversity (2 wins)
    # =========================================================================
    
    def _add_to_archive_crowding(self, new_elites):
    ...
    # =========================================================================
    # VARIANT 7: Simple VStack (original, 8 wins)
    # =========================================================================
    
    def _add_to_archive_simple(self, new_elites):
        """Add new elite individuals using simple vstack (original method)."""
        if new_elites is None or len(new_elites) == 0:
            return
        self.elite_archive = np.vstack([self.elite_archive, new_elites])
    
    # =========================================================================
    # MASTER OPERATOR DISPATCHER
    # =========================================================================
    
    def _add_to_archive(self, new_elites):
        """Add new elite individuals using the adaptively selected operator."""
        operators = [
            self._add_to_archive_simple,
            self._add_to_archive_exp_crossover,
            self._add_to_archive_blx_alpha,
            self._add_to_archive_segment,
            self._add_to_archive_eigen_rotated,
            self._add_to_archive_fitness_weighted,
            self._add_to_archive_crowding,
        ]
        operators[self._current_operator](new_elites)
    ...
    def _evaluate_archive_members(self):
        ...
        if not hasattr(self, 'current_population') or self.current_population is None:
            return np.full(len(self.elite_archive), np.inf)
        
        valid_fitness = self.current_fitness[np.isfinite(self.current_fitness)]
        if len(valid_fitness) == 0:
            return np.full(len(self.elite_archive), np.inf)
        
        fitness_values = np.full(len(self.elite_archive), np.inf)
        for i, elite in enumerate(self.elite_archive):
            distances = np.sum((self.current_population - elite)**2, axis=1)
            nearest_idx = np.argmin(distances)
            fitness_values[i] = float(self.current_fitness[nearest_idx])
