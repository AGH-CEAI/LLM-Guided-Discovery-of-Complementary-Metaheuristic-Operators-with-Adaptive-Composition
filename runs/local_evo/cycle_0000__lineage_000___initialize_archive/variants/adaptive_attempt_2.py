...
class CovarianceGuidedEvolutionStrategy:
    ...
    Extends CG-ES with adaptive selection of initialization strategies using
    Thompson Sampling to determine which archive initialization method works
    best for the current problem during optimization.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Adaptive initialization tracking (Thompson Sampling)
        self.num_init_methods = 9  # 0=original, 1-8=variants
        self._init_alpha = np.ones(self.num_init_methods)  # Successes
        self._init_beta = np.ones(self.num_init_methods)   # Failures
        self.current_init_method = None
        self.init_best_fitness = None
    ...
    def _initialize_archive(self):
        """Select initialization method using Thompson Sampling and initialize archive."""
        samples = np.array([float(self._rng.beta(self._init_alpha[i], self._init_beta[i])) 
                           for i in range(self.num_init_methods)])
        self.current_init_method = int(np.argmax(samples))
        
        if self.current_init_method == 0:
            self._init_archive_original()
        elif self.current_init_method == 1:
            self._init_archive_variant_1()
        elif self.current_init_method == 2:
            self._init_archive_variant_2()
        elif self.current_init_method == 3:
            self._init_archive_variant_3()
        elif self.current_init_method == 4:
            self._init_archive_variant_4()
        elif self.current_init_method == 5:
            self._init_archive_variant_5()
        elif self.current_init_method == 6:
            self._init_archive_variant_6()
        elif self.current_init_method == 7:
            self._init_archive_variant_7()
        elif self.current_init_method == 8:
            self._init_archive_variant_8()
    
    def _init_archive_original(self):
        """Initialize empty elite archive (original method)."""
        self.elite_archive = np.zeros((0, self.dim))
    
    def _init_archive_variant_1(self):
        """Initialize archive using Latin Hypercube Sampling (LHS)."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_2(self):
        """Initialize archive using segment-based crossover from mean and random point."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_3(self):
        """Initialize archive using bounded random combination of two random parents."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_4(self):
        """Initialize archive using lower and upper half centers for each dimension."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_5(self):
        """Initialize archive using eigenvectors of covariance matrix."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_6(self):
        """Initialize archive using Halton sequence for quasi-random sampling."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_7(self):
        """Initialize archive using orthogonal directions from mean."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _init_archive_variant_8(self):
        """Initialize archive using corners, edges and center of search space."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
    ...
    def _sample_and_evaluate(self, func):
        ...
        if self.init_best_fitness is None and len(self.current_fitness) > 0:
            valid_fitness = self.current_fitness[np.isfinite(self.current_fitness)]
            if len(valid_fitness) > 0:
                self.init_best_fitness = float(np.min(valid_fitness))
    ...
    def _restart_optimizer(self):
        ...
        self._update_thompson_reward()
        self._reinitialize_distribution()
        self._initialize_archive()
        self._reset_tracking()
        
    def _update_thompson_reward(self):
        """Update Thompson Sampling based on initialization method performance."""
        if self.current_init_method is not None and len(self.current_fitness) > 0:
            reward = self._compute_init_reward()
            method = int(self.current_init_method)
            if reward > 0:
                self._init_alpha[method] += reward * 10.0
            else:
                self._init_beta[method] += abs(reward) * 2.0 + 0.1
            self._init_alpha[method] = float(np.clip(self._init_alpha[method], 0.01, 1e10))
            self._init_beta[method] = float(np.clip(self._init_beta[method], 0.01, 1e10))
            self.total_restarts += 1
        
    def _compute_init_reward(self):
        """Compute reward for the initialization method based on improvement."""
        current_best = float(np.min(self.current_fitness))
        if self.init_best_fitness is not None:
            improvement = self.init_best_fitness - current_best
            return float(np.clip(improvement, -1e10, 1e10))
        return 0.0
