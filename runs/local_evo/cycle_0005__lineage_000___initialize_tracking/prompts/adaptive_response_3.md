...
class CovarianceGuidedEvolutionStrategy:
    ...
    Automatically selects the best _initialize_tracking strategy using Thompson Sampling
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        self._init_restart_history = []
        self._last_init_restart_best_fitness = None
        self._total_init_restarts = 0
        
        # Track iterations for reward assessment
        self._iterations_since_last_init_restart = 0
        self._iteration_at_last_init_restart = 0
        self._min_iterations_before_reward = 3
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            self._iterations_since_last_init_restart = iteration - self._iteration_at_last_init_restart
            
            if self._should_restart():
                self._select_init_operator()
                self._record_pre_init_fitness()
                self._restart_optimizer()
                self._total_init_restarts += 1
                self._iteration_at_last_init_restart = iteration
                self._iterations_since_last_init_restart = 0
    ...
    def _select_init_operator(self):
        """Select initialization operator using Thompson Sampling with sliding window rewards."""
        min_trials = 2
        
        if self._total_init_restarts < min_trials:
        ...
        self._init_restart_history.append({
            'operator_idx': int(self._selected_init_operator),
            'iteration': len(self._init_restart_history)
        })
        
    def _record_pre_init_fitness(self):
        """Record fitness before restart for reward calculation."""
        curr_fitness = np.min(self.current_fitness)
        if np.isfinite(curr_fitness):
            self._last_init_restart_best_fitness = float(curr_fitness)
        else:
            self._last_init_restart_best_fitness = 1e10
    ...
    def _update_init_operator_reward(self):
        """Update operator reward based on improvement after restart with sufficient iterations."""
        if self._last_init_restart_best_fitness is None or len(self._init_restart_history) == 0:
            return
        
        if self._iterations_since_last_init_restart < self._min_iterations_before_reward:
            return
            
        last_entry = self._init_restart_history[-1]
        ...
        prev_fitness = self._last_init_restart_best_fitness
    ...
    def _reinitialize_distribution(self):
        """Reinitialize the distribution after restart."""
        self.mean = self._rng.uniform(
            self.bounds[0], self.bounds[1], size=self.dim
        )
        self.covariance = np.eye(self.dim)
    ...
    def _clear_archive(self):
        """Clear the elite archive (unchanged from original)."""
        self.elite_archive = np.zeros((0, self.dim))
    ...
    def _initialize_tracking(self):
        """Call the selected initialization tracking method using adaptive operator selection."""
        op = self._selected_init_operator
        
        if op == 0:
            self._initialize_tracking_original()
        elif op == 1:
            self._initialize_tracking_variant_1()
        elif op == 2:
            self._initialize_tracking_variant_2()
        elif op == 3:
            self._initialize_tracking_variant_3()
        elif op == 4:
            self._initialize_tracking_variant_4()
        elif op == 5:
            self._initialize_tracking_variant_5()
        elif op == 6:
            self._initialize_tracking_variant_6()
        elif op == 7:
            self._initialize_tracking_variant_7()
        elif op == 8:
            self._initialize_tracking_variant_8()
        else:
            self._initialize_tracking_original()
    
    def _initialize_tracking_original(self):
        """Original: Initialize stagnation and diversity tracking."""
        self.best_fitness_history = []
        self.stagnation_counter = 0
        self.diversity_history = []
    
    def _initialize_tracking_variant_1(self):
        """Variant 1: Exponential crossover initialization."""
    ...
    def _initialize_tracking_variant_2(self):
        """Variant 2: Arithmetic blend (BLX-α) initialization."""
        samples = self._sample_from_distribution(2)
        p1, p2 = samples[0], samples[1]
        alpha = 0.5
    ...
    def _initialize_tracking_variant_3(self):
        """Variant 3: Segment-based crossover initialization."""
        samples = self._sample_from_distribution(2)
        p1, p2 = samples[0], samples[1]
    ...
    def _initialize_tracking_variant_4(self):
        """Variant 4: Eigenvector-rotated initialization."""
        try:
    ...
    def _initialize_tracking_variant_5(self):
        """Variant 5: Quasi-random (Van der Corput) initialization."""
        ...
        quasi = (seq2 + seq3) / 2.0
        range_len = high - low
        range_len = range_len if not np.isscalar(range_len) else (range_len if range_len != 0 else 1e-12)
        if np.isscalar(range_len):
            quasi = low + quasi * range_len
        else:
            quasi = low + quasi * range_len
        quasi = np.clip(quasi, low, high)
        pseudo_fit = float(np.linalg.norm(quasi - self.mean))
        self.best_fitness_history = [pseudo_fit]
        self.stagnation_counter = 0
        self.diversity_history = [float(np.linalg.norm(quasi - self.mean))]
    
    def _initialize_tracking_variant_6(self):
        """Variant 6: Self-adversarial initialization."""
        try:
            eigenvalues, eigenvectors = np.linalg.eigh(self.covariance)
    ...
    def _initialize_tracking_variant_7(self):
        """Variant 7: Hybrid multi-operator initialization."""
    ...
    def _initialize_tracking_variant_8(self):
        """Variant 8: Random projection initialization using orthogonal matrix."""
    ...
    def _should_restart(self):
        ...
        return (
            ...
            np.any(np.isfinite(self.mean) == False) or
