...
class CovarianceGuidedEvolutionStrategy:
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Adaptive operator selection for _initialize_tracking (Thompson Sampling with sliding window)
    ...
    def _initialize_optimizer(self):
        ...
        self._select_init_tracking_operator()
    ...
    def _select_init_tracking_operator(self):
        """Select _initialize_tracking operator using Thompson Sampling with sliding window rewards."""
        min_trials = 2
    ...
    def _record_pre_init_fitness(self):
        """Record fitness before initialization for reward calculation."""
        if self.mean is not None:
            curr_fitness = float(np.linalg.norm(self.mean))
            if np.isfinite(curr_fitness):
                self._last_init_restart_best_fitness = curr_fitness
            else:
    ...
    def _update_init_operator_reward(self):
        """Update operator reward based on initialization quality."""
        ...
        curr_fitness = float(np.linalg.norm(self.mean)) if self.mean is not None else 1e10
        if not np.isfinite(curr_fitness):
            curr_fitness = 1e10
            
        prev_fitness = self._last_init_restart_best_fitness
        
        diversity_score = float(np.mean(self.diversity_history)) if len(self.diversity_history) > 0 else 0.0
        improvement = prev_fitness - curr_fitness + 0.1 * diversity_score
    ...
    def _initialize_tracking(self):
        """Initialize stagnation and diversity tracking using selected adaptive operator."""
    ...
    def _initialize_tracking_variant_5(self):
        ...
        quasi = (seq2 + seq3) / 2.0
        range_len = high - low
        range_len = range_len if np.any(range_len != 0) else 1e-12
    ...
    def _manage_stagnation(self):
        ...
        if len(self.diversity_history) >= 10:
            recent_div = self.diversity_history[-10:]
            if np.std(recent_div) < self.diversity_threshold * 0.1:
    ...
    def _reinitialize_distribution(self):
        """Reinitialize distribution parameters for restart."""
        self.mean = self._rng.uniform(
            self.bounds[0], self.bounds[1], size=self.dim
        )
        self.covariance = np.eye(self.dim)
