...
class CovarianceGuidedEvolutionStrategy:
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Adaptive init operator selection (Thompson Sampling)
        ...
        self._last_init_restart_fitness = None
        self._total_inits = 0
    ...
    def _select_init_operator(self):
        """Select init operator using Thompson Sampling with sliding window rewards."""
        min_trials = 2
    ...
    def _execute_init_operator(self):
        """Execute the selected init operator."""
        op = self._selected_init_operator
        
        if op == 0:
            self._init_operator_original()
        elif op == 1:
            self._init_operator_variant_1()
        elif op == 2:
            self._init_operator_variant_2()
        elif op == 3:
            self._init_operator_variant_3()
        elif op == 4:
            self._init_operator_variant_4()
        elif op == 5:
            self._init_operator_variant_5()
        elif op == 6:
            self._init_operator_variant_6()
        elif op == 7:
            self._init_operator_variant_7()
        else:
            self._init_operator_original()
    
    def _init_operator_original(self):
        """Original: Initialize tracking with default values."""
        self.best_fitness_history = []
        self.stagnation_counter = 0
        self.diversity_history = []
    
    def _init_operator_variant_1(self):
    ...
    def _init_operator_variant_2(self):
    ...
    def _init_operator_variant_3(self):
    ...
    def _init_operator_variant_4(self):
    ...
    def _init_operator_variant_5(self):
    ...
    def _init_operator_variant_6(self):
    ...
    def _init_operator_variant_7(self):
    ...
    def _record_pre_init_fitness(self):
        """Record fitness before restart for init operator reward calculation."""
        if hasattr(self, 'current_fitness') and self.current_fitness is not None:
            curr_fitness = np.min(self.current_fitness)
            if np.isfinite(curr_fitness):
                self._last_init_restart_fitness = float(curr_fitness)
            else:
                self._last_init_restart_fitness = 1e10
        else:
            self._last_init_restart_fitness = 1e10
    
    def _update_init_operator_reward_after_restart(self):
        """Update init operator reward based on improvement after restart."""
        if self._last_init_restart_fitness is None or len(self._init_restart_history) == 0:
            return
        ...
        prev_fitness = self._last_init_restart_fitness
    ...
    def _restart_optimizer(self):
        ...
        self._update_init_operator_reward_after_restart()
        
    def _reinitialize_distribution(self):
        """Reinitialize distribution parameters after restart."""
        self.mean = self._rng.uniform(
            self.bounds[0], self.bounds[1], size=self.dim
        )
        self.covariance = np.eye(self.dim)
