...
class CovarianceGuidedEvolutionStrategy:
    ...
    based on sliding-window credit assignment of improvement rewards.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Adaptive operator selection for _initialize_optimizer
        self.num_init_operators = 7
        self._init_operator_rewards = [[] for _ in range(self.num_init_operators)]
        self._init_reward_window_size = 10
        self._selected_init_operator = 0
        self._init_history = []
        self._init_trial_count = 0
        self._init_reward_baseline = []
        self._pending_init_reward = None
        self._init_evaluation_buffer = []
        self._init_evaluation_window = 50
        
        # Adaptive operator selection for _clear_archive (Thompson Sampling with sliding window)
    ...
    def _initialize_optimizer(self):
        ...
        self._execute_init_operator()
        self._init_trial_count += 1
        
    def _select_init_operator(self):
        """Select initialization operator using Thompson Sampling."""
        min_trials = 2
        
        if self._init_trial_count < min_trials:
        ...
        self._init_history.append({
            ...
            'trial': self._init_trial_count
        })
        
    def _record_pre_init_fitness(self):
        """Record fitness baseline before initialization."""
        if self.current_fitness is not None and len(self.current_fitness) > 0:
            curr_fitness = float(np.min(self.current_fitness))
            if np.isfinite(curr_fitness):
                self._init_reward_baseline.append(curr_fitness)
            else:
                self._init_reward_baseline.append(1e10)
        else:
            self._init_reward_baseline.append(1e10)
            
    def _track_init_performance(self):
        """Track performance after initialization to update rewards."""
        if self._pending_init_reward is not None and self.current_fitness is not None:
            curr_fitness = float(np.min(self.current_fitness))
            if not np.isfinite(curr_fitness):
                curr_fitness = 1e10
            
            baseline_idx = max(0, len(self._init_reward_baseline) - 1)
            if baseline_idx < len(self._init_reward_baseline):
                prev_fitness = self._init_reward_baseline[baseline_idx]
            else:
                prev_fitness = 1e10
            ...
            operator_idx = self._pending_init_reward
            rewards = self._init_operator_rewards[operator_idx]
            if len(rewards) >= self._init_reward_window_size:
                rewards.pop(0)
            rewards.append(reward)
            self._pending_init_reward = None
    
    def _execute_init_operator(self):
        """Execute the selected initialization operator."""
        op = self._selected_init_operator
        
        if op == 0:
            self._initialize_operator_original()
        elif op == 1:
            self._initialize_operator_variant_1()
        elif op == 2:
            self._initialize_operator_variant_2()
        elif op == 3:
            self._initialize_operator_variant_3()
        elif op == 4:
            self._initialize_operator_variant_4()
        elif op == 5:
            self._initialize_operator_variant_5()
        elif op == 6:
            self._initialize_operator_variant_6()
        else:
            self._initialize_operator_original()
        
        self._pending_init_reward = int(op)
    
    def _initialize_operator_original(self):
        """Original initialization operator."""
        ...
        offspring = np.array(offspring) if n > 0 else np.zeros((0, self.dim))
    ...
    def _initialize_operator_variant_1(self):
        """Variant 1: Exponential crossover operator for initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_operator_variant_2(self):
        """Variant 2: BLX-alpha crossover for initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_operator_variant_3(self):
        """Variant 3: Segment-based crossover for initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_operator_variant_4(self):
        """Variant 4: QR-rotation based initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_operator_variant_5(self):
        """Variant 5: Stratified grid initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_operator_variant_6(self):
        """Variant 6: PCA-based initialization."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _reinitialize_distribution(self):
        """Reinitialize the distribution mean and covariance."""
        low, high = self.bounds
        self.mean = self._rng.uniform(low, high, size=self.dim)
        self.covariance = np.eye(self.dim)
        self.step_size = 0.5 * (high - low)
        self.step_size = max(1e-10, self.step_size)
