...
class CovarianceGuidedEvolutionStrategy:
    ...
    based on sliding-window credit assignment of initialization improvement rewards.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Post-initialization evaluation tracking
        self._init_eval_counter = 0
        self._init_eval_window = 20
        self._best_after_init = None
    ...
    def _select_init_operator(self):
        ...
        if self._total_restarts < min_trials:
            self._selected_operator = int(self._rng.integers(0, self.num_operators))
        else:
    ...
    def _record_pre_init_fitness(self):
        ...
        if self.current_fitness is not None and len(self.current_fitness) > 0:
            ...
            if np.isfinite(curr_fitness):
                self._last_restart_best_fitness = curr_fitness
            else:
                self._last_restart_best_fitness = 1e10
        else:
            self._last_restart_best_fitness = 1e10
            
    def _update_best_after_init(self):
        """Track best fitness after initialization for reward calculation."""
        if self.current_fitness is not None and len(self.current_fitness) > 0:
            curr_best = float(np.min(self.current_fitness))
            if np.isfinite(curr_best):
                if self._best_after_init is None or curr_best < self._best_after_init:
                    self._best_after_init = curr_best
                self._init_eval_counter += 1
                
    def _update_operator_reward(self):
        """Update operator reward based on improvement after initialization."""
        if self._last_restart_best_fitness is None or len(self._restart_history) == 0:
            return
            
        if self._best_after_init is None:
            return
        ...
        curr_fitness = self._best_after_init
        
        if not np.isfinite(prev_fitness):
            prev_fitness = 1e10
    ...
    def _initialize_optimizer(self):
        """Initialize all optimizer state using selected operator."""
        ...
        if n <= 0:
            ...
            if op == 0:
                self._initialize_archive_original()
            elif op == 1:
                self._initialize_archive_variant_1()
            elif op == 2:
                self._initialize_archive_variant_2()
            elif op == 3:
                self._initialize_archive_variant_3()
            elif op == 4:
                self._initialize_archive_variant_4()
            elif op == 5:
                self._initialize_archive_variant_5()
            elif op == 6:
                self._initialize_archive_variant_6()
            else:
                self._initialize_archive_original()
    ...
    def _initialize_archive_original(self):
        """Original: simple uniform random initialization."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
        self.elite_archive = self._rng.uniform(self.bounds[0], self.bounds[1], size=(n, self.dim))
        
    def _initialize_archive_variant_1(self):
        """Variant 1: Exponential crossover operator."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
        num_parents = max(2, self.dim)
    ...
    def _initialize_archive_variant_2(self):
        """Variant 2: BLX-α crossover between two random parents."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
    ...
    def _initialize_archive_variant_3(self):
        """Variant 3: Segment-based crossover."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
        ...
        for i in range(n):
            ...
            if self.dim == 1:
                ...
                seg_start = int(self._rng.integers(0, self.dim))
                max_len = max(1, self.dim - seg_start)
                seg_len = int(self._rng.integers(1, max_len + 1))
            child = p1.copy()
    ...
    def _initialize_archive_variant_4(self):
        """Variant 4: QR decomposition rotation."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
    ...
    def _initialize_archive_variant_5(self):
        """Variant 5: Stratified/quasi-random initialization."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
    ...
    def _initialize_archive_variant_6(self):
        """Variant 6: Covariance eigendecomposition based initialization."""
        low, high = self.bounds
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
