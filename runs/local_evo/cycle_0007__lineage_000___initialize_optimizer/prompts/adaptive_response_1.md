...
class CovarianceGuidedEvolutionStrategy:
    ...
    Automatically selects the best _initialize_optimizer strategy using Thompson Sampling
    based on early improvement rewards during optimization.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # Adaptive operator selection for initialization (Thompson Sampling)
        self.num_init_operators = 6
        self._init_operator_rewards = [[] for _ in range(self.num_init_operators)]
        self._reward_window_size = 10
        self._selected_init_operator = 0
        self._init_history = []
        self._last_init_best_fitness = None
        self._total_inits = 0
        self._init_evaluation_count = 0
        self._reward_window_evaluations = 20
        self._fitness_at_init = None
        
        # Adaptive operator selection for clear_archive (Thompson Sampling with sliding window)
        self.num_clear_operators = 7
        self._clear_operator_rewards = [[] for _ in range(self.num_clear_operators)]
        self._selected_clear_operator = 0
        self._clear_history = []
    ...
    def __call__(self, func, stopping_condition):
        ...
        self._finalize_init_reward()
        return self._get_best_result()
    
    def _select_init_operator(self):
        """Select initialization operator using Thompson Sampling."""
        min_trials = 2
        
        if self._total_inits < min_trials:
            self._selected_init_operator = self._rng.integers(0, self.num_init_operators)
        else:
            samples = np.zeros(self.num_init_operators)
            for i in range(self.num_init_operators):
                rewards = self._init_operator_rewards[i]
                if len(rewards) > 0:
            ...
            self._selected_init_operator = int(np.argmax(samples))
        
        self._init_history.append({
            'operator_idx': int(self._selected_init_operator),
            'iteration': len(self._init_history)
        })
    
    def _check_init_reward(self, func):
        """Check if we should compute and record reward for initialization."""
        if self._fitness_at_init is None:
            return
        
        self._init_evaluation_count += len(self.current_fitness)
        
        if self._init_evaluation_count >= self._reward_window_evaluations:
            self._update_init_operator_reward()
            self._fitness_at_init = None
            self._init_evaluation_count = 0
    
    def _finalize_init_reward(self):
        """Finalize initialization reward if not already computed."""
        if self._fitness_at_init is not None:
            self._update_init_operator_reward()
            self._fitness_at_init = None
            self._init_evaluation_count = 0
    
    def _update_init_operator_reward(self):
        """Update initialization operator reward based on early improvement."""
        if self._fitness_at_init is None or len(self._init_history) == 0:
            return
        
        last_entry = self._init_history[-1]
        ...
        prev_fitness = self._fitness_at_init
        ...
        rewards = self._init_operator_rewards[operator_idx]
        if len(rewards) >= self._reward_window_size:
            rewards.pop(0)
        rewards.append(reward)
    
    def _initialize_optimizer(self):
        """Initialize all optimizer state variables using selected operator."""
        self._initialize_distribution()
        
        op = self._selected_init_operator
        if op == 0:
            self._init_archive_original()
        elif op == 1:
            self._init_archive_variant_1()
        elif op == 2:
            self._init_archive_variant_2()
        elif op == 3:
            self._init_archive_variant_3()
        elif op == 4:
            self._init_archive_variant_4()
        elif op == 5:
            self._init_archive_variant_5()
        else:
            self._init_archive_original()
    ...
    def _init_archive_original(self):
        """Original initialization: simple BLX-alpha crossover."""
        n = self.archive_size
        if n <= 0:
            self.elite_archive = np.zeros((0, self.dim))
            return
        low, high = self.bounds
        parent1 = self._rng.uniform(low, high, size=self.dim)
        parent2 = self._rng.uniform(low, high, size=self.dim)
        alpha = 0.5
        offspring = []
        for _ in range(n):
    ...
    def _init_archive_variant_1(self):
    ...
    def _init_archive_variant_2(self):
        """Variant 2: BLX-alpha crossover with paired parents."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _init_archive_variant_3(self):
        """Variant 3: Segment-based crossover: swap random contiguous segments."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _init_archive_variant_4(self):
        """Variant 4: Rotate archive using QR decomposition."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _init_archive_variant_5(self):
        """Variant 5: Deterministic quasi-random stratified sampling."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _initialize_tracking(self):
        """Initialize tracking variables after archive setup."""
    ...
    def _sample_and_evaluate(self, func):
        ...
        if self._fitness_at_init is None:
            valid_fitness = self.current_fitness[np.isfinite(self.current_fitness)]
            if len(valid_fitness) > 0:
                self._fitness_at_init = float(np.min(valid_fitness))
            else:
                self._fitness_at_init = 1e10
    ...
    def _update_covariance(self, selected_pop):
        ...
        rank_mu /= max(1, len(selected_pop))
    ...
    def _compute_adaptation_learning_rates(self):
        ...
        pop_ratio = self.pop_size / max(1, self.dim)
    ...
    def _adjust_step_size(self, improvement_rate):
        ...
        if improvement_rate >= 0:
            ...
            self.step_size *= 0.9
        low, high = self.bounds
        max_step = max(1e-10, (high - low) * 0.5) if high > low else 0.5
        self.step_size = np.clip(self.step_size, 1e-10, max_step)
    ...
    def _add_to_archive(self, new_elites):
        ...
        if len(self.elite_archive) == 0:
            self.elite_archive = new_elites.copy() if len(new_elites) > 0 else np.zeros((0, self.dim))
        elif len(new_elites) > 0:
    ...
    def _manage_stagnation(self):
        ...
        low, high = self.bounds
        self.diversity_threshold = 1e-6 * (high - low)
    ...
    def _select_clear_archive_operator(self):
        ...
        if self._total_restarts < min_trials:
            self._selected_clear_operator = self._rng.integers(0, self.num_clear_operators)
        else:
            samples = np.zeros(self.num_clear_operators)
            for i in range(self.num_clear_operators):
                rewards = self._clear_operator_rewards[i]
                if len(rewards) > 0:
                    mean_reward = float(np.mean(rewards))
                    std_reward = float(np.std(rewards)) if len(rewards) > 1 else 0.1
                    std_reward = max(0.01, std_reward)
            ...
            self._selected_clear_operator = int(np.argmax(samples))
        
        self._clear_history.append({
            'operator_idx': int(self._selected_clear_operator),
            'iteration': len(self._clear_history)
        })
    ...
    def _restart_optimizer(self):
        ...
        self._update_clear_operator_reward_after_restart()
        
    def _update_clear_operator_reward_after_restart(self):
        """Update operator reward based on improvement after restart."""
        if self._last_restart_best_fitness is None or len(self._clear_history) == 0:
            return
            
        last_entry = self._clear_history[-1]
        operator_idx = last_entry['operator_idx']
        ...
        rewards = self._clear_operator_rewards[operator_idx]
        if len(rewards) >= self._reward_window_size:
            rewards.pop(0)
        rewards.append(reward)
    ...
    def _clear_archive(self):
        ...
        op = self._selected_clear_operator
    ...
    def _reinitialize_distribution(self):
        """Reinitialize distribution parameters."""
        low, high = self.bounds
        rng = high - low
        if rng == 0:
            rng = 1.0
    ...
    def _get_best_result(self):
        ...
        if not np.any(valid_mask):
            return 1e10, np.zeros(self.dim)
        
        valid_fitness = self.current_fitness[valid_mask]
        valid_pop = self.current_population[valid_mask]
        
        best_idx = np.argmin(valid_fitness)
        best_position = valid_pop[best_idx]
        best_fitness = float(valid_fitness[best_idx])
