...
class CovarianceGuidedEvolutionStrategy:
    ...
    with an adaptive operator selection mechanism for the archive clearing strategy.
    Uses Thompson Sampling (Multi-Armed Bandit) to learn which _clear_archive
    operator works best during optimization.
    """
    ...
    def __init__(self, dim=30, pop_size=None, bounds=(-100, 100)):
        ...
        # 7 operators: original + 6 variants
        self.num_operators = 7
        self.operator_names = [
            'original', 'variant_1', 'variant_2', 'variant_3',
            'variant_4', 'variant_5', 'variant_6'
        ]
        # Beta distribution parameters (alpha, beta) for Thompson Sampling
        ...
        # Selection counts per operator
        self.operator_selections = np.zeros(self.num_operators, dtype=np.int64)
        # Cumulative rewards per operator
        self.operator_rewards = np.zeros(self.num_operators, dtype=np.float64)
        # Sliding window for recent performance
        self.recent_improvements = []
        self.max_recent_history = 20
        # Track performance before and after operator application
        self._fitness_before_clear = None
        self._selected_operator_idx = None
        # Warmup period before using adaptive selection
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if self._should_restart():
                # Record fitness before clearing for reward calculation
                if len(self.current_fitness) > 0:
                    self._fitness_before_clear = float(np.min(self.current_fitness))
    ...
    def _restart_optimizer(self):
        """Perform strategic restart of the optimizer with adaptive operator selection."""
        # Update reward for previously selected operator
        if self._selected_operator_idx is not None and self._fitness_before_clear is not None:
            self._update_operator_reward(self._selected_operator_idx)
        
        # Select next operator using Thompson Sampling
        operator_idx = self._select_operator_thompson()
        self._selected_operator_idx = operator_idx
        self.operator_selections[operator_idx] += 1
        self.restart_count += 1
        
        # Perform restart
        self._reinitialize_distribution()
        self._apply_clear_archive_operator(operator_idx)
        self._reset_tracking()
        
    def _select_operator_thompson(self):
        """Select operator using Thompson Sampling with Beta distribution."""
        # During warmup, use round-robin to get initial estimates
        if self.restart_count < self.warmup_period:
            return self.restart_count % self.num_operators
        
        # Thompson Sampling: sample from Beta distribution for each operator
        samples = np.array([
            self._rng.beta(
                float(max(1, self.operator_alpha[i])),
                float(max(1, self.operator_beta[i]))
            )
            for i in range(self.num_operators)
        ], dtype=np.float64)
        
        # Handle NaN/Inf in samples
        samples = np.nan_to_num(samples, nan=0.5, posinf=1.0, neginf=0.0)
        
        selected_idx = int(np.argmax(samples))
        return selected_idx
    
    def _update_operator_reward(self, operator_idx):
        """Update reward for the selected operator based on performance improvement."""
        if self._fitness_before_clear is None or len(self.current_fitness) == 0:
            return
        
        current_best = float(np.min(self.current_fitness))
        fitness_before = float(self._fitness_before_clear)
        
        # Calculate improvement ratio (negative means improvement for minimization)
        if fitness_before != 0:
            improvement = (fitness_before - current_best) / (abs(fitness_before) + 1e-10)
        else:
            improvement = fitness_before - current_best
        
        # Normalize improvement to [0, 1] reward
        # Higher reward = better operator (led to larger improvement)
        normalized_improvement = 0.5 + np.clip(improvement / (1.0 + abs(fitness_before)), -0.5, 0.5)
        reward = float(np.clip(normalized_improvement, 0.0, 1.0))
        
        # Update Beta distribution parameters
        self.operator_alpha[operator_idx] += reward
        self.operator_beta[operator_idx] += (1.0 - reward)
        
        # Track cumulative reward
        self.operator_rewards[operator_idx] += reward
        
        # Update sliding window for recent improvements
        self.recent_improvements.append(reward)
        if len(self.recent_improvements) > self.max_recent_history:
            self.recent_improvements.pop(0)
    
    def _apply_clear_archive_operator(self, operator_idx):
        """Apply the selected _clear_archive operator."""
        if operator_idx == 0:
    ...
    # ===== CLEAR ARCHIVE OPERATORS =====
    
    def _clear_archive_original(self):
        """Clear the elite archive (original implementation)."""
        self.elite_archive = np.zeros((0, self.dim))
    ...
    def _clear_archive_variant_2(self):
        """Arithmetic blend (BLX-α) crossover between archive centroid and current mean."""
