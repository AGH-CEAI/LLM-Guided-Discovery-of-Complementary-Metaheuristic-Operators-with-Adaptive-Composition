This is iteration 9 of 10. Propose ONE replacement implementation for `_check_stagnation`.
...
- Keep the EXACT function signature: `def _check_stagnation(self, best_fitness):`
...
═══════════════════════════════════════════════════════════════════════
FOCUS TASK FOR THIS ITERATION: task 17
PHASE 2 (highest-variance task selection). The focus task was picked because current variants DISAGREE most on it — meaning the choice of operator has the most leverage here. A new variant that's distinct from prior ones is especially valuable on this task.
═══════════════════════════════════════════════════════════════════════

CURRENT VARIANTS' PERFORMANCE ON TASK 17 (lower = better):
  original.py                               7.8359e+00
  variant_01_catA_t08_idea_0.py             6.5704e+00
  variant_02_catB_t15_idea_0.py             4.0923e+02
  variant_04_catD_t06_idea_0.py             4.8072e+03
  variant_05_catE_t23_idea_0.py             8.2357e+00
  variant_06_catF_t17_idea_0.py             6.6310e+03
  variant_07_catG_t17_idea_0.py             7.1355e+03
  variant_08_catH_t17_idea_0.py             8.0881e+02

HISTORICAL BEST on task 17 across the entire run: 6.5704e+00  (achieved by `variant_01_catA_t08_idea_0.py` in a previous cycle).
Your job is to BEAT this number — but with a fundamentally different approach, not by imitating an algorithm whose source you do not have access to.

The historical best is many orders of magnitude above the convergence target (1e-08). This task likely needs a STRUCTURALLY DIFFERENT approach, not a refinement of existing methods. Don't reproduce what's already been tried — invent something new from your assigned category.
...
YOUR TASK:
Write a fundamentally new implementation of `_check_stagnation` that, working
through the lens of your assigned category 'Geometry / spatial', is a candidate
that has a real chance of beating 6.570e+00 on task 17 (the run-history best from `variant_01_catA_t08_idea_0.py`). It is acceptable for your variant to be WORSE than
existing variants on other tasks — the adaptive selector will route
between variants per task. What matters is that THIS task gets a
candidate that's distinct from prior approaches AND firmly within your
assigned category.

Full algorithm for context (the body of `_check_stagnation` has been STUBBED OUT
on purpose — your job is to write the body from scratch, NOT copy what
was there. The stub's return value is a placeholder, not a hint about
the right answer):
```python
import numpy as np
...
class AdaptiveCompassDE:
    ...
    def _check_stagnation(self, best_fitness):
        ...
        # [STUB — your job is to write the body of `_check_stagnation` from scratch.
        #  The signature is fixed; the implementation must be your own design.]
        return None
...
**Idea: Short Name**
One-line description (must mention category A: Geometry / spatial and target task 17).
```python
