...
- 7 distinct winners, with wins fairly distributed (6, 5, 4, 3, 1, 1, 1)
- All gaps to runner-up are <2× (no DISPATCH-MANDATORY tasks)
- One blend-hostile task (17): 123× gap to median competitor, but only 1.2× to runner-up

The win distribution favors a bandit-based approach, but the per-task diversity means no single stagnation method wins everywhere. I'll implement a **Thompson Sampling bandit with a probe-and-commit phase**: the algorithm empirically tests each stagnation detector during a brief probe phase, then commits to the best performer.
