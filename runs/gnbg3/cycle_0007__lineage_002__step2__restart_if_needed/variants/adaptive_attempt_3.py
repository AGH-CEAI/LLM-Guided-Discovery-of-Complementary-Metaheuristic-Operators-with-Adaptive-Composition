...
class AdaptiveCompassDE:
    """
    ADAPTIVE RESTART SELECTION via PROBE-AND-COMMIT (mechanism #4 contextual bandit).
    
    The per-task winner-gap table shows:
    - Two distinct winners (original.py: 10 wins, variant_08: 11 wins)
    - ALL gaps are <3× — no task is blend-hostile (no gap >= 10×)
    - Win counts are nearly even, suggesting neither variant dominates universally
    
    The adaptation mechanism:
    - PHASE A (PROBE): Run both restart strategies for ~5% of budget, measure
      diversity improvement (rank-normalized) from each restart.
    - PHASE B (COMMIT): Use the empirically better restart strategy; with small
      probability epsilon_revisit, try the alternative if the committed strategy
      stagnates.
    
    This is NOT ensemble blending — we commit to ONE strategy after probing,
    preserving any per-task advantage without mixing. The probe is a contextual
    bandit where the "context" is the empirical diversity signal from the landscape.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # --- ADAPTIVE RESTART SELECTION (probe-and-commit) ---
        # Possible restart strategies
        self._restart_strategies = {
            'original': self._restart_original,
            'hybrid': self._restart_hybrid,
        }
        self._selected_restart = 'original'  # default until probe
        self._probe_budget = max(30, int(0.05 * self.max_iter))  # ~5% of budget
        self._probe_iter = 0
        self._in_probe_phase = True
        
        # Per-strategy performance tracking (rank-based metrics)
        self._restart_diversity_scores = {k: [] for k in self._restart_strategies}
        self._restart_improvement_scores = {k: [] for k in self._restart_strategies}
        self._restart_success_counts = {k: 0 for k in self._restart_strategies}
        self._restart_total_counts = {k: 0 for k in self._restart_strategies}
        
        # Post-commit exploration
        self._epsilon_revisit = 0.08  # small prob to try alternative after commit
        self._consecutive_poor_restarts = 0
        self._poor_restart_threshold = 3
    ...
    def _restart_hybrid(self, population, fitness):
    ...
    def _restart_original(self, population, fitness):
        """Restart based on k-NN graph connectivity analysis (original method)."""
    ...
    def _probe_restart_strategies(self, population, fitness, func):
        """Run probe phase to compare restart strategies using rank-based diversity improvement."""
        probe_budget = min(self._probe_budget, 50)
        
        pop_original = population.copy()
        fit_original = fitness.copy()
        pop_hybrid = population.copy()
        fit_hybrid = fitness.copy()
        
        diversity_scores = {'original': [], 'hybrid': []}
        improvement_scores = {'original': [], 'hybrid': []}
        
        for p_iter in range(probe_budget):
            best_orig_before = np.nanmin(fit_original)
            best_hyb_before = np.nanmin(fit_hybrid)
            
            pop_original, f_o, x_o = self._restart_original(pop_original, fit_original)
            pop_hybrid, f_h, x_h = self._restart_hybrid(pop_hybrid, fit_hybrid)
            
            fit_original = func(pop_original)
            fit_hybrid = func(pop_hybrid)
            
            if len(fit_original) < len(pop_original):
                fit_original = np.full(len(pop_original), np.nan)
            if len(fit_hybrid) < len(pop_hybrid):
                fit_hybrid = np.full(len(pop_hybrid), np.nan)
            
            best_orig_after = np.nanmin(fit_original)
            best_hyb_after = np.nanmin(fit_hybrid)
            
            spatial_orig = self._compute_diversity(pop_original)
            spatial_hybrid = self._compute_diversity(pop_hybrid)
            
            valid_orig = fit_original[~np.isnan(fit_original)]
            valid_hybrid = fit_hybrid[~np.isnan(fit_hybrid)]
            
            fit_var_orig = np.std(valid_orig) / (np.abs(np.mean(valid_orig)) + 1e-10) if len(valid_orig) > 1 else 0
            fit_var_hybrid = np.std(valid_hybrid) / (np.abs(np.mean(valid_hybrid)) + 1e-10) if len(valid_hybrid) > 1 else 0
            
            diversity_scores['original'].append(spatial_orig + fit_var_orig)
            diversity_scores['hybrid'].append(spatial_hybrid + fit_var_hybrid)
            
            improvement_scores['original'].append(max(0, best_orig_before - best_orig_after))
            improvement_scores['hybrid'].append(max(0, best_hyb_before - best_hyb_after))
        
        median_orig = np.median(diversity_scores['original']) if diversity_scores['original'] else 0
        median_hybrid = np.median(diversity_scores['hybrid']) if diversity_scores['hybrid'] else 0
        
        if median_hybrid > median_orig * 1.05:
            return 'hybrid'
        elif median_orig > median_hybrid * 1.05:
            return 'original'
        else:
            return 'original'
        ...
        while not stopping_condition():
            ...
            if self._in_probe_phase:
                if self._probe_iter >= self._probe_budget:
                    self._selected_restart = self._probe_restart_strategies(population, fitness, func)
            ...
            if self._check_stagnation(f_opt):
                restart_func = self._restart_strategies[self._selected_restart]
                population, f_opt, x_opt = restart_func(population, fitness)
                
                if not self._in_probe_phase:
                    self._restart_total_counts[self._selected_restart] += 1
                    old_best = self.prev_best_fitness
                    new_best = f_opt
                    if old_best - new_best > 1e-8:
                        self._restart_success_counts[self._selected_restart] += 1
                        self._consecutive_poor_restarts = 0
                    else:
                        self._consecutive_poor_restarts += 1
                    
                    if (np.random.rand() < self._epsilon_revisit or 
                        self._consecutive_poor_restarts >= self._poor_restart_threshold):
                        alt = 'hybrid' if self._selected_restart == 'original' else 'original'
                        self._selected_restart = alt
                        self._consecutive_poor_restarts = 0
