Looking at the gap table: max gap is only 2.7× (task 17), no blend-hostile tasks, and only 2 distinct winners with nearly even win counts. The two restart strategies are correlated (both detect convergence) but use different signals. A discrete bandit with Thompson Sampling is appropriate here — it commits to one strategy at a time, which is safe given the small gaps, while still learning which strategy to prefer online. The key is that rank-based rewards within a sliding window avoid scale-dependent magic constants.
