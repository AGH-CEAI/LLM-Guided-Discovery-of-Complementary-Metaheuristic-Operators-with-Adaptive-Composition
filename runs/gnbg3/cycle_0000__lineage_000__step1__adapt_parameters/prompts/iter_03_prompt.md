...
CURRENT VARIANTS' PERFORMANCE ON TASK 12 (lower = better):
  original.py                               2.6299e-01
  variant_01_catA_t16_idea_0.py             1.1733e+01

HISTORICAL BEST on task 12 across the entire run: 2.6299e-01  (achieved by `original.py` in a previous cycle).
...
that has a real chance of beating 2.630e-01 on task 12 (the run-history best from `original.py`). It is acceptable for your variant to be WORSE than
...
**Idea: Short Name**
One-line description (must mention category C: Information-theoretic / distributional and target task 12).
```python
