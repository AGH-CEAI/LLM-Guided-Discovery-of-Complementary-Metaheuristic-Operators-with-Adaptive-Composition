This is iteration 5 of 10. Propose ONE replacement implementation for `_mutate_compass_batch`.
...
- Keep the EXACT function signature: `def _mutate_compass_batch(self, population, fitness):`
...
═══════════════════════════════════════════════════════════════════════
FOCUS TASK FOR THIS ITERATION: task 16
PHASE 1 (random non-trivial task selection). The focus task was picked at random from the non-trivial pool. Coverage matters more than convergence here — even if your variant is mediocre on this task, that's fine as long as it represents a fundamentally different approach.
═══════════════════════════════════════════════════════════════════════

CURRENT VARIANTS' PERFORMANCE ON TASK 16 (lower = better):
  original.py                               8.3200e+02
  variant_01_catA_t13_idea_0.py             2.0013e+03
  variant_02_catB_t16_idea_0.py             1.4904e+03
  variant_03_catC_t19_idea_0.py             2.0974e+03

HISTORICAL BEST on task 16 across the entire run: 4.2899e+01  (achieved by `variant_02_catB_t21_idea_0.py` in a previous cycle).
...
YOUR TASK:
Write a fundamentally new implementation of `_mutate_compass_batch` that, working
through the lens of your assigned category 'Topology / graph-based', is a candidate
that has a real chance of beating 4.290e+01 on task 16 (the run-history best from `variant_02_catB_t21_idea_0.py`). It is acceptable for your variant to be WORSE than
...
Full algorithm for context (the body of `_mutate_compass_batch` has been STUBBED OUT
...
class AdaptiveCompassDE:
    ...
    def _mutate_compass_batch(self, population, fitness):
        ...
        # [STUB — your job is to write the body of `_mutate_compass_batch` from scratch.
...
**Idea: Short Name**
One-line description (must mention category E: Topology / graph-based and target task 16).
```python
def _mutate_compass_batch(self, ...):
    ...
```