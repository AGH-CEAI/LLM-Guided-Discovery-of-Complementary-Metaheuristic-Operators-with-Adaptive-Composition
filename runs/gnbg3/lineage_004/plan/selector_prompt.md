...
CANDIDATE methods with metadata (you MUST pick exactly 3 distinct names):
  ...
  _update_strategy_scores                      control_flow 78   in_main_loop  called_by=[__call__]
...
(no prior cycles in this lineage)
  ...
  Methods excluded (banned: weighted cost >= 2.0 OR rejected streak >= 3): _crossover_batch, _mutate_compass_batch
  Global pick costs so far (method: weighted_cost — where strong-improvement picks cost 0, weak-improvement picks cost 0.5, rejected picks cost 1.0):
    _crossover_batch: 2.00
    _mutate_compass_batch: 2.00
    _update_strategy_scores: 1.50
