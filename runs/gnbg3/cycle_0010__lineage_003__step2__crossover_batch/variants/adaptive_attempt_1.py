...
class AdaptiveCompassDE:
    """
    ADAPTATION MECHANISM: #4 — Per-Task Probe-and-Commit (Contextual Bandit, one-shot context).
    
    The per-task gap table is BLEND-HOSTILE: 6 tasks have winner/runner-up gaps >= 100×,
    and the worst case (task 4) has a 1.38-million× gap. A 50/50 linear blend of
    1e-8 and 1.38e-2 yields ~6.9e-3 — completely destroying the winner's advantage.
    Ensemble blending (#2) is therefore FORBIDDEN.
    
    Instead, this implementation uses a PROBE-AND-COMMIT pattern:
      PHASE A (PROBE, ~5% of budget): Round-robin across 4 crossover arms for a few
        generations. Compute rank-based score vs. the original baseline. Commit to the
        arm with the best probe score (weight = 1.0, no blending).
      PHASE B (COMMIT, ~95% of budget): Run the chosen crossover exclusively for the
        remaining budget. A small stagnation-triggered fallback (~5%) to the original
        prevents total failure on misclassified landscapes.
    
    This preserves per-task winner advantages because the committed arm gets weight 1.0,
    exactly matching the dispatch-mandatory requirement from the gap table.
    
    Win counts from benchmark: original=15, variant_01=4, variant_07=2, variant_02=1.
    The probe learns which crossover geometry fits the current landscape.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # --- CROSSOVER ARM SELECTION (probe-and-commit) ---
        # 4 arms: original (baseline), centroid-geom, SVD-spectral, LHS-Beta
        self._crossover_arms = ['original', 'centroid_geom', 'svd_spectral', 'lhs_beta']
        self._probe_scores = {arm: [] for arm in self._crossover_arms}
        self._probe_best_fitness = {arm: [] for arm in self._crossover_arms}
        self._probe_gen_best = []  # track global best per probe gen for scoring
        self._committed_arm = None
        self._commit_generation = None
        self._probe_complete = False
        
        # Per-arm success counters for stagnation fallback
        self._arm_success_counts = {arm: 0 for arm in self._crossover_arms}
        self._arm_total_counts = {arm: 0 for arm in self._crossover_arms}
    ...
    # =====================================================================
    # CROSSOVER ARMS (4 candidates for probe-and-commit)
    # =====================================================================
    
    def _crossover_original(self, population, mutants):
        """Standard binomial crossover (baseline arm)."""
    ...
    def _crossover_centroid_geom(self, population, mutants):
        """Centroid-proximity weighting (variant_01 geometry)."""
    ...
    def _crossover_svd_spectral(self, population, mutants):
        """SVD spectral-weighted per-dimension CR (variant_02)."""
        NP, dim = population.shape
    ...
    def _crossover_lhs_beta(self, population, mutants):
        """LHS-guided binomial with adaptive Beta-distributed CR (variant_07)."""
        NP, dim = population.shape
    ...
    def _dispatch_crossover(self, arm, population, mutants):
        """Route to the selected crossover arm."""
        if arm == 'original':
            return self._crossover_original(population, mutants)
        elif arm == 'centroid_geom':
            return self._crossover_centroid_geom(population, mutants)
        elif arm == 'svd_spectral':
            return self._crossover_svd_spectral(population, mutants)
        elif arm == 'lhs_beta':
            return self._crossover_lhs_beta(population, mutants)
        else:
            return self._crossover_original(population, mutants)
    ...
    def _run_probe_phase(self, func, population, fitness, current_gen, total_budget):
        """
        Probe phase: round-robin across 4 crossover arms for probe_budget generations.
        Each arm gets equal generations. Track rank-based improvement.
        """
        probe_budget = max(3, int(0.05 * total_budget))  # 5% of budget, min 3
        
        if current_gen >= probe_budget:
            self._probe_complete = True
            return None, None, current_gen
        
        arm = self._crossover_arms[current_gen % len(self._crossover_arms)]
        
        # Store global best at start of this probe gen
        best_f = np.nanmin(fitness)
        self._probe_gen_best.append(best_f)
        
        # Mutate (compass + ensemble blend)
        mutants_compass = self._mutate_compass_batch(population, fitness)
        mutants_ensemble, _ = self._mutate_ensemble_batch(population, fitness)
        blend_ratio = np.random.rand(self.NP, 1)
        mutants = np.where(blend_ratio < 0.6, mutants_compass, mutants_ensemble)
        
        # Crossover with current probe arm
        trials = self._dispatch_crossover(arm, population, mutants)
        ...
        # Track arm success
        self._arm_success_counts[arm] += np.sum(better)
        self._arm_total_counts[arm] += len(better)
        
        # Track rank-based improvement for this arm (relative to global best)
        new_best = np.nanmin(new_fit)
        rank_improvement = (best_f - new_best) / (abs(best_f) + 1e-12) if best_f > new_best else 0.0
        self._probe_scores[arm].append(rank_improvement)
        
        return new_pop, new_fit, current_gen + 1
    
    def _compute_probe_decision(self):
        """
        After probe phase: pick the arm with the best rank-based score.
        Uses cumulative rank score (sum of rank improvements across probe gens).
        Falls back to original if no arm clearly wins.
        """
        arm_total_scores = {}
        for arm in self._crossover_arms:
            scores = self._probe_scores[arm]
            if len(scores) == 0:
                arm_total_scores[arm] = -np.inf
            else:
                # Rank-based: sum of rank improvements (scale-invariant)
                arm_total_scores[arm] = np.sum(scores)
        
        # Also check success rate as secondary signal
        arm_success_rates = {}
        for arm in self._crossover_arms:
            if self._arm_total_counts[arm] > 0:
                arm_success_rates[arm] = self._arm_success_counts[arm] / self._arm_total_counts[arm]
            else:
                arm_success_rates[arm] = 0.0
        
        # Primary: best cumulative rank score
        best_arm = max(arm_total_scores, key=arm_total_scores.get)
        best_score = arm_total_scores[best_arm]
        
        # If best score is not meaningfully positive, default to original
        if best_score <= 0.0:
            self._committed_arm = 'original'
        else:
            self._committed_arm = best_arm
        
        # Sanity: if committed arm has zero success rate, revert to original
        if arm_success_rates[self._committed_arm] == 0.0:
            self._committed_arm = 'original'
    ...
    def _update_archive(self, population, fitness, improved_mask):
        """Maintain archive of improved solutions."""
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if not self._probe_complete:
                new_pop, new_fit, iteration = self._run_probe_phase(
                    func, population, fitness, iteration, total_budget
                )
                if new_pop is not None:
                    ...
                    if iteration >= max(3, int(0.05 * total_budget)):
                        self._compute_probe_decision()
                        self._commit_generation = iteration
                        iteration = 0  # Reset for commit phase
            ...
            # --- PHASE B: COMMIT ---
            committed = getattr(self, '_committed_arm', 'original')
            
            # Stagnation-triggered fallback: small probability to switch to original
            if self.stagnation_counter > 30 and committed != 'original':
            ...
            # Crossover: use committed arm (weight = 1.0, no blending)
            trials = self._dispatch_crossover(committed, population, mutants)
