...
CURRENT VARIANTS' PERFORMANCE ON TASK 14 (lower = better):
  original.py                               3.5096e+00
  variant_01_catA_t16_idea_0.py             1.4853e+01
  variant_02_catB_t21_idea_0.py             6.0274e+00

HISTORICAL BEST on task 14 across the entire run: 3.5096e+00  (achieved by `original.py` in a previous cycle).
...
that has a real chance of beating 3.510e+00 on task 14 (the run-history best from `original.py`). It is acceptable for your variant to be WORSE than
...
**Idea: Short Name**
One-line description (must mention category C: Information-theoretic / distributional and target task 14).
```python
