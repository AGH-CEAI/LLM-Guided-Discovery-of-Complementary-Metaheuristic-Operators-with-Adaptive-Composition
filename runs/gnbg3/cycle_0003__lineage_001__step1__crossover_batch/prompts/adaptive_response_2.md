Looking at the gap table, there are **6 tasks with gaps ≥100×** and **10 blend-hostile tasks (gaps ≥10×)**, so ensemble blending is mathematically excluded. The correct mechanism is **PROBE-AND-COMMIT (mechanism #4 / contextual bandit)** — the context is computed once from the landscape at the start of the run, and the committed operator gets weight 1.0.
