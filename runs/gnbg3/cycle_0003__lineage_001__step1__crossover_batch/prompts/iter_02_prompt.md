...
CURRENT VARIANTS' PERFORMANCE ON TASK 21 (lower = better):
  original.py                               4.6329e+00
  variant_01_catA_t16_idea_0.py             5.9125e+00

HISTORICAL BEST on task 21 across the entire run: 4.6329e+00  (achieved by `original.py` in a previous cycle).
...
that has a real chance of beating 4.633e+00 on task 21 (the run-history best from `original.py`). It is acceptable for your variant to be WORSE than
...
**Idea: Short Name**
One-line description (must mention category B: Spectral / linear-algebraic and target task 21).
```python
