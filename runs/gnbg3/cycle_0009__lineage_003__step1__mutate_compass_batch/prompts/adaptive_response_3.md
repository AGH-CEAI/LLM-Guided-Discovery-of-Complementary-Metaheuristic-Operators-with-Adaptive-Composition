...
class AdaptiveCompassDE:
    """
    PROBE-AND-COMMIT (mechanism #4, contextual bandit with one-shot context).
    
    Per-task gap table analysis:
    - 8 DISPATCH-MANDATORY tasks (gap >= 100×): original.py wins tasks 0-22 by
      margins up to 25,000,000×; variant_08 wins task 23 by 1.03×.
    - BLEND-HOSTILE confirmed: linear blending CANNOT preserve the original's
      advantage on tasks 0,1,2,4,5,7,8,10,12 where it beats runner-up by >=100×.
    - 2 distinct winners across 24 tasks.
    
    Mechanism rationale:
    - PROBE-AND-COMMIT is mandated because the gap table is blend-hostile and
      has 2 distinct winners. A 50/50 blend of values 1 and 100 is 50.5, not 1.
    - The probe (~5 generations × 2 strategies × 6D evals) extracts a
      rank-based improvement fingerprint from the actual landscape, then commits
      exclusively (weight 1.0) to avoid diluting the winner's advantage.
    - Rank-based rewards are scale-invariant and avoid the forbidden
      scale-dependent constants (rule a).
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self._probe_original_improvements = []
        self._probe_variant_improvements = []
        self._probe_gen = 0
        self._probe_budget = 5  # generations per strategy
    ...
    def _mutate_variant_08(self, population, fitness):
        """
        variant_08_catH_t05_idea_0.py's _mutate_compass_batch implementation.
        Uses FDC-based exploitation/exploration balancing with centroid tracking.
        """
        ...
        # Mechanism 1: Spatial centroid direction (exploration-biased)
        direction_to_centroid = centroid - population
        centroid_mutations = population + explore_weight * self.F * direction_to_centroid
        
        # Mechanism 2: Fitness-rank based direction (exploitation-biased)
        ...
        for i in range(NP):
            ...
            # Add scaled noise proportional to centroid velocity
    ...
    def _run_probe_phase(self, func, population, fitness):
        """
        Probe phase: alternate between original and variant_08 for probe_budget
        generations each. Track rank-based improvement rates.
        
        Rank-based improvement: fraction of individuals whose rank improved
        (lower rank = better fitness). Scale-invariant by design (rule a).
        """
        prev_ranks = self._compute_fitness_ranking(fitness)
        
        for probe_step in range(self._probe_budget * 2):
            # Determine which strategy to test in this probe generation
            is_variant = (probe_step % 2 == 1)
            strategy_name = 'variant_08' if is_variant else 'original'
            
            # Generate mutants with the probed strategy
            if is_variant:
                mutants = self._mutate_variant_08(population, fitness)
            else:
            ...
            # Compute rank-based improvement rate (scale-invariant, rule a)
            curr_ranks = self._compute_fitness_ranking(fitness)
            rank_improved = (curr_ranks < prev_ranks).astype(float)
            rank_improvement_rate = np.nanmean(rank_improved)
            
            if is_variant:
                self._probe_variant_improvements.append(rank_improvement_rate)
            else:
                self._probe_original_improvements.append(rank_improvement_rate)
            
            prev_ranks = curr_ranks
        
        # Statistical decision: t-test on rank-based improvement rates
        if len(self._probe_original_improvements) >= 2 and len(self._probe_variant_improvements) >= 2:
            orig_mean = np.mean(self._probe_original_improvements)
            var_mean = np.mean(self._probe_variant_improvements)
            orig_std = np.std(self._probe_original_improvements, ddof=1) + 1e-10
            var_std = np.std(self._probe_variant_improvements, ddof=1) + 1e-10
            n_orig = len(self._probe_original_improvements)
            n_var = len(self._probe_variant_improvements)
            
            # Welch's t-test for unequal variances
            t_stat = (orig_mean - var_mean) / np.sqrt((orig_std ** 2 / n_orig) + (var_std ** 2 / n_var))
            df = ((orig_std ** 2 / n_orig) + (var_std ** 2 / n_var)) ** 2 / (
                ((orig_std ** 2 / n_orig) ** 2 / (n_orig - 1)) +
                ((var_std ** 2 / n_var) ** 2 / (n_var - 1))
            )
            df = max(1, min(df, n_orig + n_var - 2))
            
            # Two-tailed test
            t_crit = 2.0  # Approximate critical value for alpha=0.05, df>=4
            if abs(t_stat) > t_crit and df >= 2:
                # Statistically significant difference
                if var_mean > orig_mean:
                    self._committed_strategy = 'variant_08'
                else:
                    self._committed_strategy = 'original'
            else:
                # No significant difference: default to original (23/24 historical wins)
                self._committed_strategy = 'original'
        else:
            # Insufficient probe data: default to original
            self._committed_strategy = 'original'
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Phase A: Probe-and-commit
        population, fitness = self._run_probe_phase(func, population, fitness)
        
        # Update best after probe
        ...
        while not stopping_condition():
            ...
            # Blend: 60% compass/variant, 40% ensemble
