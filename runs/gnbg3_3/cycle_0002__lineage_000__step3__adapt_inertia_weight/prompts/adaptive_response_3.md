...
class AdaptiveTopologyParticleSwarm:
    ...
    beating the median competitor by 95.6×. A 50/50 blend of 21.35 and
    2039.9 yields ~1030, which is ~48× WORSE than the winner alone.
    Linear blending mathematically cannot preserve per-task advantages.
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Operators available for selection (7 benchmark winners)
        self._operators = [
            'original',
            'variant_01_catA',
            'variant_04_catD',
        ...
        # Sliding window for rank normalization (must be >= 3 * num_arms)
        self._rank_window_size = max(21, 3 * len(self._operators))
    ...
    # =========================================================================
    # INERTIA WEIGHT ADAPTATION METHODS (7 benchmark winners)
    # =========================================================================
    ...
    def _adapt_inertia_weight_variant_01_catA(self):
        """Variant 01: Adapt inertia weight based on axis-aligned geometric spread ratio."""
    ...
    def _adapt_inertia_weight_variant_04_catD(self):
        """Variant 04: Adapt inertia weight using fitness-rank landscape analysis."""
    ...
    def _adapt_inertia_weight_variant_06_catF(self):
        """Variant 06: Adapt inertia weight using temporal dynamics."""
    ...
    def _adapt_inertia_weight_variant_07_catG(self):
        """Variant 07: Adapt inertia weight using bootstrap confidence intervals."""
        pop = self.population
        np_pop = self.np
    ...
    def _adapt_inertia_weight_variant_08_catH(self):
        """Variant 08: Hybrid inertia adaptation: stagnation signal with spectral condition."""
        diversity = self._compute_diversity()
    ...
    def _adapt_inertia_weight_variant_09_catA(self):
        """Variant 09: Adapt inertia weight based on axis-aligned spread ratio (per-dimension)."""
        min_pos = np.min(self.population, axis=0)
        max_pos = np.max(self.population, axis=0)
    ...
    # =========================================================================
    # VELOCITY UPDATE METHODS (7 benchmark winners)
    # =========================================================================
    ...
    def _velocity_update_variant_01_catA(self):
        """Variant 01: Geometric layout - centroid, k-NN repulsion, spread."""
        centroid = np.mean(self.population, axis=0)
    ...
    def _velocity_update_variant_04_catD(self):
        """Variant 04: Fitness-rank-adaptive components."""
        if self.global_best is not None:
            distances = np.linalg.norm(self.population - self.global_best, axis=1)
    ...
    def _velocity_update_variant_06_catF(self):
        """Variant 06: Spectral-condition-guided anisotropic mutation."""
        cognitive, social = self._adaptive_coefficients()
    ...
    def _velocity_update_variant_07_catG(self):
        """Variant 07: Entropy-modulated distribution-guided exploration."""
        cognitive, social = self._adaptive_coefficients()
    ...
    def _velocity_update_variant_08_catH(self):
        """Variant 08: k-NN graph topology with betweenness-based social routing."""
        cognitive, social = self._adaptive_coefficients()
    ...
    def _velocity_update_variant_09_catA(self):
        """Variant 09: DE/rand/1 mutation with adaptive scaling."""
        cognitive, social = self._adaptive_coefficients()
    ...
    # =========================================================================
    # DISPATCHER: maps operator name -> inertia + velocity update methods
    # =========================================================================
    
    def _dispatch_inertia_update(self, operator):
        """Route to the appropriate inertia weight adaptation."""
    ...
    def _dispatch_velocity_update(self, operator):
        ...
        if operator == 'original':
            ...
            self._velocity_update_variant_07_catG()
        elif operator == 'variant_08_catH':
            self._velocity_update_variant_08_catH()
        elif operator == 'variant_09_catA':
    ...
    def _record_probe_score(self, operator, best_fitness):
        ...
        Uses rank-based scoring within a sliding window: a generation's score
        is its percentile rank among the last K probe generations (lower fitness
        = better = higher score). This is scale-independent.
        """
        self._probe_fitness_history.append(best_fitness)
        self._probe_op_history.append(operator)
        
        # Maintain sliding window
        if len(self._probe_fitness_history) > self._rank_window_size:
            self._probe_fitness_history.pop(0)
            self._probe_op_history.pop(0)
        
        # Compute rank-based score: position in sorted order (higher = better)
        if len(self._probe_fitness_history) < 2:
            score = 1.0
        else:
            sorted_fitness = sorted(self._probe_fitness_history)
            rank = sorted_fitness.index(best_fitness)
            score = 1.0 - (rank / (len(sorted_fitness) - 1))
    ...
    def __call__(self, func, stopping_condition):
        ...
        if hasattr(self, '_ema_diversity'):
            delattr(self, '_ema_diversity')
        ...
        while not stopping_condition():
            ...
            if self._in_probe_phase:
                ...
                # Adaptation phase (inertia + neighborhood)
                self._dispatch_inertia_update(current_op)
