...
class AdaptiveTopologyParticleSwarm:
    """
    Mechanism: RANK-BASED ENSEMBLE VOTING (mechanism #2) with learned exponential weights.

    The gap table is NOT blend-hostile:
      - 0 tasks with gap >= 10×; max gap is 4.8× (task 5, winner vs median).
      - 0 tasks with gap >= 100× (dispatch-mandatory).
      - All 24 tasks have gap < 2× except task 5 (4.8×) and task 12 (1.8×).

    Because linear blending can preserve per-task advantages when gaps are small,
    we run ALL 9 winning operators simultaneously each generation and weight their
    position updates by rank-based exponential moving average (EMA) scores.
    No hard-coded regime→weight tables. No scale-dependent magic constants.
    Each operator can reach weight ~1.0 if it consistently wins the rank vote.

    Win-count distribution (5/4/4/3/3/2/2/2/1 across 9 winners) confirms that
    different operators are best on different tasks. The rank-based EMA adapts
    weights online — when task 5 favors variant_04, variant_04 rises; when
    task 1 favors variant_08, variant_08 rises. No probing budget is wasted.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # --- ENSEMBLE STATE ---
        # All 9 benchmark-winning operators (in order of win count)
        self._operators = [
            'svd_subspace_collapse',   # variant_08_catH_idea_0.py — 5 wins
            'spearman_kendall',        # variant_04_catD_idea_0.py — 4 wins
            'svd_spectral_linear',     # variant_02_catB_idea_0.py — 4 wins
            'mst_bridge',              # variant_05_catE_idea_0.py — 3 wins
            'fitness_rank_graph',      # variant_05_catE_idea_0.py — 3 wins (variant_05)
            'temporal_autocorr',       # variant_06_catF_idea_0.py — 2 wins
            'svd_power_law',           # variant_10_catB_idea_0.py — 2 wins
            'info_theory_gmm',         # variant_03_catC_idea_0.py — 2 wins
            'monte_carlo_bootstrap',   # variant_07_catG_idea_0.py — 1 win
            'original',                # original.py — 1 win
        ]
        
        # Per-operator rank-based EMA scores (scale: 0 to 1, rank of fitness)
        self._op_rank_scores = {op: [] for op in self._operators}
        self._op_ema_score = {op: 0.5 for op in self._operators}
        self._op_sample_count = {op: 0 for op in self._operators}
        
        # Ensemble combination weights (learned online)
        self._op_weights = {op: 1.0 / len(self._operators) for op in self._operators}
        
        # EMA decay for weight adaptation
        self._ema_decay = 0.85  # Higher = smoother (slower adaptation)
    ...
    def _position_update_svd_spectral_linear(self):
        """SVD Subspace Collapse Recovery (Category B: Spectral/Linear-Algebraic).
        
        variant_02_catB_idea_0.py — 4 wins on tasks 2, 13, 20, 22.
        Uses singular value entropy and effective rank to detect population collapse.
        """
        try:
            centered = self.population - np.mean(self.population, axis=0)
    ...
    def _position_update_info_theory_gmm(self):
        """Information-theoretic: Entropy + KL divergence + GMM multi-modality (Category C).
        
        variant_03_catC_idea_0.py — 2 wins on tasks 16, 19.
        """
        try:
            fitness = self.current_fitness
            ...
            try:
                ...
                max_entropy_bin = np.log(n_bins)
                norm_entropy = fitness_entropy / (max_entropy_bin + 1e-10)
                ...
                U_eig, V = np.linalg.eigh(cov)
                idx = np.argsort(U_eig)[::-1]
                U_eig = U_eig[idx]
                V = V[:, idx]
                vel_proj = self.velocity @ V
                sv_scale = np.sqrt(U_eig + 1e-10)
    ...
    def _position_update_spearman_kendall(self):
        """Spearman-Kendall fitness-rank modulation with success-history (Category D).
        
        variant_04_catD_idea_0.py — 4 wins on tasks 5, 8, 9, 18.
        Uses ONLY fitness signals: rank correlation, Kendall tau, fitness percentiles.
        """
        try:
    ...
    def _position_update_mst_bridge(self):
        """MST bridge-breaking for topology-aware exploration (Category E).
        
        variant_05_catE_idea_0.py — 3 wins on tasks 14, 15, 21.
        Builds MST to identify bridge edges connecting clusters.
        """
        try:
    ...
    def _position_update_temporal_autocorr(self):
        """Pure temporal/dynamical: Autocorrelation-stagnation momentum (Category F).
        
        variant_06_catF_idea_0.py — 2 wins on tasks 4, 17.
        Tracks fitness improvement autocorrelation and EMA-based centroid momentum.
        """
        try:
            if not hasattr(self, '_fitness_history'):
                self._fitness_history = []
    ...
    def _position_update_monte_carlo_bootstrap(self):
        """Monte Carlo Bootstrap Uncertainty-Driven Exploration (Category G).
        
        variant_07_catG_idea_0.py — 1 win on task 23.
        Bootstrap resampling and Monte Carlo random projections.
        """
        try:
            n_particles = self.np
            dim = self.dim
    ...
    def _position_update_svd_subspace_collapse(self):
        """Hybrid: Improvement-gated fitness-gradient + geometric-spread (Category H).
        
        variant_08_catH_idea_0.py — 5 wins on tasks 0, 1, 6, 11, 12.
        Combines temporal fitness-gradient with geometric spread, data-driven switching.
        """
        try:
    ...
    def _position_update_svd_power_law(self):
        """Subspace power-law velocity modulation via SVD (Category B).
        
        variant_10_catB_idea_0.py — 2 wins on tasks 3, 22.
        Power-law scaling boosts velocity along collapsed directions.
        """
        try:
            centered = self.population - np.mean(self.population, axis=0)
            U_svd, singular_values, Vt = np.linalg.svd(centered, full_matrices=False)
            singular_values = np.clip(singular_values, 1e-10, None)
    ...
    def _position_update_fitness_rank_graph(self):
        """Graph-Laplacian eigenvalue-based velocity modulation (variant_05).
        
        variant_05_catE_idea_0.py — 3 wins on tasks 14, 15, 21.
        k-NN graph Laplacian spectral analysis for topology-aware exploration.
        """
        k = min(5, self.np - 1)
        ...
        try:
            ...
            d_inv_sqrt_diag = csr_matrix(
                (d_inv_sqrt, (np.arange(self.np), np.arange(self.np))),
    ...
    # -------------------------------------------------------------------------
    # ENSEMBLE: compute all position updates, rank them, blend
    # -------------------------------------------------------------------------
    
    def _get_operator_position_update(self, operator):
        """Get the position update (delta) from a specific operator.
        
        Saves current state, applies operator, returns delta, restores state.
        All operators are deterministic given same (pop, vel, fitness) state.
        """
        ...
        elif operator == 'svd_spectral_linear':
            self._position_update_svd_spectral_linear()
        elif operator == 'info_theory_gmm':
            self._position_update_info_theory_gmm()
        elif operator == 'spearman_kendall':
            self._position_update_spearman_kendall()
        elif operator == 'mst_bridge':
            self._position_update_mst_bridge()
        elif operator == 'temporal_autocorr':
            self._position_update_temporal_autocorr()
        elif operator == 'monte_carlo_bootstrap':
            self._position_update_monte_carlo_bootstrap()
        elif operator == 'svd_subspace_collapse':
            self._position_update_svd_subspace_collapse()
        elif operator == 'svd_power_law':
            self._position_update_svd_power_law()
        elif operator == 'fitness_rank_graph':
            self._position_update_fitness_rank_graph()
        else:
            self._position_update_original()
        
        delta = self.population - saved_pop
        self.population = saved_pop
        self.velocity = saved_vel
        return delta
    
    def _update_ensemble_weights(self):
        """Update operator weights using rank-based EMA scores.
        
        Each operator is scored by the rank of its proposed delta quality.
        Score = 1 - (rank / num_operators) → 1.0 for best, 0.0 for worst.
        
        This is scale-independent: we rank within the current generation's
        proposals, so magnitudes don't matter. No magic constants.
        """
        if len(self._operators) < 2:
            self._op_weights = {op: 1.0 / len(self._operators) for op in self._operators}
            return
        
        # Compute delta quality: average magnitude of position delta
        delta_qualities = {}
        for op in self._operators:
            delta = self._get_operator_position_update(op)
            # Quality = mean L2 norm of delta (higher = more exploration)
            quality = np.mean(np.linalg.norm(delta, axis=1))
            delta_qualities[op] = quality
        
        # Rank-based scoring: rank 1 = best (highest quality)
        sorted_ops = sorted(delta_qualities.keys(), key=lambda op: delta_qualities[op], reverse=True)
        ranks = {op: sorted_ops.index(op) + 1 for op in self._operators}
        
        # Convert rank to score: 1.0 for rank 1, 0.0 for rank N
        for op in self._operators:
            rank_score = 1.0 - (ranks[op] - 1) / max(1, len(self._operators) - 1)
            self._op_rank_scores[op].append(rank_score)
            self._op_sample_count[op] += 1
            
            # Maintain a sliding window of last 20 scores
            if len(self._op_rank_scores[op]) > 20:
                self._op_rank_scores[op].pop(0)
            
            # EMA update: smooth over time
            recent_avg = np.mean(self._op_rank_scores[op]) if self._op_rank_scores[op] else 0.5
            self._op_ema_score[op] = self._ema_decay * self._op_ema_score[op] + (1.0 - self._ema_decay) * recent_avg
        
        # Normalize to weights (softmax-like, but on [0,1] scores)
        ema_values = np.array([self._op_ema_score[op] for op in self._operators])
        # Shift to positive for softmax stability
        shifted = ema_values - ema_values.min() + 0.01
        exp_weights = np.exp(shifted * 5.0)  # Temperature = 5.0 (sharpens peaks)
        total = np.sum(exp_weights)
        for idx, op in enumerate(self._operators):
            self._op_weights[op] = exp_weights[idx] / total
    
    def _apply_ensemble_position_update(self):
        """Apply weighted ensemble of all operator position updates."""
        total_delta = np.zeros((self.np, self.dim))
        
        for op in self._operators:
            delta = self._get_operator_position_update(op)
            total_delta += self._op_weights[op] * delta
        
        self.population = self._clip_to_bounds(self.population + total_delta)
    ...
    def __call__(self, func, stopping_condition):
        """Run optimizer with rank-based ensemble position update selection.
        ...
        # Reset temporal operator states
        for attr in ['_ema_centroid', '_ema_diversity', '_ema_improvement',
                     '_success_count', '_fitness_entropy_history', '_fitness_history',
                     '_centroid_drift_history', '_spread_history', '_prev_fitness_spread',
                     '_stagnation_ema', '_prev_best_fitness', '_accel_history',
                     '_stagnation_counter_temporal', '_prev_global_best',
                     '_centroid_vel_ema_mag', '_lhc_samples', '_lhc_generation',
                     '_prev_centroid']:
        ...
        while not stopping_condition():
            ...
            # Base velocity update (shared across all operators)
            self._velocity_update_base()
            
            # Update ensemble weights based on current state
            # (after velocity update, before position update)
            self._update_ensemble_weights()
            
            # Apply ensemble position update (weighted blend of all operators)
            self._apply_ensemble_position_update()
