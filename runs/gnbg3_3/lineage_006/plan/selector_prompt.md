...
CANDIDATE methods with metadata (you MUST pick exactly 3 distinct names):
  ...
  _position_update_temporal_drift              hot_path     130  MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]
...
(no prior cycles in this lineage)
  ...
  Methods excluded (banned: weighted cost >= 2.0 OR rejected streak >= 3): _position_update_centroid_knn, _position_update_fitness_rank
  Global pick costs so far (method: weighted_cost — where strong-improvement picks cost 0, weak-improvement picks cost 0.5, rejected picks cost 1.0):
    _position_update_fitness_rank: 2.50
    _position_update_centroid_knn: 2.00
