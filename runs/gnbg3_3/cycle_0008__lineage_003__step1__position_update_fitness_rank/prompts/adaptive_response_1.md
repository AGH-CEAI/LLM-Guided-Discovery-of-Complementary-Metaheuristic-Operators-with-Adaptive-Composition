Looking at the benchmark results and gap table:

**Key observations:**
- NO task has gap ≥ 10× (max gap is 1.2× on task 5)
- 5 distinct winners with win counts: original=7, variant_04=6, variant_10=5, variant_08=4, variant_02=2
- All gaps are small (1.0-1.2×), meaning operators are competitive but task-dependent
- Win distribution is roughly even across 5 operators

**Mechanism choice: Thompson Sampling Bandit (mechanism #1)**
- Small, consistent gaps (no blend-hostile tasks) make continuous selection viable
- Thompson Sampling naturally balances explore/exploit without probe overhead
- Rank-normalized sliding-window rewards ensure scale independence
- The original code's probe-and-commit wastes budget on near-equivalent operators; continuous selection is more efficient when gaps are small

**Critical fix:** The original code uses incorrect operators (variant_06, variant_09) that don't appear in the benchmark. Replacing with the 5 actual winners.
