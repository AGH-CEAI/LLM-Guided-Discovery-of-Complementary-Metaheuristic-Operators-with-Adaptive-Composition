Below is the lineage champion metaheuristic. You will plan this lineage by
choosing EXACTLY 3 methods from the CANDIDATE list. Those 3 methods will
each be adapted in a separate cycle, so your ranking determines impact.

Focus on maximizing improvement in final convergence accuracy (target <= 1e-8).

CRITICAL — USE THE ROLE METADATA:
The candidate list below includes a 'role' tag and signals for every method.
Roles indicate where in the algorithm the method sits:

  - hot_path     : modifies the search distribution OR evaluates fitness.
                   These methods directly drive search — HIGHEST priority.
  - leaf         : called from the main loop but does not directly modify
                   the search distribution. Medium priority.
  - control_flow : orchestrates other methods (typically __call__-like).
                   Low priority — usually too small or too generic.
  - observer     : returns a value, does NOT modify state, does NOT call
                   func. These are MEASUREMENTS (e.g. _compute_diversity).
                   LOWEST priority — almost always wrong to pick. Adapting
                   how the algorithm *measures itself* rarely changes how
                   it *searches*. Pick an observer ONLY if you can
                   explicitly justify a feedback loop where its output
                   drives a hot-path decision.

CANDIDATE methods with metadata (you MUST pick exactly 3 distinct names):
  name                                         role          loc  signals
  -------------------------------------------- ------------ ---- -----------------------------------
  _clip_to_bounds                              observer     3    in_main_loop  PURE_observer  called_by=[_evaluate_batch, _position_update_original, _position_update_svd_whitening, +4 more]
  _update_personal_best_batch                  leaf         5    in_main_loop  called_by=[__call__]
  _compute_diversity                           observer     5    in_main_loop  PURE_observer  called_by=[_compute_fingerprint]
  _update_local_best_from_personal             leaf         5    in_main_loop  called_by=[__call__]
  _adaptive_coefficients                       observer     6    in_main_loop  PURE_observer  called_by=[_velocity_update_base]
  _select_runner_up_operator                   observer     7    in_main_loop  PURE_observer  called_by=[__call__]
  _evaluate_batch                              observer     9    in_main_loop  PURE_observer  called_by=[__call__]
  _compute_global_best                         leaf         10   in_main_loop  called_by=[__call__]
  _select_best_operator                        observer     12   in_main_loop  PURE_observer  called_by=[__call__]
  _dispatch_position_update                    leaf         14   in_main_loop  called_by=[__call__]
  _initialize_population                       hot_path     17   MODIFIES_distribution  called_by=[__call__]
  _compute_ucb_score                           observer     17   in_main_loop  PURE_observer  called_by=[_select_best_operator, _select_runner_up_operator]
  _compute_local_best_batch                    control_flow 19   in_main_loop  called_by=[__call__]
  _position_update_original                    hot_path     22   MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]
  _record_probe_score                          leaf         24   in_main_loop  called_by=[__call__]
  _restart_if_stagnant                         leaf         25   in_main_loop  called_by=[__call__]
  _adapt_inertia_weight                        leaf         31   in_main_loop  called_by=[__call__]
  _position_update_centroid_knn                hot_path     32   MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]
  _position_update_svd_whitening               hot_path     34   MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]
  _velocity_update_base                        control_flow 37   in_main_loop  called_by=[__call__]
  _adapt_neighborhood_size                     leaf         57   in_main_loop  called_by=[__call__]
  _compute_fingerprint                         leaf         62   in_main_loop  called_by=[__call__]
  _position_update_fitness_rank                hot_path     105  MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]
  _position_update_temporal_drift              hot_path     195  MODIFIES_distribution  in_main_loop  called_by=[_dispatch_position_update]

Signals legend:
  CALLS_func()          — invokes self.func(...) (fitness evaluation)
  MODIFIES_distribution — assigns to self.population/fitness/mean/C/sigma/...
  in_main_loop          — reachable from a loop in __call__
  PURE_observer         — returns a value, doesn't modify any self.X
  called_by=[...]       — methods that call this one

HISTORY of cycles in this lineage so far (most recent last):
(no prior cycles in this lineage)

  Global pick costs so far (method: weighted_cost — where strong-improvement picks cost 0, weak-improvement picks cost 0.5, rejected picks cost 1.0):
    _position_update_fitness_rank: 1.50
    _adapt_inertia_weight: 1.00
    _position_update_centroid_knn: 1.00
    _position_update_svd_whitening: 1.00
    _position_update_temporal_drift: 1.00
    _velocity_update_base: 1.00
    _velocity_update_batch: 0.50
    _adapt_neighborhood_size: 0.00
    _position_update_batch: 0.00
...
Reply with JSON only, listing picks in DESCENDING order of expected impact.
For each method you score, the role_match field measures whether the role
makes the method a sensible target — observers should score 0–1 unless you
have a strong feedback-loop justification.

{"reasoning": "Step-by-step comparison of candidate methods using the role metadata to filter out observers and prioritise hot-path methods.", "analysis": {"method_scores": {"method_name": {"role_match": "0-5 \u2014 does the role match what we want to adapt?", "influence": "0-5", "replaceability": "0-5", "precision_impact": "0-5"}}, "top_candidates": ["method_a", "method_b", "..."]}, "reason": "...", "picks": ["...", "...", "..."]}
Field "picks" is the most important here.
