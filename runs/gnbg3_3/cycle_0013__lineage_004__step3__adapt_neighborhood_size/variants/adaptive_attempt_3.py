...
class AdaptiveTopologyParticleSwarm:
    ...
    The gap table is BLEND-HOSTILE:
      - Task 5: variant_10_catB wins with 4.4× gap to runner-up, 176.7× gap 
        to median. A 50/50 blend of 5.96 and 26.3 yields ~16.1, which is 
        ~2.7× WORSE than the winner alone. Linear blending mathematically 
        cannot preserve this advantage.
      - 1 task with gap >=10× → dispatch-style mechanism is MANDATORY.
    
    The dispatcher preserves each winner's advantage by:
      - PHASE A (probe): round-robin burst with each of the 8 winning
        _adapt_neighborhood_size strategies, tracking rank-based improvement 
        on the ACTUAL landscape. Each strategy gets 3 generations in a 
        sub-population.
    ...
    Win-count distribution (5/4/4/3/3/2/2/1 across 8 winners) confirms that
    different neighborhood adaptation strategies win on different tasks.
    The probe extracts a cheap FINGERPRINT from the landscape to route.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # THE 8 ACTUAL WINNING _adapt_neighborhood_size STRATEGIES
        self._strategies = [
            'spectral_cond',      # variant_02_catB_idea_0.py (4 wins): condition number
            'info_theory',        # variant_03_catC_idea_0.py (5 wins): entropy + KL
            'fitness_landscape',  # variant_04_catD_idea_0.py (3 wins): rank signals
            'knn_graph',          # variant_05_catE_idea_0.py (2 wins): k-NN connectivity
            'stochastic_mc',      # variant_07_catG_idea_0.py (1 win): Monte Carlo
            'temporal_topology',  # variant_08_catH_idea_0.py (4 wins): hybrid
            'knn_density',        # variant_09_catA_idea_0.py (3 wins): local density
            'spectral_entropy',   # variant_10_catB_idea_0.py (2 wins): spectral entropy
        ]
        for st in self._strategies:
            self._strategy_scores[st] = []
            self._strategy_total_score[st] = 0.0
            self._strategy_sample_count[st] = 0
        
        # Probe budget: 3 gens per strategy, min 24 total
        self._probe_gens_per_strat = 3
        self._probe_total_budget = max(len(self._strategies) * self._probe_gens_per_strat, 24)
        
        # Epsilon-review probability when committed strategy stagnates
        self._epsilon_review = 0.05
        
        # Probe phase state
        self._in_probe_phase = False
        self._probe_strat_index = 0
        self._probe_gens_in_current_strat = 0
        self._probe_best_fitness_seen = np.inf
        
        # Running history for rank-based scoring
        self._probe_fitness_history = []
        self._probe_strat_history = []
    ...
    # -------------------------------------------------------------------------
    # THE 8 WINNING _adapt_neighborhood_size STRATEGIES (from benchmark)
    # -------------------------------------------------------------------------
    
    def _adapt_neighborhood_spectral_cond(self):
        """variant_02_catB_idea_0 (4 wins): Spectral condition number based.
    ...
    def _adapt_neighborhood_info_theory(self):
        """variant_03_catC_idea_0 (5 wins): Information-theoretic signals.
        
        Uses: fitness entropy + KL divergence of covariance eigenvalues from uniform.
    ...
    def _adapt_neighborhood_fitness_landscape(self):
        """variant_04_catD_idea_0 (3 wins): Fitness landscape signals only.
        
        Uses: fitness rank entropy, rank correlation across generations,
    ...
    def _adapt_neighborhood_knn_graph(self):
        """variant_05_catE_idea_0 (2 wins): k-NN graph connectivity.
        
        Build k-NN graph; use connected component count and spectral gap
        to detect fragmentation vs convergence.
        """
        n = self.np
        k = min(5, n - 1)
        ...
        try:
            ...
            lap = csr_matrix((np.eye(n))) - d_inv_sqrt_diag @ adj @ d_inv_sqrt_diag
    ...
    def _adapt_neighborhood_stochastic_mc(self):
        """variant_07_catG_idea_0 (1 win): Stochastic Monte Carlo sampling.
        
        Uses bootstrap sampling to estimate expected improvement for each
        candidate neighborhood size, then softmax-samples.
        """
    ...
    def _adapt_neighborhood_temporal_topology(self):
        """variant_08_catH_idea_0 (4 wins): Hybrid temporal + topology.
        
        Combines: temporal convergence signals (improvement EMA, stagnation)
        with topology fragmentation detection (k-NN connected components).
        """
    ...
    def _adapt_neighborhood_knn_density(self):
        """variant_09_catA_idea_0 (3 wins): k-NN local density structure.
        
        Uses k-NN nearest-neighbor distances and axis-aligned spread ratio
        to detect clustering vs dispersion. Focuses on LOCAL neighborhood
    ...
    def _adapt_neighborhood_spectral_entropy(self):
        """variant_10_catB_idea_0 (2 wins): Spectral properties of population.
        
        Uses condition number and spectral entropy of population covariance.
        High condition number OR low spectral entropy → larger neighborhood.
        Isotropic distribution → smaller neighborhood.
        """
        try:
            centered = self.population - np.mean(self.population, axis=0)
            cov = np.cov(centered.T)
    ...
    # -------------------------------------------------------------------------
    # DISPATCHER: maps strategy name -> _adapt_neighborhood_size method
    # -------------------------------------------------------------------------
    
    def _dispatch_neighborhood_adaptation(self, strategy):
        """Route to the appropriate _adapt_neighborhood_size implementation."""
        if strategy == 'spectral_cond':
            self._adapt_neighborhood_spectral_cond()
        elif strategy == 'info_theory':
            self._adapt_neighborhood_info_theory()
        elif strategy == 'fitness_landscape':
            self._adapt_neighborhood_fitness_landscape()
        elif strategy == 'knn_graph':
            self._adapt_neighborhood_knn_graph()
        elif strategy == 'stochastic_mc':
            self._adapt_neighborhood_stochastic_mc()
        elif strategy == 'temporal_topology':
            self._adapt_neighborhood_temporal_topology()
        elif strategy == 'knn_density':
            self._adapt_neighborhood_knn_density()
        elif strategy == 'spectral_entropy':
            self._adapt_neighborhood_spectral_entropy()
        else:
            # Fallback: simple diversity-based
            diversity = self._compute_diversity()
            if diversity < self.diversity_threshold_low:
                self.neighborhood_size = min(self.np // 2, self.neighborhood_size + 1)
            elif diversity > self.diversity_threshold_high:
    ...
    def _record_probe_score(self, strategy, best_fitness):
        ...
        self._probe_strat_history.append(strategy)
    ...
    def _select_best_strategy(self):
        """Select strategy with highest UCB score from probe data."""
        best_st = self._strategies[0]
        best_score = -float('inf')
        
        for st in self._strategies:
            ucb = self._compute_ucb_score(st)
            if ucb > best_score:
                best_score = ucb
                best_st = st
    ...
    def _select_runner_up_strategy(self):
        """Select the second-best strategy by UCB score."""
        scores = [(st, self._compute_ucb_score(st)) for st in self._strategies]
    ...
    # -------------------------------------------------------------------------
    # OTHER ADAPTATION METHODS (shared, not part of probe)
    # -------------------------------------------------------------------------
    ...
    def __call__(self, func, stopping_condition):
        """
        Run the optimizer with PROBE-AND-COMMIT neighborhood adaptation.
        ...
        for attr in ['_prev_fitness_ranks', '_neighborhood_history', '_neighborhood_success',
                     ...
                     '_prev_best_for_history', '_success_count', '_prev_centroid',
                     '_centroid_drift_history', '_ema_drift', '_spectral_ema_improvement']:
        ...
        while not stopping_condition():
            ...
            if self._in_probe_phase:
                current_strat = self._strategies[self._probe_strat_index]
                
                # Adaptation
                self._adapt_inertia_weight()
                self._dispatch_neighborhood_adaptation(current_strat)
                self._update_local_best_from_personal()
                
                # Velocity update (shared), then position update
                ...
                self._record_probe_score(current_strat, self._probe_best_fitness_seen)
                
                self._probe_gens_in_current_strat += 1
                
                # Advance to next strategy after probe_gens_per_strat
                if self._probe_gens_in_current_strat >= self._probe_gens_per_strat:
                    self._probe_strat_index += 1
                    self._probe_gens_in_current_strat = 0
                
                # Check if probe phase is done
                if self._probe_strat_index >= len(self._strategies):
                    self._in_probe_phase = False
                    self._committed_strategy = self._select_best_strategy()
                    self._runner_up_strategy = self._select_runner_up_strategy()
            ...
            # --- PHASE B: COMMIT ---
                ...
                if (self.stagnation_counter > 20 and
                    ...
                    self._adapt_inertia_weight()
                    self._dispatch_neighborhood_adaptation(self._runner_up_strategy)
                    self._update_local_best_from_personal()
                    self._velocity_update_base()
                    self.population = self._clip_to_bounds(self.population + self.velocity)
                else:
                    self._adapt_inertia_weight()
                    self._dispatch_neighborhood_adaptation(self._committed_strategy)
                    self._update_local_best_from_personal()
                    self._velocity_update_base()
                    self.population = self._clip_to_bounds(self.population + self.velocity)
