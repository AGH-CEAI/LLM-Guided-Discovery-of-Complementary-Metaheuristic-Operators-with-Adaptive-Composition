...
class AdaptiveTopologyParticleSwarm:
    ...
    The gap table is BLEND-HOSTILE (1 task with gap >=50× to median):
      - Task 5: variant_10_catB wins with 5.96 vs original 26.3 = 4.4× runner-up
        gap but 176.7× GAP TO MEDIAN competitor. A 50/50 blend of 6 and 48742
        yields ~24374, which is ~4100× WORSE than the winner alone. Linear
        blending cannot preserve this advantage.
      - 8 distinct winners across 24 tasks; no single operator dominates.
      - Most gaps are small (1.0×-2.3×) but Task 5's extreme median gap means
        ensemble blending is mathematically excluded.
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # THE 8 ACTUAL BENCHMARK WINNING _adapt_neighborhood_size STRATEGIES
        self._operators = [
            'variant_02_catB',  # 4 wins: spectral condition number → expand/contract
            'variant_03_catC',  # 5 wins: info-theoretic (entropy + KL divergence)
            'variant_04_catD',  # 3 wins: fitness-landscape signals only
            'variant_05_catE',  # 2 wins: k-NN graph connectivity
            'variant_07_catG',  # 1 win:  stochastic sampling + UCB
            'variant_08_catH',  # 4 wins: temporal convergence + topology fragmentation
            'variant_09_catA',  # 3 wins: k-NN local density structure
            'variant_10_catB',  # 2 wins: spectral entropy + condition number
        ]
        ...
        # Probe budget: 3 gens per operator, min 24 total (8 ops × 3)
    ...
    # -------------------------------------------------------------------------
    # THE 8 BENCHMARK-WINNING _adapt_neighborhood_size STRATEGIES
    # -------------------------------------------------------------------------
    
    def _adapt_neighborhood_size_variant_02_catB(self):
        """variant_02_catB_idea_0 (4 wins): spectral condition number."""
        try:
            centered = self.population - np.mean(self.population, axis=0)
            cov = np.cov(centered.T)
            eigenvalues = np.linalg.eigvalsh(cov)
    ...
    def _adapt_neighborhood_size_variant_03_catC(self):
        """variant_03_catC_idea_0 (5 wins): info-theoretic signals."""
    ...
    def _adapt_neighborhood_size_variant_04_catD(self):
    ...
    def _adapt_neighborhood_size_variant_05_catE(self):
    ...
    def _adapt_neighborhood_size_variant_07_catG(self):
        """variant_07_catG_idea_0 (1 win): stochastic sampling + UCB."""
        ...
        min_stat = min(candidate_stats.values()) if candidate_stats else 0.0
        max_stat = max(candidate_stats.values()) if candidate_stats else 1.0
    ...
    def _adapt_neighborhood_size_variant_08_catH(self):
        """variant_08_catH_idea_0 (4 wins): temporal + topology hybrid."""
    ...
    def _adapt_neighborhood_size_variant_09_catA(self):
    ...
    def _adapt_neighborhood_size_variant_10_catB(self):
        """variant_10_catB_idea_0 (2 wins): spectral entropy + condition number."""
        try:
            centered = self.population - np.mean(self.population, axis=0)
            cov = np.cov(centered.T)
            eigenvalues = np.linalg.eigvalsh(cov)
    ...
    # -------------------------------------------------------------------------
    # DISPATCHER: maps operator name -> _adapt_neighborhood_size method
    # -------------------------------------------------------------------------
    
    def _dispatch_neighborhood_adaptation(self, operator):
        """Route to the appropriate _adapt_neighborhood_size implementation."""
        if operator == 'variant_02_catB':
            self._adapt_neighborhood_size_variant_02_catB()
        elif operator == 'variant_03_catC':
            self._adapt_neighborhood_size_variant_03_catC()
        elif operator == 'variant_04_catD':
            self._adapt_neighborhood_size_variant_04_catD()
        elif operator == 'variant_05_catE':
            self._adapt_neighborhood_size_variant_05_catE()
        elif operator == 'variant_07_catG':
            self._adapt_neighborhood_size_variant_07_catG()
        elif operator == 'variant_08_catH':
            self._adapt_neighborhood_size_variant_08_catH()
        elif operator == 'variant_09_catA':
            self._adapt_neighborhood_size_variant_09_catA()
        elif operator == 'variant_10_catB':
            self._adapt_neighborhood_size_variant_10_catB()
        else:
            # Fallback: diversity-based
    ...
    def __call__(self, func, stopping_condition):
        """
        Run the optimizer with PROBE-AND-COMMIT _adapt_neighborhood_size selection.
        ...
        # Reset history-dependent states
        ...
        while not stopping_condition():
            ...
            if self._in_probe_phase:
                ...
                # Adaptation using the PROBED operator's strategy
                self._adapt_inertia_weight()
                self._dispatch_neighborhood_adaptation(current_op)
                self._update_local_best_from_personal()
                
                # Velocity update (shared base)
                self._velocity_update_base()
                
                # Position update (standard PSO)
            ...
            # --- PHASE B: COMMIT ---
                ...
                # Adaptation using the COMMITTED operator's strategy
                self._adapt_inertia_weight()
                self._dispatch_neighborhood_adaptation(self._committed_operator)
                self._update_local_best_from_personal()
                ...
                if (self.stagnation_counter > 20 and
                    ...
                    self._dispatch_neighborhood_adaptation(self._runner_up_operator)
