...
class AdaptiveTopologyParticleSwarm:
    ...
    The gap table is BLEND-HOSTILE: Task 5 shows a 768× gap (original.py 6.35e+01
    vs variant_02 4.87e+04). A 50/50 blend of these yields ~2.44e+04, which is
    384× WORSE than the winner alone. Linear blending cannot preserve per-task
    advantages when gaps are this extreme.
    
    The dispatcher preserves each winner's advantage by:
      - PHASE A (probe): round-robin burst with each of the 5 benchmark-winning
        operators, tracking rank-based improvement scores on the ACTUAL landscape.
      - PHASE B (commit): run ONLY the winning operator for the remaining budget.
        No blending. No hard-coded weights. The committed operator gets weight 1.0.
      - EPSILON-REVIEW: if the committed operator stagnates, allow a small
        probability of switching to the runner-up.
    
    Win-count distribution (8/6/4/3/3 across 5 winners) confirms that no single
    operator dominates everywhere, making per-task probing essential.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Operators matching the 5 benchmark winners
        self._operators = [
            'original',
            'variant_02_catB',   # SVD-based spectral modulation (3 wins)
            'variant_04_catD',   # Fitness-rank signals (8 wins)
            'variant_06_catF',   # Temporal-drift modulation (3 wins)
            'variant_09_catA',   # k-NN density + centroid gravity (6 wins)
        ]
        ...
        # Probe config: 4 gens per operator to get reliable signal
        ...
        # Per-operator success history for adaptive epsilon
        self._op_success_window = 10
        self._op_recent_improvements = {op: [] for op in self._operators}
    ...
    # -------------------------------------------------------------------------
    # VELOCITY UPDATE VARIANTS (from the 5 benchmark winners)
    # -------------------------------------------------------------------------
    ...
    def _velocity_update_variant_02_catB(self):
        """SVD-based population whitening with anisotropic damping (3 wins)."""
        cognitive, social = self._adaptive_coefficients()
        ...
        try:
            ...
            mutation_component = 0.3 * (vel_scaled @ Vt)
            
        except np.linalg.LinAlgError:
            mutation_component = 0.3 * self.velocity
    ...
    def _velocity_update_variant_04_catD(self):
        """Fitness-landscape rank signals only (8 wins)."""
        cognitive, social = self._adaptive_coefficients()
    ...
    def _velocity_update_variant_06_catF(self):
        """Temporal-drift-modulated velocity scaling (3 wins)."""
        cognitive, social = self._adaptive_coefficients()
    ...
    def _velocity_update_variant_09_catA(self):
        """Centroid gravity and k-NN density modulation (6 wins)."""
        cognitive, social = self._adaptive_coefficients()
    ...
    # -------------------------------------------------------------------------
    # PROBE-AND-COMMIT: rank-based scoring with UCB
    # -------------------------------------------------------------------------
    
    def _record_probe_score(self, operator, best_fitness):
        """Record a probe score using rank-based evaluation (scale-independent)."""
        ...
        # Track recent improvements for adaptive epsilon
        self._op_recent_improvements[operator].append(score)
        if len(self._op_recent_improvements[operator]) > self._op_success_window:
            self._op_recent_improvements[operator].pop(0)
    ...
    # -------------------------------------------------------------------------
    # POSITION UPDATE: eigenvalue-anisotropic modulation (consistent across all ops)
    # -------------------------------------------------------------------------
    ...
    def __call__(self, func, stopping_condition):
        ...
        for op in self._operators:
            ...
            self._op_recent_improvements[op] = []
        self._probe_fitness_history = []
        self._probe_op_history = []
        self._probe_best_fitness_seen = np.inf
        self._committed_operator = None
        ...
        if hasattr(self, '_centroid_vel_history'):
            del self._centroid_vel_history
        if hasattr(self, '_ema_improvement'):
            del self._ema_improvement
        if hasattr(self, '_prev_global_best_fitness'):
            del self._prev_global_best_fitness
        ...
        while not stopping_condition():
            ...
            # Adaptation phase (runs during both probe and commit)
            ...
            # --- PHASE B: COMMIT ---
                ...
                # Epsilon-review with adaptive probability based on recent performance
                recent_scores = self._op_recent_improvements.get(self._committed_operator, [])
                avg_recent = np.mean(recent_scores) if recent_scores else 0.5
                adaptive_epsilon = self._epsilon_review * (1.0 - 0.5 * avg_recent)
                
                if (self.stagnation_counter > 20 and
                    np.random.random() < adaptive_epsilon):
