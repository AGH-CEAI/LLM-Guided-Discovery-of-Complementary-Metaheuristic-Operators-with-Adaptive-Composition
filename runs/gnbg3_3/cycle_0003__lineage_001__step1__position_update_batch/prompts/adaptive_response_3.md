...
1. **5 distinct winners** across 24 tasks with **blend-hostile gaps** (Task 5 has 768× gap)
2. **Dispatch-mandatory**: Task 5 requires a single committed operator, not blending
3. The current code's operator list is WRONG — it lists `variant_01_catA`, `variant_03_catC`, `variant_05_catE`, `variant_10_catB` which are NOT the actual winners

I need to implement **PROBE-AND-COMMIT** with the correct 5 winning operators from the benchmark: `original`, `variant_02_catB`, `variant_04_catD`, `variant_06_catF`, `variant_09_catA`.
