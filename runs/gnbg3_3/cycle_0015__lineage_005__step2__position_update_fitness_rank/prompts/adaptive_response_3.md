Looking at the benchmark data, I need to reconsider the mechanism:

1. **Gap table analysis**: ALL gaps are < 2× (max is 1.5× on task 5). No task has gap ≥ 10×. The original docstring's claim of "768× gap" is incorrect — the actual gap is 1.16×.
2. **Win distribution**: 7 variants with 4/4/4/3/3/3/3 wins — roughly even, no single dominant winner.
3. **Decision**: Mechanism #1 (Thompson Sampling bandit) is appropriate: scale-invariant rank rewards, sliding window for non-stationarity, UCB for exploration bonus.

Key changes vs original:
- Use Thompson Sampling instead of probe-and-commit (probe wastes budget when gaps are small)
- Rank-based rewards within sliding window (K=35, ≥ 3× num_arms)
- UCB1 exploration bonus on the same [0,1] scale
- Correct operator implementations (all 7 benchmark winners)
- Remove per-generation probe overhead
