...
class AdaptiveTopologyParticleSwarm:
    ...
    Gap-table diagnosis: ALL gaps are < 2× (max is 1.7× on task 12). No task
    triggers blend-hostile or dispatch-mandatory thresholds. Wins are spread
    evenly across 7 variants (4/4/4/3/3/3/3). This makes a bandit the ideal
    choice — it learns online which operator suits the landscape, without
    the wasted budget of probe-and-commit, and without the mathematical
    limitations of fixed blending.
    
    Key design choices:
    - Thompson Sampling with Beta(α,β) posterior per operator — naturally
      handles uncertainty and avoids overcommitting to early winners.
    - Rank-normalized rewards within a sliding window of size K=21 (≥3× the
      7 arms so each arm is sampled enough). No scale-dependent constants.
    - Epsilon-switch (p=0.03) allows escape if the committed operator stagnates
      for >15 generations.
    - All 7 benchmark winners are included; no dead arms.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # 7 benchmark-winning operators
        self._operators = [
            'original',      # original.py (4 wins)
            'catB_eigen',    # variant_02_catB_idea_0.py (3 wins)
            'catD_fitness',  # variant_04_catD_idea_0.py (4 wins)
            'catG_mc',       # variant_07_catG_idea_0.py (4 wins)
            'catH_graph',    # variant_08_catH_idea_0.py (3 wins)
            'catA_geo',      # variant_09_catA_idea_0.py (3 wins)
            'catB_effrank',  # variant_10_catB_idea_0.py (3 wins)
        ]
        
        # Beta posterior parameters (successes, failures)
        ...
        self._reward_window_size = 21  # ≥ 3× num_arms
        self._reward_history = []  # list of (operator, rank_score) tuples
        
        # Current selected operator
        self._current_operator = None
        
        # Epsilon-switch settings
        self._epsilon_switch = 0.03
        self._stagnation_threshold = 15
        
        # Per-operator stagnation tracking
        self._op_last_selected_gen = {op: -100 for op in self._operators}
        self._op_stagnation_since = {op: 0 for op in self._operators}
    ...
    def _position_update_catB_eigen(self):
        """variant_02_catB_idea_0.py: Eigenvalue-anisotropic velocity scaling with condition-number modulation."""
        try:
            centered = self.population - np.mean(self.population, axis=0)
            cov = np.cov(centered.T)
            eigenvalues, eigenvectors = np.linalg.eigh(cov)
    ...
    def _position_update_catD_fitness(self):
        """variant_04_catD_idea_0.py: Fitness-percentile & Kendall-tau rank stability modulation."""
        try:
            fitness_ranks = np.argsort(np.argsort(self.current_fitness)) / max(1, self.np - 1)
    ...
    def _position_update_catG_mc(self):
        """variant_07_catG_idea_0.py: Monte Carlo convergence-probability velocity modulation."""
        try:
            n_mc_samples = 50
    ...
    def _position_update_catH_graph(self):
        """variant_08_catH_idea_0.py: Hybrid graph spectral gap + temporal improvement rate."""
        k = min(5, self.np - 1)
    ...
    def _position_update_catA_geo(self):
        """variant_09_catA_idea_0.py: Axis-aligned bounding box + convex hull + pairwise distance modulation."""
        pop = self.population
    ...
    def _position_update_catB_effrank(self):
        """variant_10_catB_idea_0.py: Covariance-eigenvalue effective-rank velocity modulation."""
        centered = self.population - np.mean(self.population, axis=0)
    ...
    def _dispatch_position_update(self, operator):
        ...
        elif operator == 'catD_fitness':
            self._position_update_catD_fitness()
        elif operator == 'catG_mc':
            self._position_update_catG_mc()
        elif operator == 'catH_graph':
            self._position_update_catH_graph()
        elif operator == 'catA_geo':
            self._position_update_catA_geo()
        elif operator == 'catB_effrank':
            self._position_update_catB_effrank()
        else:
            self._position_update_original()
    ...
    def _thompson_sample_operator(self):
        """Select operator using Thompson Sampling from Beta posterior."""
        samples = {}
    ...
    def _update_bandit(self, operator, reward):
        ...
        Reward is already rank-normalized in [0, 1] from _compute_rank_reward.
        """
        # Clip reward to valid range
        reward = np.clip(reward, 0.0, 1.0)
        
        # Update Beta posterior: reward -> success, (1-reward) -> failure
        self._alpha[operator] += reward
        self._beta[operator] += (1.0 - reward)
        
        # Keep priors bounded to prevent numerical issues
        self._alpha[operator] = np.clip(self._alpha[operator], 1e-6, 1e6)
        self._beta[operator] = np.clip(self._beta[operator], 1e-6, 1e6)
    
    def _compute_rank_reward(self, gen_best_fitness):
        """Compute rank-normalized reward within the sliding window.
        
        Rank-based scoring: within the last K=21 generations, what fraction
        of generations had WORSE (higher) fitness than the current one?
        This is scale-independent — no magic constants.
        """
        self._reward_history.append(gen_best_fitness)
        ...
        if len(self._reward_history) < 2:
            return 1.0  # First generation: neutral reward
        
        # Rank-normalized: what fraction of window has worse fitness?
        current_best = min(self._reward_history)
        current_worst = max(self._reward_history)
        
        if current_worst > current_best:
            # Higher reward = better (lower fitness)
            normalized = (current_worst - gen_best_fitness) / (current_worst - current_best + 1e-10)
            return np.clip(normalized, 0.0, 1.0)
        else:
            return 1.0
    
    def _epsilon_switch_decision(self):
        """Decide whether to epsilon-switch to a different operator."""
        if self.stagnation_counter > self._stagnation_threshold:
            if np.random.random() < self._epsilon_switch:
                # Get second-best operator by posterior mean
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Track initial best for reward computation
        self._gen_best_fitness = float(np.min(self.current_fitness))
        ...
        while not stopping_condition():
            ...
            self._current_operator = self._thompson_sample_operator()
            self._op_last_selected_gen[self._current_operator] = self.generation
            
            # Check for epsilon switch
            switch_op = self._epsilon_switch_decision()
            if switch_op is not None:
                self._current_operator = switch_op
            ...
            if gen_best < self._gen_best_fitness:
                self._gen_best_fitness = gen_best
            
            reward = self._compute_rank_reward(self._gen_best_fitness)
            self._update_bandit(self._current_operator, reward)
