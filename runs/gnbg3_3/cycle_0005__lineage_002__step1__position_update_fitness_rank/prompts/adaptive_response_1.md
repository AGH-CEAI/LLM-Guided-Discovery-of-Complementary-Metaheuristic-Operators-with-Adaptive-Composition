...
- 5 distinct winners with wins distributed as 7/7/6/3/1
- **ALL per-task gaps are < 3×** (max gap = 2.8× on task 5)
- No blend-hostile tasks (0 tasks with gap ≥ 10×)
- This rules out mandatory dispatch — Thompson Sampling (mechanism #1) is appropriate since the operators are mutually exclusive, credit-assignable, and the small gaps mean exploration is worthwhile.

The original code's operators don't match the actual benchmark winners. I'll implement the 5 true winning strategies with Thompson Sampling using rank-based UCB rewards.
