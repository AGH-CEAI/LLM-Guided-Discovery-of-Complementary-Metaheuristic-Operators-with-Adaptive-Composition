...
- **Maximum gap: 2.3×** (task 5) — NOT blend-hostile
- **Wins spread evenly** across many variants
- **All gaps < 5×** — linear blending is mathematically viable, but with 10 arms and small gaps, a bandit with proper rank-based credit assignment is more sample-efficient than blending

The correct mechanism is **Thompson Sampling (mechanism #1)** with:
- Rank-based Beta posterior updates (scale-independent)
- Probe phase to warm-start the bandit
- Contextual features (fingerprint) to condition arm distributions
