...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.num_strategies = 7  # original + 6 variants
        self.strategy_alpha = np.ones(self.num_strategies, dtype=float)  # Thompson sampling alpha
        self.strategy_beta = np.ones(self.num_strategies, dtype=float)   # Thompson sampling beta
        self.current_strategy = 0
        self.prev_best_f_for_reward = np.inf
        # Sliding window for credit assignment
        self.window_size = 20
        self.reward_history = []  # list of (strategy_idx, reward)
    ...
    def _select_strategy(self):
        """Thompson Sampling to select a strategy."""
        samples = np.array([
    ...
    def _update_strategy_reward(self, strategy_idx, reward):
        ...
        self.strategy_alpha = 1.0 + (self.strategy_alpha - 1.0) * self.decay
        self.strategy_beta = 1.0 + (self.strategy_beta - 1.0) * self.decay
        
        # Clip reward to [0, 1]
        r = np.clip(reward, 0.0, 1.0)
        self.strategy_alpha[strategy_idx] += r
        self.strategy_beta[strategy_idx] += (1.0 - r)
    ...
    # Strategy 1: Cholesky (variant_01)
    def _sample_cholesky(self):
        z = np.random.randn(self.pop_size, self.dim)
    ...
    def _sample_hybrid_multi(self):
        ...
        if n_cmaes < 1:
            n_cmaes = 1
            n_explore = max(1, pop_size - n_cmaes - n_boundary)
            if n_explore + n_cmaes + n_boundary > pop_size:
                n_boundary = pop_size - n_cmaes - n_explore
    ...
    def _sample_population_batch(self):
        """Adaptive strategy selection for sampling."""
        ...
        try:
            if strategy == 0:
                ...
                result = self._sample_cholesky()
            elif strategy == 2:
                result = self._sample_diversity_injection()
            elif strategy == 3:
                result = self._sample_mirrored()
            elif strategy == 4:
                result = self._sample_bimodal()
            elif strategy == 5:
                result = self._sample_deme_niching()
            elif strategy == 6:
                result = self._sample_hybrid_multi()
            else:
                result = self._sample_original()
            
            pop, z = result
            # Validate shapes
            if pop.shape[0] != self.pop_size or pop.shape[1] != self.dim:
                return self._sample_original()
            if z.shape[0] != self.pop_size or z.shape[1] != self.dim:
                # Fix z shape
                if z.shape[0] < self.pop_size:
                    z = np.vstack([z, np.zeros((self.pop_size - z.shape[0], self.dim))])
                elif z.shape[0] > self.pop_size:
                    z = z[:self.pop_size]
    ...
    def __call__(self, func, stopping_condition):
        ...
        self.prev_best_f_for_reward = float(self.best_f)
        ...
        while not stopping_condition():
            ...
            # Record best before sampling
            pre_gen_best = float(self.best_f)

            # Sample new population (adaptive strategy selection happens here)
            ...
            if abs(pre_gen_best) > 1e-30:
                relative_improvement = improvement / (abs(pre_gen_best) + 1e-30)
            else:
                relative_improvement = improvement
            
            # Map to [0, 1] using a sigmoid-like transform
            # Positive improvement -> reward > 0.5, no improvement -> ~0.5
            reward = float(1.0 / (1.0 + np.exp(-1000.0 * relative_improvement)))
            
            # Also give bonus for having best fitness in population
            valid_fit = fitness[np.isfinite(fitness)]
            if len(valid_fit) > 0:
                gen_best = float(np.min(valid_fit))
                gen_median = float(np.median(valid_fit))
                if gen_median > gen_best and np.isfinite(gen_median):
                    quality_bonus = (gen_median - gen_best) / (abs(gen_median) + 1e-30)
                    quality_reward = float(np.clip(quality_bonus, 0.0, 1.0))
                    reward = 0.7 * reward + 0.3 * quality_reward
