...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    Uses Thompson Sampling (Beta distributions) to select among multiple
    _update_covariance_matrix strategies during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.n_strategies = 7  # number of covariance update strategies
        self.strategy_alpha = np.ones(self.n_strategies)  # Beta distribution alpha
        self.strategy_beta = np.ones(self.n_strategies)   # Beta distribution beta
        self.current_strategy = 0
        self.strategy_window_size = 10
        # Sliding window for credit assignment
        self.strategy_history = []  # list of (strategy_idx, fitness_before, fitness_after)
        self.decay_factor = 0.995  # decay old observations
    ...
    def _update_strategy_reward(self, strategy_idx, fitness_before, fitness_after):
        ...
        # Decay all parameters slightly towards prior
        self.strategy_alpha *= self.decay_factor
        self.strategy_beta *= self.decay_factor
        # Ensure minimum values
        self.strategy_alpha = np.maximum(self.strategy_alpha, 1.0)
        self.strategy_beta = np.maximum(self.strategy_beta, 1.0)
        
        # Compute reward: did fitness improve?
        if np.isfinite(fitness_before) and np.isfinite(fitness_after):
            improvement = fitness_before - fitness_after
            if improvement > 1e-15 * (1.0 + abs(fitness_before)):
                # Success - reward proportional to improvement magnitude
                reward_magnitude = min(3.0, 1.0 + np.log1p(abs(improvement) / (abs(fitness_before) + 1e-20)))
                self.strategy_alpha[strategy_idx] += float(reward_magnitude)
            else:
                # No improvement - mild penalty
    ...
    def _update_cov_active(self, old_mean, sorted_population, h_sigma):
        ...
        if max_eigen / min_eigen > 1e12:
            scale_factor = np.sqrt(max_eigen / (min_eigen * 1e12))
            self.C *= scale_factor
        min_eigen_check = np.min(np.linalg.eigvalsh(self.C))
        if min_eigen_check < 1e-15:
            self.C += np.eye(dim) * (1e-14 - min_eigen_check)

    # ========== STRATEGY 2: Fitness-proportional (variant_02) ==========
    def _update_cov_fitness_proportional(self, old_mean, sorted_population, h_sigma):
        rank_one = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
        selected = sorted_population[:self.mu]
        diffs = (selected - old_mean[np.newaxis, :]) / self.sigma
    ...
    def _update_cov_diversity_modulated(self, old_mean, sorted_population, h_sigma):
        rank_one = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
    ...
    # ========== STRATEGY 6: Active CMA with negative weights (variant_08) ==========
    def _update_cov_active_negative(self, old_mean, sorted_population, h_sigma):
        rank_one = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
    ...
    def _update_covariance_matrix(self, old_mean, sorted_population, h_sigma):
        """Adaptive covariance matrix update using Thompson Sampling to select strategy."""
        ...
        try:
            if strategy == 0:
                ...
                self._update_cov_fitness_proportional(old_mean, sorted_population, h_sigma)
            elif strategy == 3:
                ...
                self._update_cov_diversity_modulated(old_mean, sorted_population, h_sigma)
            elif strategy == 6:
                self._update_cov_active_negative(old_mean, sorted_population, h_sigma)
            else:
                self._update_cov_original(old_mean, sorted_population, h_sigma)
            ...
            if not np.all(np.isfinite(self.C)):
                self.C = C_backup
                # Penalize this strategy
                self.strategy_beta[strategy] += 2.0
        except Exception:
            self.C = C_backup
            self.strategy_beta[strategy] += 2.0
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            # Select strategy for this generation using Thompson Sampling
            self._select_strategy()
            fitness_before_gen = float(self.best_f)
            ...
            # Credit assignment for the strategy used
            fitness_after_gen = float(self.best_f)
            self._update_strategy_reward(self.current_strategy, fitness_before_gen, fitness_after_gen)
