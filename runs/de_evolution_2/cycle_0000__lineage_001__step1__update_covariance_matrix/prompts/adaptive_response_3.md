...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    for the covariance matrix update strategy. Uses Thompson Sampling to
    select among multiple _update_covariance_matrix variants during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.ts_alpha = np.ones(self.n_strategies, dtype=float)  # Beta distribution alpha
        self.ts_beta = np.ones(self.n_strategies, dtype=float)   # Beta distribution beta
        self.current_strategy = 0
        self.pre_update_best_f = np.inf
        
        # Sliding window for credit assignment
        self.window_size = 20
        self.reward_history = [[] for _ in range(self.n_strategies)]
    ...
    # ========== Strategy 1: Active CMA (variant_01) ==========
    ...
    def _update_cov_diversity(self, old_mean, sorted_population, h_sigma):
        rank_one = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
    ...
    # ========== Strategy 6: Full-population weighted (variant_10) ==========
    def _update_cov_full_pop(self, old_mean, sorted_population, h_sigma):
        rank_one = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)

        all_diffs = (sorted_population - self.mean[np.newaxis, :]) / self.sigma
        ...
        weighted_diffs = fit_weights[:, np.newaxis] * all_diffs
        full_pop_cov = all_diffs.T @ weighted_diffs

        if self.best_f < self.last_best_f:
            c_full = self.c_mu_cov * 1.5
        else:
            c_full = self.c_mu_cov * 0.5
    ...
    def _select_strategy(self):
        """Thompson Sampling: sample from Beta distributions and pick best."""
        samples = np.array([
    ...
    def _update_strategy_reward(self, strategy_idx, reward):
        """Update Beta distribution parameters for the chosen strategy."""
        reward = float(reward)
        # Clamp reward to [0, 1]
    ...
    def _update_covariance_matrix(self, old_mean, sorted_population, h_sigma):
        """
        Adaptive covariance matrix update: selects among multiple strategies
        using Thompson Sampling.
        """
        # Save state before update for credit assignment
        ...
        try:
            if self.current_strategy == 0:
                ...
                self._update_cov_diversity(old_mean, sorted_population, h_sigma)
            elif self.current_strategy == 6:
                self._update_cov_full_pop(old_mean, sorted_population, h_sigma)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Record best_f before this generation for credit assignment
            pre_gen_best_f = float(self.best_f)
            ...
            # Store pre-update best for credit assignment
            self.pre_update_best_f = float(self.best_f)
            ...
            # Reward based on improvement: did best_f improve this generation?
            post_gen_best_f = float(self.best_f)
            if np.isfinite(pre_gen_best_f) and np.isfinite(post_gen_best_f):
                improvement = pre_gen_best_f - post_gen_best_f
                if improvement > 0:
                    # Scale reward by relative improvement
                    rel_improvement = improvement / (abs(pre_gen_best_f) + 1e-20)
                    reward = float(np.clip(np.tanh(rel_improvement * 100), 0.1, 1.0))
                else:
                    # No improvement - small reward if not getting worse
                    reward = 0.2
            else:
                reward = 0.1
