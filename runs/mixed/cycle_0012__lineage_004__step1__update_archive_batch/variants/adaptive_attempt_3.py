...
class CulturalDEWithAdaptiveArchive:
    ...
    Mutation, and Adaptive Operator Selection using Multi-Armed Bandit.
    ...
    - Adaptive operator selection for archive updates (Thompson Sampling)
    - 7 different archive strategies learned from benchmark results
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self._operator_names = ['v01', 'v02', 'v03', 'v05', 'v06', 'v09', 'v10']
        self._n_operators = len(self._operator_names)
        self._operator_successes = [0.0] * self._n_operators
        self._operator_counts = [0] * self._n_operators
        self._current_operator = 0
        self._reward_window = 15
        
        # Sliding window for each operator
        self._operator_rewards = [[] for _ in range(self._n_operators)]
    
    def _select_operator(self):
        """Thompson Sampling for operator selection with Beta distribution."""
        samples = np.zeros(self._n_operators)
        for i in range(self._n_operators):
            if self._operator_counts[i] == 0:
                samples[i] = np.random.uniform(0, 0.3)
            else:
                alpha = max(1.0, self._operator_successes[i] + 1.0)
                beta = max(1.0, self._operator_counts[i] - self._operator_successes[i] + 1.0)
                samples[i] = np.random.beta(alpha, beta)
        return int(np.argmax(samples))
    
    def _credit_operator(self, op_idx, reward):
        """Update operator statistics with sliding window credit assignment."""
        reward = float(np.clip(reward, 0.0, 1.0))
        self._operator_counts[op_idx] += 1
        self._operator_rewards[op_idx].append(reward)
        if len(self._operator_rewards[op_idx]) > self._reward_window:
            self._operator_rewards[op_idx].pop(0)
        if reward > 0.5:
            self._operator_successes[op_idx] += reward
    
    def _update_archive_adaptive(self, archive, new_solutions, new_fitness, f_best_before):
        """Update archive using adaptive operator selection."""
        if len(new_solutions) == 0:
            return archive
        
        op_idx = self._select_operator()
        self._current_operator = op_idx
        ...
        new_archive = strategies[op_idx](archive, new_solutions, new_fitness)
        
        f_best_after = float(np.min(new_fitness)) if len(new_fitness) > 0 else f_best_before
        fit_range = max(abs(f_best_before), abs(f_best_after), 1e-10)
        improvement = max(0.0, min(1.0, (f_best_before - f_best_after) / fit_range))
        self._credit_operator(op_idx, improvement)
        
        return new_archive
    
    def _strategy_v01(self, archive, new_solutions, new_fitness):
        """Hypergrid with cell capacity and diversity bonus (4 wins)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _strategy_v02(self, archive, new_solutions, new_fitness):
        """Tier-based with greedy diversity selection (1 win)."""
        if len(new_solutions) == 0:
            return archive
        
        combined = np.vstack([archive, new_solutions]) if len(archive) > 0 else new_solutions
    ...
    def _strategy_v03(self, archive, new_solutions, new_fitness):
        """Fitness-distance Pareto selection (2 wins)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _strategy_v05(self, archive, new_solutions, new_fitness):
        """Fitness-diversity composite scoring (1 win)."""
        if len(new_solutions) == 0:
            return archive
        ...
        if n_new > 0:
            ...
            new_best_count = self.archive_max - (arch_best_count if n_arch > 0 else 0)
    ...
    def _strategy_v06(self, archive, new_solutions, new_fitness):
        """Centroid-based clustering with greedy selection (1 win)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _strategy_v09(self, archive, new_solutions, new_fitness):
        """Crowding distance in objective space (2 wins)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _strategy_v10(self, archive, new_solutions, new_fitness):
        """Nearest-neighbor diversity with composite scoring (1 win)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _find_unique_solutions(self, solutions, tol=1e-3):
        ...
        for i in range(n):
            ...
            if i + 1 < n:
                ...
                if len(diffs.shape) > 1 and np.any(np.all(diffs < tol, axis=1)):
                    is_unique[i+1:] = False
                elif len(diffs.shape) == 1 and np.all(diffs < tol):
                    is_unique[i+1:] = False
        return is_unique
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            f_best_before = f_best
            
            population, fitness, archive, success_F, success_CR = self._select_survivors_batch(
                population, fitness, trials, trial_fit, archive, success_F, success_CR, f_best_before
            )
            ...
            if float(fitness[new_best_idx]) < f_best - 1e-12:
                f_best = float(fitness[new_best_idx])
    ...
    def _select_survivors_batch(self, population, fitness, trials, trial_fit, 
                                 archive, success_F, success_CR, f_best_before):
        NP = len(population)
        ...
        new_archive = self._update_archive_adaptive(
            archive, new_pop[improve_idx], new_fit[improve_idx], f_best_before
        )
