...
class CulturalDEWithAdaptiveArchive:
    """
    Cultural Differential Evolution with Adaptive Archive Selection.
    
    Uses Multi-Armed Bandit with Thompson Sampling to dynamically select
    the best archive update strategy during optimization based on
    benchmark results from 24 test functions.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = [
            'variant_01', 'variant_02', 'variant_03', 
            'variant_05', 'variant_06', 'variant_09', 'variant_10'
        ]
        ...
        self.operator_selection_counts = [0] * self.num_operators
        self.total_selections = 0
        self.reward_window_size = 50
        self.selection_count_threshold = 5
    ...
    def _select_operator(self):
        ...
        for i in range(self.num_operators):
            successes = sum(1 for r in self.operator_rewards[i] if r > 0)
            failures = len(self.operator_rewards[i]) - successes
            alpha = max(1.0, successes + 1.0)
            beta = max(1.0, failures + 1.0)
            samples[i] = np.random.beta(alpha, beta)
        
        selected = int(np.argmax(samples))
        self.operator_selection_counts[selected] += 1
        self.total_selections += 1
        return selected
    
    def _update_operator_stats(self, operator_idx, reward):
        """Update statistics for the selected operator with sliding window."""
        safe_reward = float(np.clip(reward if np.isfinite(reward) else 0.0, -1e10, 1e10))
        self.operator_rewards[operator_idx].append(safe_reward)
        
        # Sliding window
        if len(self.operator_rewards[operator_idx]) > self.reward_window_size:
            self.operator_rewards[operator_idx] = self.operator_rewards[operator_idx][-self.reward_window_size:]
    
    def _compute_reward(self, new_solutions, new_fitness, archive_size):
        """Compute reward based on improvement quality."""
        if len(new_fitness) == 0 or archive_size == 0:
            return 0.0
        reward = float(np.mean(np.clip(new_fitness, -1e10, 1e10))) / float(archive_size)
        return reward if np.isfinite(reward) else 0.0
    
    def _get_operator_method(self, operator_idx):
        """Return the appropriate archive update method."""
        methods = [
            self._update_archive_batch_variant_01,
            self._update_archive_batch_variant_02,
            self._update_archive_batch_variant_03,
            self._update_archive_batch_variant_05,
            self._update_archive_batch_variant_06,
            self._update_archive_batch_variant_09,
            self._update_archive_batch_variant_10,
        ]
        return methods[operator_idx]
    ...
    def _select_survivors_batch(self, population, fitness, trials, trial_fit, 
        ...
        # Adaptive operator selection for archive update
        if gen % 10 == 0 or current_operator >= len(self.operator_names):
            current_operator = self._select_operator()
        
        operator_method = self._get_operator_method(current_operator)
        new_archive = operator_method(archive, new_pop[improve_idx], new_fit[improve_idx])
        
        # Update operator statistics
        if len(improve_idx) > 0:
            reward = self._compute_reward(new_pop[improve_idx], new_fit[improve_idx], len(new_archive))
            self._update_operator_stats(current_operator, reward)
        
        # Update operator periodically
        if gen % 10 == 0:
            current_operator = self._select_operator()
    ...
    # ============== Archive Update Variants ==============
    
    def _update_archive_batch_variant_01(self, archive, new_solutions, new_fitness):
        """Variant 01: Hypergrid with diversity bonus (4 benchmark wins)."""
    ...
    def _update_archive_batch_variant_02(self, archive, new_solutions, new_fitness):
        """Variant 02: Elite + greedy diversity selection (1 benchmark win)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _update_archive_batch_variant_03(self, archive, new_solutions, new_fitness):
        """Variant 03: Fitness-distance Pareto selection (2 benchmark wins)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _update_archive_batch_variant_05(self, archive, new_solutions, new_fitness):
        """Variant 05: Fitness-diversity composite scoring (1 benchmark win)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _update_archive_batch_variant_06(self, archive, new_solutions, new_fitness):
        """Variant 06: Centroid-based clustering selection (1 benchmark win)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _update_archive_batch_variant_09(self, archive, new_solutions, new_fitness):
        """Variant 09: Crowding distance selection (2 benchmark wins)."""
        if len(new_solutions) == 0:
            return archive
    ...
    def _update_archive_batch_variant_10(self, archive, new_solutions, new_fitness):
        """Variant 10: Nearest-neighbor diversity scoring (1 benchmark win)."""
        if len(new_solutions) == 0:
            return archive
