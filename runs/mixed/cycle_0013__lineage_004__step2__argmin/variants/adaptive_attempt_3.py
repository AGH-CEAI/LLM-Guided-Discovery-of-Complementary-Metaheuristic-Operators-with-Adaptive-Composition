...
class CulturalDEWithAdaptiveArchive:
    ...
    Mutation, Targeted Local Search, and Adaptive _argmin Selection via 
    ...
    - ADAPTIVE _argmin selection via Thompson Sampling Bandit
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive _argmin: Thompson Sampling Multi-Armed Bandit with 8 variants + original
        self._num_argmin_ops = 9
        self._ts_alpha = np.ones(self._num_argmin_ops)
        self._ts_beta = np.ones(self._num_argmin_ops)
        self._ts_selections = np.zeros(self._num_argmin_ops, dtype=int)
        self._ts_rewards = [[] for _ in range(self._num_argmin_ops)]
        self._sliding_window = 30
        self._min_samples_for_selection = 3
        self._current_argmin_op = 0
        self._generation = 0
        self._last_pop = None
        self._current_pop = None
        self._argmin_temp = 0.5
        
    def _select_argmin_operator_ts(self):
        """Thompson Sampling: sample from Beta distribution for each operator."""
        samples = np.random.beta(self._ts_alpha, self._ts_beta)
        selected = int(np.argmax(samples))
        self._ts_selections[selected] += 1
        return selected
    
    def _update_argmin_bandit(self, op_id, improvement, fitness_before, fitness_after):
        """Update Thompson Sampling bandit based on selection outcome."""
        improvement = float(np.clip(improvement, -1e10, 1e10))
        
        self._ts_rewards[op_id].append(improvement)
        if len(self._ts_rewards[op_id]) > self._sliding_window:
            self._ts_rewards[op_id] = self._ts_rewards[op_id][-self._sliding_window:]
        
        if len(self._ts_rewards[op_id]) >= self._min_samples_for_selection:
            recent_rewards = np.array(self._ts_rewards[op_id], dtype=np.float64)
            pos_reward = np.sum(recent_rewards < 0)
            neg_reward = np.sum(recent_rewards >= 0)
            self._ts_alpha[op_id] = 1.0 + float(pos_reward) * 0.5
            self._ts_beta[op_id] = 1.0 + float(neg_reward) * 0.5
    ...
    def _argmin(self, arr):
        ...
        if op == 0:
            ...
            adapt = min(0.6 + 0.3 * np.tanh(self._generation / 100), 0.9)
            composite = (adapt * fitness_score + (1 - adapt) * 0.5 * diversity_score)
            
            k = max(3, len(arr) // 4)
            top_indices = np.argsort(composite)[-k:]
            top_probs = np.clip(composite[top_indices], 1e-15, None)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            self._current_argmin_op = self._select_argmin_operator_ts()
            fitness_before_sel = fitness.copy()
            ...
            fitness_after_sel = fitness.copy()
            improvement = float(np.mean(fitness_after_sel - fitness_before_sel))
            self._update_argmin_bandit(self._current_argmin_op, improvement, fitness_before_sel, fitness_after_sel)
            ...
            if float(fitness[new_best_idx]) < f_best - 1e-12:
                f_best, x_best = float(fitness[new_best_idx]), population[new_best_idx].copy()
                stagnation = 0
            else:
                stagnation += 1
    ...
    def _update_archive_batch(self, archive, new_solutions, new_fitness):
        ...
        for i in range(0, n_combined, chunk_size):
            ...
            chunk_range = end_i - i
            if chunk_range > 0 and n_combined >= i + chunk_size:
                dists[np.arange(chunk_range), np.arange(i, end_i)] = np.inf
            elif chunk_range > 0:
                dists[np.arange(chunk_range), np.arange(i, n_combined)] = np.inf
    ...
    def _find_unique_solutions(self, solutions, tol=1e-3):
        ...
        for i in range(n):
            ...
            if i + 1 < n:
                ...
                if len(diffs.shape) == 2 and np.any(np.all(diffs < tol, axis=1)):
                    is_unique[i+1:] = False
        return is_unique
