...
class CulturalMemeticDE:
    """
    Adaptive DE with Thompson Sampling for parameter adaptation strategy selection.
    Automatically learns which parameter adaptation strategy works best during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Parameter Strategy Selection (NEW) ===
        self.num_param_strategies = 3
        self.param_strategy_names = ['original', 'variant_03', 'variant_07']
        self.param_strategy_alpha = np.ones(self.num_param_strategies)
        self.param_strategy_beta = np.ones(self.num_param_strategies)
        self.param_strategy_rewards = [[] for _ in range(self.num_param_strategies)]
        self.current_param_strategy_idx = 0
        self.param_strategy_switch_gen = 0
        self.param_strategy_min_switch_gens = 10
        self.param_strategy_reward_window = 30
        self.param_strategy_switches = []
        self._prev_best_fitness_for_param = np.inf
        self._param_strategy_improvement_count = [0, 0, 0]
    ...
    def _select_param_strategy_thompson(self):
        """Select parameter adaptation strategy using Thompson Sampling"""
        samples = np.random.beta(self.param_strategy_alpha, self.param_strategy_beta)
        selected = int(np.argmax(samples))
        return selected
    ...
    def _compute_param_strategy_reward(self, fitness_before, fitness_after):
        """Compute reward for parameter adaptation strategy"""
        fitness_before = np.clip(np.asarray(fitness_before, dtype=np.float64), -1e50, 1e50)
        fitness_after = np.clip(np.asarray(fitness_after, dtype=np.float64), -1e50, 1e50)

        best_before = float(np.min(fitness_before))
        best_after = float(np.min(fitness_after))

        if np.isfinite(best_before) and np.isfinite(best_after):
            improvement = best_before - best_after
            denom = max(abs(best_before), 1e-10, abs(best_after))
            reward = float(np.clip(improvement / denom, -1.0, 1.0))
        else:
            reward = 0.0
    ...
    def _update_param_strategy_rewards(self, strategy_idx, reward):
        """Update rewards for parameter strategy with sliding window"""
        rewards = self.param_strategy_rewards[strategy_idx]
        rewards.append(float(reward))

        if len(rewards) > self.param_strategy_reward_window:
            rewards.pop(0)
        ...
        if len(recent) >= 3:
            ...
            alpha_val = 1.0 + success_rate * 5.0 + avg_reward * 2.0
            beta_val = 1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0

            self.param_strategy_alpha[strategy_idx] = float(np.clip(alpha_val, 0.1, 100.0))
            self.param_strategy_beta[strategy_idx] = float(np.clip(beta_val, 0.1, 100.0))

            if float(reward) > 0.01:
                self._param_strategy_improvement_count[strategy_idx] += 1
    ...
    def __call__(self, func, stopping_condition):
        ...
        self._prev_best_fitness_for_param = self.best_fitness

        self.current_operator_idx = self._select_operator_thompson()
        self.current_param_strategy_idx = self._select_param_strategy_thompson()
        ...
        while not stopping_condition():
            ...
            fitness_before_selection = fitness.copy()
            ...
            # Adaptive parameter strategy selection
            param_reward = self._compute_param_strategy_reward(fitness_before_selection, fitness)
            self._update_param_strategy_rewards(self.current_param_strategy_idx, param_reward)
            self._adapt_parameters_batch(population, fitness, mutants, trials)
            self._prev_best_fitness_for_param = float(np.min(fitness))
            ...
            if self.generation - self.param_strategy_switch_gen >= self.param_strategy_min_switch_gens:
                new_strategy = self._select_param_strategy_thompson()
                if new_strategy != self.current_param_strategy_idx:
                    self.current_param_strategy_idx = new_strategy
                    self.param_strategy_switch_gen = self.generation
                    self.param_strategy_switches.append((self.generation, new_strategy))
    ...
    def _adapt_parameters_batch(self, population, fitness, mutants, trials):
        ...
        if self.current_param_strategy_idx == 0:
            self._adapt_parameters_original(population, fitness, mutants, trials)
        elif self.current_param_strategy_idx == 1:
            self._adapt_parameters_variant03(population, fitness, mutants, trials)
        else:
    ...
    def _adapt_parameters_original(self, population, fitness, mutants, trials):
        """Original parameter adaptation strategy"""
        sorted_idx = np.argsort(fitness)
        top_half = sorted_idx[:max(1, self.NP // 2)]
        ...
        if np.random.rand() < 0.2:
            ...
            self.p_best_rate = float(np.clip(self.p_best_rate + delta, 0.05, 0.3))

    def _adapt_parameters_variant03(self, population, fitness, mutants, trials):
        """Variant 03: weighted improvement rate adaptation"""
        ...
        if np.isfinite(self._prev_best_fitness_for_param):
            direction = self._prev_best_fitness_for_param - current_best
            decay = 0.7
            self._adapt_weighted_improvement = decay * self._adapt_weighted_improvement + (1 - decay) * direction
        self._prev_best_fitness_for_param = current_best
    ...
    def _adapt_parameters_variant07(self, population, fitness, mutants, trials):
        """Variant 07: success history tracking adaptation"""
        ...
        if trials is not None and len(trials) > 0:
            try:
                ...
                if n_success > 0:
                    for _ in range(min(n_success, self.history_max_size - len(self.F_history))):
