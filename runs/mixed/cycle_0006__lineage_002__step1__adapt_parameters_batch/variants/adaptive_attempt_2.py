...
class CulturalMemeticDE:
    ...
    Now includes adaptive parameter adaptation strategy selection using
    Thompson Sampling to dynamically choose between original, variant_03,
    and variant_07 adaptation strategies based on their recent performance.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.adapt_strategy_names = ['original', 'variant_03', 'variant_07']
        ...
        self.min_adapt_switch_gens = 8
        self.adapt_switches = []
        self.last_best_fitness_for_adapt = np.inf

        # Per-strategy state for variant_03 (weighted improvement tracking)
        ...
        # Per-strategy state for variant_07 (success history tracking)
        self.F_history = []
        self.CR_history = []
    ...
    def _select_adapt_strategy_thompson(self):
        """Select adaptation strategy using Thompson Sampling from Beta distributions"""
        samples = np.random.beta(self.adapt_alpha, self.adapt_beta)
        selected = int(np.argmax(samples))
        return selected

    def _compute_adapt_reward(self, old_best, new_best):
        """Compute reward for adaptation strategy based on improvement quality"""
        old_best = float(np.clip(old_best, -1e50, 1e50))
        new_best = float(np.clip(new_best, -1e50, 1e50))

        improvement = old_best - new_best

        if improvement > 0:
            reward = float(np.clip(np.log1p(improvement) / 10.0, 0.0, 1.0))
        else:
            reward = float(np.clip(improvement / 100.0, -1.0, 0.0))

        return reward

    def _update_adapt_strategy(self, strategy_idx, reward):
        """Update adaptation strategy performance with sliding window"""
        rewards = self.adapt_strategy_rewards[strategy_idx]
        rewards.append(float(reward))

        if len(rewards) > self.adapt_reward_window:
            rewards.pop(0)

        recent = rewards[-min(10, len(rewards)):]
        if len(recent) >= 3:
            successes = sum(1.0 for r in recent if float(r) > 0.0)
            success_rate = successes / max(float(len(recent)), 1.0)
            avg_reward = float(np.mean(recent))

            self.adapt_alpha[strategy_idx] = float(np.clip(1.0 + success_rate * 5.0 + max(0.0, avg_reward) * 3.0, 0.1, 100.0))
            self.adapt_beta[strategy_idx] = float(np.clip(1.0 + (1.0 - success_rate) * 5.0 + max(0.0, -avg_reward) * 3.0, 0.1, 100.0))

    def _adapt_parameters_original(self, population, fitness, mutants, trials):
        """Original adaptation: simple random sampling"""
    ...
    def _adapt_parameters_variant03(self, population, fitness, mutants, trials):
        """Variant 03 adaptation: weighted improvement rate with momentum"""
        ...
        if np.isfinite(self._prev_best_fitness):
            direction = self._prev_best_fitness - current_best
            decay = 0.7
            self._adapt_weighted_improvement = decay * self._adapt_weighted_improvement + (1.0 - decay) * direction
        ...
        if len(self._adapt_history_F) > 0:
            momentum = 0.3
            new_F = momentum * float(np.mean(self._adapt_history_F[-5:])) + (1.0 - momentum) * new_F
            new_CR = momentum * float(np.mean(self._adapt_history_CR[-5:])) + (1.0 - momentum) * new_CR
    ...
    def _adapt_parameters_variant07(self, population, fitness, mutants, trials):
        """Variant 07 adaptation: success history tracking with stagnation reset"""
        ...
        if trials is not None and len(trials) > 0:
            try:
                ...
                if n_success > 0:
                    for _ in range(min(n_success, 5)):
                        if len(self.F_history) < self.history_max_size:
                            self.F_history.append(float(self.F))
                            self.CR_history.append(float(self.CR))

                    success_rate = float(n_success) / max(float(len(fitness)), 1.0)
        ...
        if self.stagnation_count > 15:
            if len(self.F_history) >= 5:
                ...
                self.F = float(np.clip(float(np.median(recent_f)) + np.random.uniform(-0.1, 0.1), 0.3, 1.5))
                self.CR = float(np.clip(float(np.median(recent_cr)) + np.random.uniform(-0.1, 0.1), 0.1, 0.95))
            else:
    ...
    def _adapt_parameters_batch(self, population, fitness, mutants, trials):
        """Dispatch to selected adaptation strategy"""
        ...
        elif self.current_adapt_strategy == 2:
    ...
    def __call__(self, func, stopping_condition):
        ...
        self.last_best_fitness_for_adapt = self.best_fitness

        self.current_adapt_strategy = self._select_adapt_strategy_thompson()
        self.current_operator_idx = 0
        ...
        while not stopping_condition():
            ...
            old_best_for_adapt = self.last_best_fitness_for_adapt
            ...
            new_best_for_adapt = current_best
            adapt_reward = self._compute_adapt_reward(old_best_for_adapt, new_best_for_adapt)
            self._update_adapt_strategy(self.current_adapt_strategy, adapt_reward)
            self.last_best_fitness_for_adapt = new_best_for_adapt
            ...
            if self.generation - self.last_adapt_switch_gen >= self.min_adapt_switch_gens:
            ...
            if self._check_diversity_low(population):
                population, fitness = self._reinitialize_population(population, fitness)
