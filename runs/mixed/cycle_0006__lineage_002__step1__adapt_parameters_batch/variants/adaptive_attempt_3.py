...
class CulturalMemeticDE:
    ...
    Automatically learns which mutation strategy and parameter adaptation strategy works best during optimization.
    Includes adaptive selection between multiple _adapt_parameters_batch strategies.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.adapt_reward_window_size = 25
        self.current_adapt_strategy = 0
        self.last_adapt_switch_gen = 0
        self.adapt_min_switch_gens = 8
        self.adapt_strategy_switches = []

        # Per-adaptation-strategy tracking state
        self._init_adapt_state()

    def _init_adapt_state(self):
        """Initialize state variables for all adaptation strategies"""
        # State for variant_03 strategy
    ...
    def _update_adapt_strategy_rewards(self, strategy_idx, reward):
        ...
        rewards = self.adapt_rewards[strategy_idx]
        rewards.append(float(reward))

        if len(rewards) > self.adapt_reward_window_size:
            rewards.pop(0)
        ...
        if len(recent) >= 3:
            ...
            self.adapt_alpha[strategy_idx] = float(np.clip(1.0 + success_rate * 5.0 + avg_reward * 2.0, 0.1, 100.0))
            self.adapt_beta[strategy_idx] = float(np.clip(1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0, 0.1, 100.0))
    ...
    def _adapt_parameters_variant03(self, population, fitness, mutants, trials):
        """Variant 03: Directional improvement tracking with momentum"""
        current_best = float(np.min(fitness[np.isfinite(fitness)]) if np.any(np.isfinite(fitness)) else np.inf)

        if np.isfinite(self._prev_best_fitness) and np.isfinite(current_best):
    ...
    def _adapt_parameters_variant07(self, population, fitness, mutants, trials):
        """Variant 07: Success history tracking with stagnation reset"""
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            self._update_adapt_strategy_rewards(self.current_adapt_strategy, reward)
            ...
            if self.generation - self.last_adapt_switch_gen >= self.adapt_min_switch_gens:
                ...
                if new_adapt != self.current_adapt_strategy:
                    ...
                    self.adapt_strategy_switches.append((self.generation, new_adapt))
    ...
    def _mutate_variant07(self, population, fitness):
        ...
        if self.stagnation_count > 10:
            F_adapted = min(self.F * (1.0 + 0.2 * float(self.stagnation_count - 10)), 2.0)

        base = population + F_adapted * (global_best - population) + F_adapted * (population[r1] - population[r2])

        if self.stagnation_count > 5:
            perturbation_scale = min(0.3 * np.log1p(float(self.stagnation_count)), 2.0)
