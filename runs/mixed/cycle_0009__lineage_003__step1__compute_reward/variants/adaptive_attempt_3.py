...
class CulturalMemeticDE:
    """
    Adaptive DE with Thompson Sampling for operator selection and adaptive reward strategy selection.
    Automatically learns which mutation strategy AND reward computation strategy works best during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # 8 reward strategies based on benchmark winners
        ...
        # Beta distribution parameters for reward strategy Thompson Sampling
        self.reward_alpha = np.ones(self.num_reward_strategies)
        self.reward_beta = np.ones(self.num_reward_strategies)
        # Sliding window reward tracking for each strategy
        self.reward_strategy_rewards = [[] for _ in range(self.num_reward_strategies)]
        self.current_reward_strategy = 0
        self.last_reward_switch_gen = 0
        self.reward_min_switch_gens = 8  # Slightly longer than operator switching
        self.reward_switches = []
        # Track cumulative performance per strategy
        self.reward_strategy_perf = np.zeros(self.num_reward_strategies)
    ...
    def _select_reward_strategy_thompson(self):
        """Select reward strategy using Thompson Sampling from Beta distributions"""
    ...
    def _compute_reward_variant_01(self, fitness, trial_fitness):
        """Variant 01: Rank-based reward with success rate and relative improvement (3 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant_02(self, fitness, trial_fitness):
        """Variant 02: Log-scaled reward with consistency bonus (2 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        ...
        consistency_ratio = float(n_improved) / max(float(len(fitness)), 1.0)
    ...
    def _compute_reward_variant_05(self, fitness, trial_fitness):
        """Variant 05: Log-scaled consistency reward (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant_06(self, fitness, trial_fitness):
        """Variant 06: Best individual improvement + rank-based robustness (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant_07(self, fitness, trial_fitness):
        """Variant 07: Logarithmic improvement toward target 1e-8 (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant_08(self, fitness, trial_fitness):
        """Variant 08: Stagnation-aware bimodal strategy (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant_10(self, fitness, trial_fitness):
        """Variant 10: Relative improvement prioritizing hard unsolved tasks (5 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward(self, fitness, trial_fitness):
        ...
        if self.current_reward_strategy == 0:
            ...
            return self._compute_reward_variant_01(fitness, trial_fitness)
        elif self.current_reward_strategy == 2:
            return self._compute_reward_variant_02(fitness, trial_fitness)
        elif self.current_reward_strategy == 3:
            return self._compute_reward_variant_05(fitness, trial_fitness)
        elif self.current_reward_strategy == 4:
            return self._compute_reward_variant_06(fitness, trial_fitness)
        elif self.current_reward_strategy == 5:
            return self._compute_reward_variant_07(fitness, trial_fitness)
        elif self.current_reward_strategy == 6:
            return self._compute_reward_variant_08(fitness, trial_fitness)
        else:
            return self._compute_reward_variant_10(fitness, trial_fitness)

    def _compute_all_reward_strategies(self, fitness, trial_fitness):
        """Compute rewards for all strategies (for adaptive learning)"""
        rewards = []
        for i in range(self.num_reward_strategies):
            if i == 0:
                r = self._compute_reward_original(fitness, trial_fitness)
            elif i == 1:
                r = self._compute_reward_variant_01(fitness, trial_fitness)
            elif i == 2:
                r = self._compute_reward_variant_02(fitness, trial_fitness)
            elif i == 3:
                r = self._compute_reward_variant_05(fitness, trial_fitness)
            elif i == 4:
                r = self._compute_reward_variant_06(fitness, trial_fitness)
            elif i == 5:
                r = self._compute_reward_variant_07(fitness, trial_fitness)
            elif i == 6:
                r = self._compute_reward_variant_08(fitness, trial_fitness)
            else:
                r = self._compute_reward_variant_10(fitness, trial_fitness)
    ...
    def _update_reward_strategy(self, strategy_idx, reward):
        """Update reward strategy performance with sliding window"""
        ...
        self.reward_strategy_perf[strategy_idx] += reward
        self.reward_strategy_counts[strategy_idx] += 1.0
        ...
        if len(recent) >= 3:
            ...
            self.reward_alpha[strategy_idx] = 1.0 + success_rate * 5.0 + avg_reward * 2.0
            self.reward_beta[strategy_idx] = 1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0

            self.reward_alpha[strategy_idx] = float(np.clip(self.reward_alpha[strategy_idx], 0.1, 100.0))
            self.reward_beta[strategy_idx] = float(np.clip(self.reward_beta[strategy_idx], 0.1, 100.0))
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize operator and reward strategy selection
        ...
        while not stopping_condition():
            ...
            # Compute all reward strategies for adaptive learning
            all_rewards = self._compute_all_reward_strategies(fitness, trial_fitness)
            ...
            if current_best < self.best_fitness:
                ...
                # Boost reward for actual improvements (bonus signal)
                reward = reward + 0.2 * min(improvement / (abs(self.best_fitness) + 1e-10), 1.0)
            else:
            ...
            # Update ALL reward strategy performances (for learning)
            for i in range(self.num_reward_strategies):
                # Use the actual improvement as bonus for all strategies when we improved
                strategy_reward = all_rewards[i]
                if current_best < self.best_fitness or self.stagnation_count == 0:
                    strategy_reward = strategy_reward + 0.1 * min(improvement / (abs(self.best_fitness) + 1e-10), 1.0)
                self._update_reward_strategy(i, strategy_reward)
            ...
            # Reward strategy switching with minimum generations between switches
            if self.generation - self.last_reward_switch_gen >= self.reward_min_switch_gens:
                new_strategy = self._select_reward_strategy_thompson()
                if new_strategy != self.current_reward_strategy:
                    self.current_reward_strategy = new_strategy
                    self.last_reward_switch_gen = self.generation
                    self.reward_switches.append((self.generation, new_strategy))
