...
class CulturalMemeticDE:
    ...
    Now with ADAPTIVE _compute_reward strategy selection that automatically
    learns which reward computation method works best during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Map reward strategies to their names for clarity
        self.reward_strategy_names = {
            0: 'original',
            1: 'variant_01',  # 3 wins: rank-based with success rate and relative improvement
            2: 'variant_02',  # 2 wins: log-scaled consistency reward
            3: 'variant_05',  # 1 win: per-individual log-scaled improvement
            4: 'variant_06',  # 1 win: best individual improvement + rank robustness
            5: 'variant_07',  # 1 win: log improvement toward target
            6: 'variant_08',  # 1 win: stagnation-aware bimodal strategy
            7: 'variant_10',  # 5 wins: relative improvement prioritizing hard tasks
        }
        ...
        # Track reward strategy history with exponential weighted average
        self.reward_strategy_ewma = np.zeros(self.num_reward_strategies)
        self.ewma_decay = 0.95
        # Sliding window for recent rewards per strategy
        self.reward_strategy_history = [[] for _ in range(self.num_reward_strategies)]
        self.current_reward_strategy = 0
        self.last_reward_switch_gen = 0
        self.reward_switch_interval = 10  # Minimum generations between strategy switches
        # Track which reward strategy was used for each generation
        self.reward_strategy_usage = [0] * self.num_reward_strategies
    ...
    def _update_reward_strategy_performance(self, strategy_idx, reward):
        """Update performance tracking for a reward strategy"""
        # Update sliding window of recent rewards
        history = self.reward_strategy_history[strategy_idx]
        history.append(float(reward))
        if len(history) > self.reward_window_size:
            history.pop(0)
        
        # Compute exponential weighted moving average
        if len(history) > 0:
            ewma = float(np.mean(history))
            self.reward_strategy_ewma[strategy_idx] = (
                self.ewma_decay * self.reward_strategy_ewma[strategy_idx] +
                (1 - self.ewma_decay) * ewma
            )
        ...
        if len(recent) >= 2:
            ...
            std_reward = float(np.std(recent)) if len(recent) > 1 else 0.1
            # Reward strategies that give higher, less variable rewards
            score = avg_reward + 0.5 * std_reward
            
            # Update Beta distribution parameters
            self.reward_alpha[strategy_idx] = float(np.clip(
                1.0 + max(score, 0.0) * 10.0, 0.1, 100.0
            ))
            self.reward_beta[strategy_idx] = float(np.clip(
                1.0 + max(-score, 0.0) * 10.0 + 1.0, 0.1, 100.0
            ))

    def _compute_reward(self, fitness, trial_fitness):
        """Compute reward using the currently selected adaptive strategy"""
    ...
    def _compute_reward_original(self, fitness, trial_fitness):
        """Original reward: simple improvement-based"""
    ...
    def _compute_reward_variant01(self, fitness, trial_fitness):
        """Rank-based reward with success rate and relative improvement (3 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        improvements = fitness - trial_fitness
        n_improved = int(np.sum(improvements > 0))
    ...
    def _compute_reward_variant02(self, fitness, trial_fitness):
        """Log-scaled consistency reward (2 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        improvements = fitness - trial_fitness
        n_improved = int(np.sum(improvements > 0))
    ...
    def _compute_reward_variant05(self, fitness, trial_fitness):
        """Per-individual log-scaled improvement (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        improvements = fitness - trial_fitness
        reward_components = []
    ...
    def _compute_reward_variant06(self, fitness, trial_fitness):
        """Best individual improvement + rank robustness (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
    ...
    def _compute_reward_variant07(self, fitness, trial_fitness):
        """Log improvement toward target (1 win)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        improvements = fitness - trial_fitness
        n_improved = int(np.sum(improvements > 0))
    ...
    def _compute_reward_variant08(self, fitness, trial_fitness):
        ...
        diversity_ratio = (float(np.std(trial_fitness[np.isfinite(trial_fitness)])) if np.any(np.isfinite(trial_fitness)) else 0.0) / (pop_spread + 1e-10)
    ...
    def _compute_reward_variant10(self, fitness, trial_fitness):
        """Relative improvement prioritizing hard tasks (5 wins)"""
        fitness = np.clip(fitness, -1e50, 1e50)
        trial_fitness = np.clip(trial_fitness, -1e50, 1e50)
        improvements = fitness - trial_fitness
        n_improved = int(np.sum(improvements > 0))
    ...
    def __call__(self, func, stopping_condition):
        ...
        self.current_reward_strategy = self._select_reward_strategy_thompson()
        self.reward_strategy_usage[self.current_reward_strategy] += 1
        ...
        while not stopping_condition():
            ...
            # Compute reward using current adaptive strategy
            reward = self._compute_reward(fitness, trial_fitness)
            # Update performance of the reward strategy that was used
            self._update_reward_strategy_performance(self.current_reward_strategy, reward)
            ...
            # Reward strategy switching with adaptive interval
            if self.generation - self.last_reward_switch_gen >= self.reward_switch_interval:
                ...
                if new_strategy != self.current_reward_strategy:
                    ...
                    self.reward_strategy_usage[new_strategy] += 1
                    # Adjust switch interval based on performance diversity
                    ewma_values = self.reward_strategy_ewma
                    ewma_std = float(np.std(ewma_values)) if len(ewma_values) > 1 else 0.0
                    self.reward_switch_interval = max(5, int(10 - ewma_std * 5))
