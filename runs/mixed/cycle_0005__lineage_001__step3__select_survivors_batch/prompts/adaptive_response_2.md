...
class CulturalMemeticDE:
    ...
    Now also includes adaptive selection of survivor strategies based on benchmark results.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Survivor Selection (Thompson Sampling) ===
        # Based on benchmark results: variant_08 (9 wins), variant_01 (6), original (3), etc.
        self.num_survivor_operators = 6
        self.survivor_names = ['original', 'variant_01', 'variant_05', 'variant_06', 'variant_08', 'variant_10']
        # Beta distribution parameters for Thompson Sampling
        self.survivor_alpha = np.ones(self.num_survivor_operators)
        self.survivor_beta = np.ones(self.num_survivor_operators)
        # Sliding window reward tracking
        self.survivor_reward_window_size = 30
        self.survivor_operator_rewards = [[] for _ in range(self.num_survivor_operators)]
        self.current_survivor_idx = 0
        self.last_survivor_switch = 0
        self.survivor_min_switch_gens = 5
        self.survivor_switches = []

    def _select_operator_thompson(self):
        """Select mutation operator using Thompson Sampling from Beta distributions"""
    ...
    def _update_operator_rewards(self, operator_idx, reward):
        """Update rewards for mutation operator with sliding window"""
    ...
    # === Survivor Selection Operator Methods ===
    def _select_survivor_operator_thompson(self):
        """Select survivor operator using Thompson Sampling from Beta distributions"""
        samples = np.random.beta(self.survivor_alpha, self.survivor_beta)
        selected = int(np.argmax(samples))
        return selected

    def _compute_survivor_reward(self, old_fitness, new_fitness):
        """Compute reward for survivor selection based on population improvement"""
        old_fitness = np.clip(old_fitness, -1e50, 1e50)
        new_fitness = np.clip(new_fitness, -1e50, 1e50)

        # Reward based on best fitness improvement
        old_best = float(np.min(old_fitness))
        new_best = float(np.min(new_fitness))
        best_improvement = old_best - new_best

        # Reward based on average fitness improvement
        avg_improvement = float(np.mean(old_fitness - new_fitness))

        # Reward based on number of improvements
        improvements = old_fitness - new_fitness
        n_improved = int(np.sum(improvements > 0))
        improvement_ratio = n_improved / max(float(len(old_fitness)), 1.0)

        # Combined reward
        reward = 0.5 * best_improvement + 0.3 * avg_improvement + 0.2 * improvement_ratio

        return float(np.clip(reward, -1.0, 1.0))

    def _update_survivor_operator_rewards(self, operator_idx, reward):
        """Update rewards for survivor operator with sliding window"""
        rewards = self.survivor_operator_rewards[operator_idx]
        rewards.append(float(reward))

        if len(rewards) > self.survivor_reward_window_size:
            rewards.pop(0)
        ...
        if len(recent) >= 3:
            ...
            self.survivor_alpha[operator_idx] = 1.0 + success_rate * 5.0 + avg_reward * 2.0
            self.survivor_beta[operator_idx] = 1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0

            self.survivor_alpha[operator_idx] = float(np.clip(self.survivor_alpha[operator_idx], 0.1, 100.0))
            self.survivor_beta[operator_idx] = float(np.clip(self.survivor_beta[operator_idx], 0.1, 100.0))
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize both operator selections
        ...
        while not stopping_condition():
            ...
            # Compute rewards before selection
            mutation_reward = self._compute_reward(fitness, trial_fitness)

            # Apply the selected survivor operator
            population, fitness = self._apply_survivor_operator(
                self.current_survivor_idx, population, fitness, trials, trial_fitness
            )
            fitness = np.clip(fitness, -1e50, 1e50)

            # Compute survivor reward after selection
            survivor_reward = self._compute_survivor_reward(trial_fitness, fitness)
            ...
            # Update operator performances
            self._update_operator_rewards(self.current_operator_idx, mutation_reward)
            self._update_survivor_operator_rewards(self.current_survivor_idx, survivor_reward)

            # Update archive if using variant_01 mutation
            ...
            # Survivor operator switching
            if self.generation - self.last_survivor_switch >= self.survivor_min_switch_gens:
                new_survivor = self._select_survivor_operator_thompson()
                if new_survivor != self.current_survivor_idx:
                    self.current_survivor_idx = new_survivor
                    self.last_survivor_switch = self.generation
                    self.survivor_switches.append((self.generation, new_survivor))
    ...
    def _select_survivors_batch(self, population, fitness, trials, trial_fitness):
        """Legacy method - kept for compatibility. Use _apply_survivor_operator instead."""
    ...
    # === Survivor Selection Operator Implementations ===
    def _apply_survivor_operator(self, operator_idx, population, fitness, trials, trial_fitness):
        """Apply the selected survivor selection operator"""
        if operator_idx == 0:
            return self._survivor_original(population, fitness, trials, trial_fitness)
        elif operator_idx == 1:
            return self._survivor_variant01(population, fitness, trials, trial_fitness)
        elif operator_idx == 2:
            return self._survivor_variant05(population, fitness, trials, trial_fitness)
        elif operator_idx == 3:
            return self._survivor_variant06(population, fitness, trials, trial_fitness)
        elif operator_idx == 4:
            return self._survivor_variant08(population, fitness, trials, trial_fitness)
        else:  # operator_idx == 5
            return self._survivor_variant10(population, fitness, trials, trial_fitness)

    def _survivor_original(self, population, fitness, trials, trial_fitness):
        """Original survivor selection: greedy 1-to-1 competition"""
        better = trial_fitness < fitness
        new_population = np.copy(population)
        new_population[better] = trials[better]
        new_fitness = np.copy(fitness)
    ...
    def _survivor_variant01(self, population, fitness, trials, trial_fitness):
        """Variant 01: μ+λ style selection with argpartition for efficiency"""
    ...
    def _survivor_variant05(self, population, fitness, trials, trial_fitness):
        """Variant 05: Diversity-weighted selection with adaptive diversity weight"""
    ...
    def _survivor_variant06(self, population, fitness, trials, trial_fitness):
        """Variant 06: Boltzmann selection with adaptive temperature and elitism"""
        best_idx = np.argmin(fitness)
    ...
    def _survivor_variant08(self, population, fitness, trials, trial_fitness):
        """Variant 08: Age-based replacement with adaptive diversity injection"""
        NP = len(population)
    ...
    def _survivor_variant10(self, population, fitness, trials, trial_fitness):
        """Variant 10: Boltzmann selection with high temperature for exploration"""
