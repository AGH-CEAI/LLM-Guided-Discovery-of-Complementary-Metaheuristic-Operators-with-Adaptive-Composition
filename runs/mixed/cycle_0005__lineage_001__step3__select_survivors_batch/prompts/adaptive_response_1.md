...
class CulturalMemeticDE:
    """
    Adaptive DE with Thompson Sampling for survivor selection operator.
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.survivor_operator_names = [
            'original', 'variant_01', 'variant_05',
            'variant_06', 'variant_08', 'variant_10'
        ]
        ...
        # Individual ages for variant_08
        self.individual_ages = None

        # Per-operator fitness tracking for reward computation
        self.operator_best_fitness = [np.inf] * self.num_survivor_operators
        self.operator_calls = [0] * self.num_survivor_operators
    ...
    def _compute_reward(self, fitness, trial_fitness, new_fitness):
        """Compute reward based on improvement quality after selection"""
        # Guard against invalid values
        fitness = np.clip(np.asarray(fitness, dtype=np.float64), -1e50, 1e50)
        new_fitness = np.clip(np.asarray(new_fitness, dtype=np.float64), -1e50, 1e50)

        # Compute improvement: lower fitness is better
    ...
    def _update_operator_rewards(self, operator_idx, reward):
        ...
        if len(recent) >= 3:
            ...
            alpha_update = 1.0 + success_rate * 5.0 + max(-0.5, min(avg_reward, 1.0)) * 2.0
            beta_update = 1.0 + (1.0 - success_rate) * 5.0 - max(-0.5, min(avg_reward, 1.0)) * 2.0
    ...
    def __call__(self, func, stopping_condition):
        ...
        self.current_operator_idx = int(self._select_operator_thompson())
        ...
        while not stopping_condition():
            ...
            # Use adaptive operator selection for survivor selection
            if self.current_operator_idx == 0:
                population, fitness = self._select_survivors_original(population, fitness, trials, trial_fitness)
            elif self.current_operator_idx == 1:
                population, fitness = self._select_survivors_variant01(population, fitness, trials, trial_fitness)
            elif self.current_operator_idx == 2:
                population, fitness = self._select_survivors_variant05(population, fitness, trials, trial_fitness)
            elif self.current_operator_idx == 3:
                population, fitness = self._select_survivors_variant06(population, fitness, trials, trial_fitness)
            elif self.current_operator_idx == 4:
                population, fitness = self._select_survivors_variant08(population, fitness, trials, trial_fitness)
            else:
                population, fitness = self._select_survivors_variant10(population, fitness, trials, trial_fitness)

            fitness = np.clip(fitness, -1e50, 1e50)

            # Compute reward after selection
            reward = self._compute_reward(fitness, trial_fitness, fitness)
            ...
            if self.current_operator_idx == 1:
            ...
            if self.generation - self.last_operator_switch_gen >= self.operator_min_switch_gens:
                new_op = int(self._select_operator_thompson())
    ...
    # === Original _select_survivors_batch ===
    def _select_survivors_original(self, population, fitness, trials, trial_fitness):
        """Original survivor selection: greedy 1-to-1 replacement"""
    ...
    # === Variant 01 _select_survivors_batch (6 wins) ===
    def _select_survivors_variant01(self, population, fitness, trials, trial_fitness):
        ...
        np_pop = min(int(population.shape[0]), int(trials.shape[0]))
    ...
    # === Variant 05 _select_survivors_batch (1 win) ===
    def _select_survivors_variant05(self, population, fitness, trials, trial_fitness):
        ...
        div_contribs = np.array([diversity_contribution(candidates_pop[i], candidates_pop)
                                 for i in range(len(candidates_pop))], dtype=np.float64)

        div_min, div_max = float(np.min(div_contribs)), float(np.max(div_contribs))
        if div_max > div_min + 1e-10:
            div_normalized = (div_contribs - div_min) / (div_max - div_min)
        else:
            div_normalized = np.ones_like(div_contribs) * 0.5

        fit_min, fit_max = float(np.min(candidates_fit)), float(np.max(candidates_fit))
        ...
        combined_score = (1.0 - diversity_weight) * np.asarray(fitness_score, dtype=np.float64) + \
                         diversity_weight * np.asarray(div_normalized, dtype=np.float64)
    ...
    # === Variant 06 _select_survivors_batch (1 win) ===
    def _select_survivors_variant06(self, population, fitness, trials, trial_fitness):
    ...
    # === Variant 08 _select_survivors_batch (9 wins) ===
    def _select_survivors_variant08(self, population, fitness, trials, trial_fitness):
        """Variant 08: Age-based replacement with adaptive diversity injection"""
        NP = len(population)

        if self.individual_ages is None or len(self.individual_ages) != NP:
            self.individual_ages = np.zeros(NP, dtype=np.float64)

        self.individual_ages += 1.0
    ...
    # === Variant 10 _select_survivors_batch (1 win) ===
    def _select_survivors_variant10(self, population, fitness, trials, trial_fitness):
        """Variant 10: Boltzmann selection with high temperature"""
    ...
    def _reinitialize_population(self, population, fitness):
        ...
        # Reset individual ages for variant_08
        if self.individual_ages is not None:
            self.individual_ages = np.zeros(len(population), dtype=np.float64)
```