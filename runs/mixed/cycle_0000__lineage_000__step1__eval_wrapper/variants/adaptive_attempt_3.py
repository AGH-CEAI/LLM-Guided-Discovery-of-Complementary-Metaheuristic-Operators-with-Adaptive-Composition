...
class AdaptiveQuantumRingSwarmOptimizer:
    """
    A PSO-based optimizer with dynamic ring topology, quantum tunneling mutation,
    and adaptive evaluation wrapper selection using Thompson Sampling.
    ...
    - Adaptive operator selection via Thompson Sampling for _eval_wrapper strategies
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.alpha_prior = 1.0  # Beta prior alpha
        self.beta_prior = 1.0   # Beta prior beta
        self.success_rewards = np.ones(self.num_operators, dtype=np.float64) * self.alpha_prior
        self.failure_rewards = np.ones(self.num_operators, dtype=np.float64) * self.beta_prior
        self.operator_selection_counts = np.zeros(self.num_operators, dtype=np.int32)
        self.reward_window_size = kwargs.get('reward_window_size', 50)
    ...
    def _thompson_sampling_select(self):
        """Select operator using Thompson Sampling with Beta distribution."""
        samples = np.array([
            np.random.beta(float(self.success_rewards[i]), float(self.failure_rewards[i]))
    ...
    def _update_operator_rewards(self, operator_idx, reward):
        """Update reward history for an operator with sliding window."""
        reward = float(reward)
    ...
    def _compute_operator_quality(self, operator_idx):
        """Compute quality score for an operator based on recent rewards."""
        rewards = self.operator_rewards[operator_idx]
        if len(rewards) == 0:
            return 0.5
        return float(np.mean(rewards))
    
    def _record_operator_outcome(self, operator_idx, improvement):
        """Record the outcome of an operator selection for Thompson Sampling."""
        improvement = float(improvement)
        self._update_operator_rewards(operator_idx, improvement)
        if improvement > 0:
            self.success_rewards[operator_idx] += max(0.1, improvement)
        else:
            self.failure_rewards[operator_idx] += 0.1
        self.success_rewards[operator_idx] = np.clip(self.success_rewards[operator_idx], 0.1, 1e6)
        self.failure_rewards[operator_idx] = np.clip(self.failure_rewards[operator_idx], 0.1, 1e6)
    
    def _eval_wrapper_operator_0(self, x):
        """Original: Direct function call."""
        x = np.atleast_2d(x)
        result = self._user_func(x)
    ...
    def _eval_wrapper_operator_2(self, x):
        ...
        if np.isfinite(f0) and f0 < 1e6:
            return f0 if np.ndim(f0) > 0 else np.array([f0])
        
        grad = np.zeros_like(x)
        f0_scalar = float(f0) if np.ndim(f0) == 0 else float(f0[0])
        
        for d in range(x.shape[1]):
            x_plus = x.copy()
            x_plus[:, d] += eps
            f_plus = self._user_func(x_plus)
            f_plus_scalar = float(f_plus) if np.ndim(f_plus) == 0 else float(f_plus[0])
            grad[:, d] = (f_plus_scalar - f0_scalar) / (eps + 1e-12)
        ...
        if grad_norm > 1e-6:
            ...
            f_new_scalar = float(f_new) if np.ndim(f_new) == 0 else float(f_new[0])
            if np.isfinite(f_new) and f_new_scalar < f0_scalar:
                return f_new if np.ndim(f_new) > 0 else np.array([f_new])
        
        if np.random.random() < 0.3:
            x_random = np.random.uniform(self.lower, self.upper, size=x.shape)
            f_random = self._user_func(x_random)
            f_random_scalar = float(f_random) if np.ndim(f_random) == 0 else float(f_random[0])
            if np.isfinite(f_random) and f_random_scalar < f0_scalar:
                return f_random if np.ndim(f_random) > 0 else np.array([f_random])
        
        x_clipped = np.clip(x, self.lower, self.upper)
        return self._user_func(x_clipped)
    
    def _eval_wrapper(self, x):
        """Adaptive evaluation wrapper that selects best strategy using Thompson Sampling."""
        operator = self._thompson_sampling_select()
        self.current_operator = operator
        
        if operator == 0:
            result = self._eval_wrapper_operator_0(x)
        elif operator == 1:
            result = self._eval_wrapper_operator_1(x)
        else:
            result = self._eval_wrapper_operator_2(x)
    ...
    def _update_global_best_batch(self):
        ...
        if self.personal_best_fit[best_idx] < self.global_best_fit:
            improvement = float(self.global_best_fit) - float(self.personal_best_fit[best_idx])
            self._record_operator_outcome(self.current_operator, improvement)
    ...
    def _record_neutral_outcome(self):
        """Record a neutral outcome when no improvement occurs."""
        neutral_reward = 0.0
        self._update_operator_rewards(self.current_operator, neutral_reward)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if generation > 0:
                if self.global_best_fit >= last_recorded_fitness - 1e-12:
                    self._record_neutral_outcome()
                last_recorded_fitness = self.global_best_fit
        ...
        final_fitness = float(self.global_best_fit) if self.global_best_fit is not None else 1e30
        final_fitness = np.clip(final_fitness, -1e30, 1e30)
        
        if not np.isfinite(final_fitness):
            final_fitness = 1e30
        
        return final_fitness, self.global_best
