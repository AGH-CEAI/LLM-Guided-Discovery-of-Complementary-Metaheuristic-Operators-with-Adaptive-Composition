...
class QuantumRingSwarmOptimizer:
    ...
    Now with adaptive eval_wrapper selection using Thompson Sampling Multi-Armed Bandit.
    ...
    - Adaptive operator selection for eval_wrapper using Thompson Sampling
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_successes = [1.0] * self.num_operators
        self.operator_failures = [1.0] * self.num_operators
    ...
    def _adaptive_operator_select(self):
        """Select operator using Thompson Sampling with sliding window credit assignment."""
        for i in range(self.num_operators):
            if len(self.operator_history[i]) >= 10:
                alpha = max(1.0, sum(self.operator_history[i]))
                beta = max(1.0, len(self.operator_history[i]) - alpha + 1.0)
            else:
                alpha = self.operator_successes[i]
                beta = self.operator_failures[i]
            
            sample = np.random.beta(float(alpha), float(beta))
            
            if i == 0 or sample > self.best_sample:
                self.best_sample = sample
                self.current_operator = i
        
    def _compute_reward(self, result):
        """Compute reward for operator based on result quality."""
        result = float(result)
    ...
    def _update_operator_feedback(self, result):
        """Update operator performance based on evaluation result."""
        reward = self._compute_reward(result)
        self.operator_history[self.current_operator].append(reward)
        if len(self.operator_history[self.current_operator]) > self.history_window:
            self.operator_history[self.current_operator].pop(0)
        self.last_result = float(result)
        
    def _eval_wrapper(self, x):
        """Adaptive eval_wrapper that switches between strategies based on Thompson Sampling."""
        x = np.atleast_2d(x)
        ...
        if self.current_operator == 0:
            ...
            if result.ndim > 0:
                ...
                result = result[0] if len(result) > 0 else 1e30
            if not np.isfinite(result):
                result = 1e30
            self._update_operator_feedback(result)
            return float(result)
            ...
            f0 = self._user_func(x_arr)
            if f0.ndim > 0:
                f0 = np.atleast_1d(f0)[0]
            f0 = float(f0)
            
            if np.isfinite(f0) and f0 < 1e6:
                self._update_operator_feedback(f0)
                return f0
            
            eps = 1e-5
            grad = np.zeros_like(x_arr)
            for d in range(x_arr.shape[1]):
                x_plus = x_arr.copy()
                x_plus[:, d] += eps
                f_plus = self._user_func(x_plus)
                if f_plus.ndim > 0:
                    f_plus = np.atleast_1d(f_plus)[0]
                grad[:, d] = (float(f_plus) - f0) / (eps + 1e-12)
            
            grad_norm = np.linalg.norm(grad) + 1e-12
            if grad_norm > 1e-6:
                step_size = min(1.0, grad_norm * 0.1)
                x_new = x_arr - grad / grad_norm * step_size
                x_new = np.clip(x_new, self.lower, self.upper)
                f_new = self._user_func(x_new)
                if f_new.ndim > 0:
                    f_new = np.atleast_1d(f_new)[0]
                f_new = float(f_new)
                if np.isfinite(f_new) and f_new < f0:
                    self._update_operator_feedback(f_new)
                    return f_new
            
            if np.random.random() < 0.3:
                x_random = np.random.uniform(self.lower, self.upper, size=x_arr.shape)
                f_random = self._user_func(x_random)
                if f_random.ndim > 0:
                    f_random = np.atleast_1d(f_random)[0]
                f_random = float(f_random)
                if np.isfinite(f_random) and f_random < f0:
                    self._update_operator_feedback(f_random)
                    return f_random
            
            x_clipped = np.clip(x_arr, self.lower, self.upper)
            result = self._user_func(x_clipped)
            if result.ndim > 0:
                result = np.atleast_1d(result)
                result = result[0] if len(result) > 0 else 1e30
    ...
    def _quantum_tunnel_batch(self):
        ...
        for i in np.where(stagnant_mask)[0]:
            ...
            fit = self._eval_wrapper(trial)
            tunnel_fitness[i] = fit
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            self.best_sample = 0.0
            self._adaptive_operator_select()
