...
class CulturalMemeticDE:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Bound Strategy Selection (NEW) ===
        self.num_bound_strategies = 8
        self.bound_strategy_names = [
            'clip_only',           # 0: Simple clip (original)
            'variant_01',          # 1: Reflection with double reflection
            'variant_02',          # 2: Reflection/wrap based on stagnation
            'variant_03',          # 3: Periodic reflection with modulo
            'variant_04',          # 4: Adaptive damping based on stagnation
            'variant_05',          # 5: Partial reflection with 0.7 factor
            'variant_06',          # 6: Simple reflection
            'variant_07',          # 7: Probabilistic reflection
        ]
        # Beta distribution parameters for bound strategy Thompson Sampling
        self.bound_alpha = np.ones(self.num_bound_strategies)
        self.bound_beta = np.ones(self.num_bound_strategies)
        # Sliding window for bound strategy rewards
        self.bound_rewards = [[] for _ in range(self.num_bound_strategies)]
        self.current_bound_strategy = 0
        self.last_bound_switch_gen = 0
        self.bound_min_switch_gens = 3
        self.bound_switches = []
        # Track bound strategy performance
        self.bound_strategy_successes = np.zeros(self.num_bound_strategies)
        self.bound_strategy_failures = np.zeros(self.num_bound_strategies)

    def _select_bound_strategy_thompson(self):
        """Select bound strategy using Thompson Sampling from Beta distributions"""
        samples = np.random.beta(self.bound_alpha, self.bound_beta)
        selected = int(np.argmax(samples))
        return selected

    def _update_bound_strategy_rewards(self, strategy_idx, reward):
        """Update rewards for bound strategy with sliding window"""
        rewards = self.bound_rewards[strategy_idx]
        ...
        if len(recent) >= 3:
            ...
            self.bound_alpha[strategy_idx] = float(np.clip(1.0 + success_rate * 5.0 + avg_reward * 2.0, 0.1, 100.0))
            self.bound_beta[strategy_idx] = float(np.clip(1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0, 0.1, 100.0))

    def _compute_bound_reward(self, trial_fitness, original_fitness):
        """Compute reward for bound strategy based on boundary handling quality"""
        # Count how many individuals improved
        improvements = np.sum(trial_fitness < original_fitness)
        total_improvement = float(np.sum(original_fitness - trial_fitness))
        
        # Check for any NaN or Inf in trial fitness (bad boundary handling)
        has_invalid = np.any(~np.isfinite(trial_fitness))
        ...
        # Reward based on improvement quality
        reward = float(np.clip(total_improvement / max(float(improvements), 1.0), -1.0, 1.0))
        return reward
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize bound strategy selection
        self._select_bound_strategy_thompson()
        ...
        while not stopping_condition():
            ...
            # Store pre-bound fitness for reward computation
            trials_pre_bound = trials.copy()
            
            # Apply adaptive bound strategy
            ...
            # Compute reward for bound strategy selection
            bound_reward = self._compute_bound_reward(trial_fitness, fitness)
            ...
            # Update bound strategy performance
            self._update_bound_strategy_rewards(self.current_bound_strategy, bound_reward)
            ...
            # Bound strategy switching with minimum generations between switches
            if self.generation - self.last_bound_switch_gen >= self.bound_min_switch_gens:
                new_bound = self._select_bound_strategy_thompson()
                if new_bound != self.current_bound_strategy:
                    self.current_bound_strategy = new_bound
                    self.last_bound_switch_gen = self.generation
                    self.bound_switches.append((self.generation, new_bound))
    ...
    def _clip_to_bounds_batch(self, population):
        """Dispatch to selected bound strategy"""
        if self.current_bound_strategy == 0:
            return self._clip_strategy_0_clip_only(population)
        elif self.current_bound_strategy == 1:
            return self._clip_strategy_1_variant01(population)
        elif self.current_bound_strategy == 2:
            return self._clip_strategy_2_variant02(population)
        elif self.current_bound_strategy == 3:
            return self._clip_strategy_3_variant03(population)
        elif self.current_bound_strategy == 4:
            return self._clip_strategy_4_variant04(population)
        elif self.current_bound_strategy == 5:
            return self._clip_strategy_5_variant05(population)
        elif self.current_bound_strategy == 6:
            return self._clip_strategy_6_variant06(population)
        else:
            return self._clip_strategy_7_variant07(population)

    def _clip_strategy_0_clip_only(self, population):
        """Strategy 0: Simple clip (original)"""
        return np.clip(population, self.lower, self.upper)

    def _clip_strategy_1_variant01(self, population):
        """Strategy 1: Reflection with double reflection for extreme overshoots"""
    ...
    def _clip_strategy_2_variant02(self, population):
        """Strategy 2: Reflection mode (stagnation < 10) or wrap mode (stagnation >= 10)"""
        lower = self.lower
        upper = self.upper
    ...
    def _clip_strategy_3_variant03(self, population):
        """Strategy 3: Periodic reflection with modulo"""
    ...
    def _clip_strategy_4_variant04(self, population):
        """Strategy 4: Adaptive damping based on stagnation"""
    ...
    def _clip_strategy_5_variant05(self, population):
        """Strategy 5: Partial reflection with 0.7 factor"""
        reflected = np.copy(population)
        # Reflect values above upper bound
    ...
    def _clip_strategy_6_variant06(self, population):
        """Strategy 6: Simple reflection (most robust)"""
        result = np.copy(population)
    ...
    def _clip_strategy_7_variant07(self, population):
        """Strategy 7: Probabilistic reflection with adaptive probability"""
        result = np.copy(population)
