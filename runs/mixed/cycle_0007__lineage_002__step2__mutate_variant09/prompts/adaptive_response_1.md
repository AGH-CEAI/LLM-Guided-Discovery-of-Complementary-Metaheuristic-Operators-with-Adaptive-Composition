...
class CulturalMemeticDE:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Operator Selection (Thompson Sampling for variant_09 strategies) ===
        self.num_operators = 6
        self.operator_names = [
            'original', 'variant_04', 'variant_09', 'variant_08', 'variant_02', 'variant_10'
        ]
        # Beta distribution parameters for Thompson Sampling
        self.operator_alpha = np.ones(self.num_operators)
        self.operator_beta = np.ones(self.num_operators)
        # Initialize prior based on benchmark wins (variant_04 has most wins)
        prior_boost = np.array([2.0, 10.0, 2.0, 1.0, 1.0, 1.0])  # Based on win counts
        self.operator_alpha = self.operator_alpha + prior_boost * 0.5
        # Sliding window reward tracking
        self.reward_window_size = 30
        self.operator_rewards = [[] for _ in range(self.num_operators)]
        self.current_operator_idx = 1  # Start with variant_04 (most wins)
        ...
        self.operator_success_counts = np.zeros(self.num_operators, dtype=int)
        self._generation_rewards = np.zeros(self.num_operators)

        # Archive for variant_01-style operations
        self.archive = None
    ...
    def _select_operator_thompson(self):
        ...
        alpha_safe = np.clip(self.operator_alpha, 0.1, 1000.0)
        beta_safe = np.clip(self.operator_beta, 0.1, 1000.0)
        samples = np.random.beta(alpha_safe, beta_safe)
    ...
    def _compute_reward(self, fitness, trial_fitness):
        ...
        total_improvement = float(np.sum(positive_improvements)) if len(positive_improvements) > 0 else 0.0
        n_improved = int(len(positive_improvements))
    ...
    def _update_operator_rewards(self, operator_idx, reward):
        """Update rewards for operator with sliding window and UCB bonus"""
        ...
        if len(recent) >= 3:
            ...
            # Update alpha and beta with confidence-aware adjustment
        ...
        # Update usage and success tracking
        self.operator_usage_counts[operator_idx] += 1
        if reward > 0:
            self.operator_success_counts[operator_idx] += 1
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize operator selection (start with best known variant)
        self.current_operator_idx = 1  # variant_04
    ...
    def _mutate_current_to_pbest_with_culture(self, population, fitness):
        ...
        if self.current_operator_idx == 0:
            ...
            return self._mutate_variant09_opposition(population, fitness)
    ...
    def _mutate_variant04(self, population, fitness):
        """Variant 04: Multi-Donor Composite Mutation with Diversity-Weighted Selection (10 wins)"""
        sorted_idx = np.argsort(fitness)
        n_best = max(1, int(self.NP * self.p_best_rate))
        best_indices = sorted_idx[:n_best]
        p_best_idx = best_indices[np.random.randint(0, n_best, size=self.NP)]
    ...
    def _mutate_variant09_opposition(self, population, fitness):
        """Variant 09: Opposition-Based DE with centroid reflection and restart injection (2 wins)"""
        sorted_idx = np.argsort(fitness)
        n_best = max(1, int(self.NP * self.p_best_rate))
        best_indices = sorted_idx[:n_best]
        p_best_idx = best_indices[np.random.randint(0, n_best, size=self.NP)]
    ...
    def _mutate_variant08(self, population, fitness):
        """Variant 08: Centroid Repulsion — escape local optima by mutating away from population center (1 win)"""
        sorted_idx = np.argsort(fitness)
        n_best = max(1, int(self.NP * self.p_best_rate))
        best_indices = sorted_idx[:n_best]
        p_best_idx = best_indices[np.random.randint(0, n_best, size=self.NP)]
    ...
    def _mutate_variant02(self, population, fitness):
        """Variant 02: Mean-Reversion Opposition DE — escape local optima via centroid pull and opposition (1 win)"""
        sorted_idx = np.argsort(fitness)
        n_best = max(1, int(self.NP * self.p_best_rate))
        best_indices = sorted_idx[:n_best]
        p_best_idx = best_indices[np.random.randint(0, n_best, size=self.NP)]
    ...
    def _mutate_variant10(self, population, fitness):
        """Variant 10: Opposition-guided mutation with restart and niching (1 win)"""
