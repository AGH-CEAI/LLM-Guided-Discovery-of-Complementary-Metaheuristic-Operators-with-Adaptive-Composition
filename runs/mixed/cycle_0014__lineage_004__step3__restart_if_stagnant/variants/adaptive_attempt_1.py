...
class CulturalDEWithAdaptiveArchive:
    ...
    - Diversity-aware restart mechanism with Thompson Sampling for strategy selection
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.n_restart_strategies = 7
        self.restart_successes = np.zeros(self.n_restart_strategies, dtype=np.float64)
        self.restart_failures = np.zeros(self.n_restart_strategies, dtype=np.float64)
        self.last_restart_fitness = None
        self.last_restart_strategy = -1
        self.last_restart_best_fitness = None
        self.restart_generation_counter = 0
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if self._compute_diversity(population) < self.diversity_threshold:
                f_best_before = float(np.min(fitness))
                population = self._restart_if_stagnant(population, fitness, x_best)
                f_best_after = float(np.min(fitness))
                
                # Update Thompson Sampling rewards based on improvement
                if self.last_restart_strategy >= 0:
                    improvement = f_best_before - f_best_after
                    if improvement > 1e-10:
                        self.restart_successes[self.last_restart_strategy] += float(improvement)
                    else:
                        self.restart_failures[self.last_restart_strategy] += 1.0
    ...
    def _select_restart_strategy_thompson(self):
        """Thompson Sampling for operator selection with optimistic priors."""
        samples = np.zeros(self.n_restart_strategies, dtype=np.float64)
        for i in range(self.n_restart_strategies):
            alpha = float(self.restart_successes[i]) + 1.0
            beta = float(self.restart_failures[i]) + 1.0
            samples[i] = np.random.beta(alpha, beta)
        return int(np.argmax(samples))
    
    def _restart_if_stagnant(self, population, fitness, x_best):
        """Apply adaptive restart strategy selection using Thompson Sampling."""
        NP, dim = population.shape
        new_pop = population.copy()
        
        # Select strategy using Thompson Sampling
        selected_strategy = self._select_restart_strategy_thompson()
        self.last_restart_strategy = selected_strategy
        self.restart_generation_counter += 1
        
        # Apply selected strategy
        if selected_strategy == 0:
            # Strategy from variant_01_idea_0.py (1 win) - Random replacement of worst half
            best_idx = np.argmin(fitness)
            n_replace = NP // 2
            worst_indices = np.argpartition(fitness, n_replace)[:n_replace]
            for idx in worst_indices:
                new_pop[idx] = np.random.uniform(self.lower, self.upper, dim)
            new_pop[best_idx] = x_best.copy()
            
        elif selected_strategy == 1:
            # Strategy from variant_03_idea_0.py (1 win) - Mixed LHS, opposition, random
            ...
            # Strategy from variant_04_idea_0.py (2 wins) - Random + Gaussian perturbation
            random_count = NP // 2
            for i in range(random_count):
                new_pop[i] = np.random.uniform(self.lower, self.upper, dim)
            
            sigma = (self.upper - self.lower) * 0.3
            for i in range(random_count, NP):
                new_pop[i] = x_best + np.random.normal(0, sigma, dim)
                
        elif selected_strategy == 3:
            # Strategy from variant_06_idea_0.py (1 win) - Boundary-focused exploration
            ...
            # Strategy from variant_07_idea_0.py (1 win) - Opposition, Halton, Cauchy
            centroid = np.mean(population, axis=0)
            ...
            for i, idx in enumerate(worst_indices):
                ...
                if strategy == 0:
                    ...
                    halton = np.array([float((i + 1) % (dim ** k)) / float(dim ** k) for k in range(1, dim + 1)])
            ...
            # Strategy from variant_09_idea_0.py (2 wins) - Opposition, adaptive, Latin
        ...
        else:  # selected_strategy == 6
            # Strategy from variant_08_idea_0.py (4 wins) - Error-scaled adaptive perturbation
            ...
            for idx in worst_indices:
                scale_factor = perturbation_range * np.random.uniform(0.5, 1.5, dim)
                new_pop[idx] = x_best + np.random.uniform(-scale_factor, scale_factor, dim)
