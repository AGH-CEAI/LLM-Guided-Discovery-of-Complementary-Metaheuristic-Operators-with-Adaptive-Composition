...
class CulturalMemeticDE:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Local Search Selection (Thompson Sampling) ===
        self.num_ls_operators = 8
        self.ls_operator_names = [
            'ls_original', 'ls_variant02', 'ls_variant03', 'ls_variant04',
            'ls_variant05', 'ls_variant06', 'ls_variant07', 'ls_variant10'
        ]
        # Beta distribution parameters for Thompson Sampling on local search
        self.ls_operator_alpha = np.ones(self.num_ls_operators)
        self.ls_operator_beta = np.ones(self.num_ls_operators)
        ...
        self.ls_switches = []
        # Track improvements per LS operator
        self.ls_improvements = [0.0] * self.num_ls_operators
        self.ls_calls = [0] * self.num_ls_operators

    def _select_ls_operator_thompson(self):
        """Select local search operator using Thompson Sampling from Beta distributions"""
        samples = np.random.beta(self.ls_operator_alpha, self.ls_operator_beta)
        selected = int(np.argmax(samples))
        return selected

    def _compute_ls_reward(self, improvement, current_fitness):
        """Compute reward for local search based on improvement quality"""
        if improvement <= 0.0:
            return -0.1
        
        # Normalize by current fitness magnitude (log-scaled like mutation rewards)
        relative_imp = improvement / (abs(current_fitness) + 1e-10)
        ...
        return float(np.clip(log_reward, -0.5, 1.5))
    ...
    def _update_ls_operator_rewards(self, operator_idx, reward):
        ...
        if len(recent) >= 2:
            ...
            success_rate = successes / float(len(recent))
            avg_reward = float(np.mean(recent))
            
            # Update alpha and beta
            self.ls_operator_alpha[operator_idx] = float(np.clip(1.0 + success_rate * 3.0 + avg_reward * 1.5, 0.1, 100.0))
            self.ls_operator_beta[operator_idx] = float(np.clip(1.0 + (1.0 - success_rate) * 3.0 - avg_reward * 1.5, 0.1, 100.0))
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if self.generation % self.local_search_interval == 0:
                # Select local search operator using Thompson Sampling
                ...
                fitness_before_ls = self.best_fitness
                self._apply_adaptive_local_search(func)
                improvement = fitness_before_ls - self.best_fitness
                
                # Update local search operator rewards
                ls_reward = self._compute_ls_reward(improvement, fitness_before_ls)
                self._update_ls_operator_rewards(self.current_ls_operator_idx, ls_reward)
                self.ls_improvements[self.current_ls_operator_idx] += max(0.0, improvement)
                self.ls_calls[self.current_ls_operator_idx] += 1
    ...
    def _apply_adaptive_local_search(self, func):
        """Dispatch to selected local search strategy"""
    ...
    def _ls_variant03(self, func):
        """Variant 03: Dense set of test directions with extrapolation"""
        if self.best_solution is None:
            return
    ...
    def _ls_variant04(self, func):
        """Variant 04: Multi-scale restart with memory"""
        if self.best_solution is None:
            return
    ...
    def _ls_variant06(self, func):
        """Variant 06: scipy.optimize.minimize with L-BFGS-B (multi-scale restart)"""
        if self.best_solution is None:
            return
    ...
    def _ls_variant07(self, func):
        """Variant 07: Many random directions with adaptive bias"""
        if self.best_solution is None:
            return
    ...
    def _ls_variant10(self, func):
        """Variant 10: Golden section search with direction history and PCA"""
        if self.best_solution is None:
            return
