...
class CulturalMemeticDE:
    ...
    Now includes ADAPTIVE LOCAL SEARCH that learns which local search variant works best.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.ls_operator_names = ['variant_02', 'variant_03', 'variant_04', 'variant_05',
                                   'variant_06', 'variant_07', 'variant_10', 'original']
        # Beta distribution parameters for Thompson Sampling on local search
        self.ls_alpha = np.ones(self.num_ls_operators)
        self.ls_beta = np.ones(self.num_ls_operators)
        # Sliding window reward tracking for local search
        self.ls_reward_window_size = 15
        ...
        self.ls_switches = []
        # Track which local search operators have been tried
        self.ls_tried_operators = set()
        self.ls_success_history = {}  # Maps operator_idx to list of (improvement, success) tuples
        self.last_ls_result = None  # Track last local search improvement
    ...
    def _compute_ls_reward(self, initial_fitness, final_fitness):
        """Compute reward for local search operator based on improvement achieved"""
        ...
        return float(np.clip(log_reward, -0.5, 2.0))
    ...
    def _update_ls_operator_rewards(self, operator_idx, reward):
        ...
        if len(recent) >= 2:
            ...
            self.ls_alpha[operator_idx] = 1.0 + success_rate * 4.0 + avg_reward * 1.5
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            if self.generation % self.local_search_interval == 0:
                # Adaptive local search with operator selection
                initial_fitness = self.best_fitness
                ...
                if final_fitness < initial_fitness:
                    ls_reward = self._compute_ls_reward(initial_fitness, final_fitness)
                    self._update_ls_operator_rewards(self.current_ls_operator_idx, ls_reward)
                    self.last_ls_result = (initial_fitness - final_fitness, True)
                else:
                    self._update_ls_operator_rewards(self.current_ls_operator_idx, -0.2)
                    self.last_ls_result = (0.0, False)
                
                # Track operator usage
                self.ls_tried_operators.add(self.current_ls_operator_idx)
                if self.current_ls_operator_idx not in self.ls_success_history:
                    self.ls_success_history[self.current_ls_operator_idx] = []
                self.ls_success_history[self.current_ls_operator_idx].append(self.last_ls_result)
                if len(self.ls_success_history[self.current_ls_operator_idx]) > 20:
                    self.ls_success_history[self.current_ls_operator_idx].pop(0)
    ...
    def _apply_adaptive_local_search(self, func):
        """Dispatch to selected local search strategy based on Thompson Sampling"""
        if self.best_solution is None:
            return

        # Select operator using Thompson Sampling
        selected_op = self._select_ls_operator_thompson()
        self.current_ls_operator_idx = selected_op

        # Dispatch to the appropriate local search strategy
        if selected_op == 0:
            self._ls_variant_02(func)
        elif selected_op == 1:
            self._ls_variant_03(func)
        elif selected_op == 2:
            self._ls_variant_04(func)
        elif selected_op == 3:
            self._ls_variant_05(func)
        elif selected_op == 4:
            self._ls_variant_06(func)
        elif selected_op == 5:
            self._ls_variant_07(func)
        elif selected_op == 6:
            self._ls_variant_10(func)
        else:
            self._ls_original(func)

    # =========================================================================
    # LOCAL SEARCH VARIANT IMPLEMENTATIONS
    # =========================================================================

    def _ls_variant_02(self, func):
        ...
        for iteration in range(max_iter):
            ...
            if not improved:
                ...
                step = float(self._ls_dim_scales[dim])
    ...
    def _ls_variant_03(self, func):
        """Variant 03: Dense random direction sampling with extrapolation"""
        if self.best_solution is None:
            return
    ...
    def _ls_variant_05(self, func):
        ...
        if hasattr(self, 'cultural_memory') and len(self.cultural_memory) > 0:
            ...
            if float(np.sum(weights)) > 1e-10:
                ...
                norm_cd = np.linalg.norm(cultural_dir) + 1e-10
                if norm_cd > 1e-10:
    ...
    def _ls_variant_06(self, func):
        """Variant 06: Multi-scale L-BFGS-B with restarts"""
        if self.best_solution is None:
            return
        ...
        try:
            ...
            # Fallback to simple local search if scipy not available
            self._ls_original(func)
            return
        ...
        scales = [max(float(self.local_radius), 1.0), 0.5, 0.1]
    ...
    def _ls_variant_07(self, func):
        """Variant 07: Many random directions with antipodal sampling"""
        if self.best_solution is None:
            return
        ...
        for _ in range(self.local_max_iter):
            ...
            dir_norm = np.linalg.norm(best_dir) + 1e-10
            if best_local_fitness < current_fitness:
    ...
    def _ls_variant_10(self, func):
        """Variant 10: Golden section search with PCA-based direction learning"""
        if self.best_solution is None:
            return
        ...
        if improved or current_fitness < self.best_fitness:
            ...
            if self._ls_history_count >= 5:
                ...
                try:
                    ...
                    eigvals = eigvals[idx]
                    eigvecs = eigvecs[:, idx]
                    if eigvals[0] > 1e-6 and eigvals[0] / (float(np.sum(eigvals)) + 1e-10) > 0.3:
    ...
    def _ls_original(self, func):
        """Original local search: simple random direction search"""
        if self.best_solution is None:
            return
