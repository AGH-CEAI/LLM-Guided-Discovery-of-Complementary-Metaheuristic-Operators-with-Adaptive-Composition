...
class CulturalMemeticDE:
    """
    Adaptive DE with Multi-Armed Bandit for parameter adaptation strategy selection.
    Automatically learns which _adapt_parameters_batch strategy works best during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Parameter Adaptation Strategy Selection (NEW) ===
        ...
        self.adapt_switches = []
        self._adapt_stagnation_count = 0
        self._adapt_improvement_streak = 0
    ...
    def _update_adapt_strategy_rewards(self, strategy_idx, reward):
        ...
        if len(recent) >= 3:
            ...
            self.adapt_alpha[strategy_idx] = 1.0 + success_rate * 5.0 + avg_reward * 2.0
            self.adapt_beta[strategy_idx] = 1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0

            self.adapt_alpha[strategy_idx] = float(np.clip(self.adapt_alpha[strategy_idx], 0.1, 100.0))
            self.adapt_beta[strategy_idx] = float(np.clip(self.adapt_beta[strategy_idx], 0.1, 100.0))
    ...
    def __call__(self, func, stopping_condition):
        ...
        self._prev_best_fitness = self.best_fitness

        # Initialize operator selection
        self._select_operator_thompson()
        # Initialize parameter adaptation strategy selection
        ...
        while not stopping_condition():
            ...
            # Compute reward for parameter adaptation strategy selection
            adapt_reward = self.best_fitness - current_best
            if adapt_reward > 0:
                self._adapt_improvement_streak += 1
                self._adapt_stagnation_count = 0
            else:
                self._adapt_stagnation_count += 1
                self._adapt_improvement_streak = 0
            ...
            # Update parameter adaptation strategy performance
            self._update_adapt_strategy_rewards(self.current_adapt_strategy, float(adapt_reward))
            ...
            # Use adaptive parameter adaptation strategy selection
            ...
            # Parameter adaptation strategy switching
    ...
    def _adapt_parameters_batch(self, population, fitness, mutants, trials):
        """Execute the selected parameter adaptation strategy"""
        if self.current_adapt_strategy == 0:
            self._adapt_original(population, fitness, mutants, trials)
        elif self.current_adapt_strategy == 1:
            self._adapt_variant05(population, fitness, mutants, trials)
        else:
            self._adapt_variant07(population, fitness, mutants, trials)
    ...
    def _adapt_variant05(self, population, fitness, mutants, trials):
        """Variant 05: EMA-based adaptation with stagnation handling"""
    ...
    def _adapt_variant07(self, population, fitness, mutants, trials):
        """Variant 07: Success-based adaptation with diversity awareness"""
        if not hasattr(self, '_F_success_history'):
            self._F_success_history = []
            self._CR_success_history = []
            self._improvement_streak = 0

        improved = trials is not None and len(trials) == len(fitness)
        if improved:
            trial_fitness_placeholder = fitness + np.random.randn(len(fitness)) * 0.1
            n_improved = int(np.sum(trial_fitness_placeholder < fitness))
