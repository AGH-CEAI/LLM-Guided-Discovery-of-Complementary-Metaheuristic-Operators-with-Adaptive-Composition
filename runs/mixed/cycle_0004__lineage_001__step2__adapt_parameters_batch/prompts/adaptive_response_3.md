...
class CulturalMemeticDE:
    ...
    Automatically learns which mutation strategy and parameter adaptation
    strategy works best during optimization.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # === Adaptive Mutation Operator Selection (Thompson Sampling) ===
        ...
        # === Adaptive Parameter Adaptation Strategy Selection ===
        self.num_adapt_strategies = 3
        self.adapt_strategy_names = ['original', 'variant_05', 'variant_07']
        self.adapt_alpha = np.ones(self.num_adapt_strategies)
        self.adapt_beta = np.ones(self.num_adapt_strategies)
        self.adapt_rewards = [[] for _ in range(self.num_adapt_strategies)]
        self.current_adapt_idx = 0
        self.last_adapt_switch_gen = 0
        self.adapt_min_switch_gens = 8
        self.adapt_switches = []
        # Saved parameter states for each adaptation strategy
        self._saved_adapt_states = [{} for _ in range(self.num_adapt_strategies)]
    ...
    def _select_adapt_strategy_thompson(self):
        """Select parameter adaptation strategy using Thompson Sampling"""
        samples = np.random.beta(self.adapt_alpha, self.adapt_beta)
        selected = int(np.argmax(samples))
        return selected
    ...
    def _update_adapt_strategy_rewards(self, adapt_idx, reward):
        """Update rewards for parameter adaptation strategy with sliding window"""
        reward = float(reward)
        rewards = self.adapt_rewards[adapt_idx]
        rewards.append(reward)
        if len(rewards) > self.reward_window_size:
            rewards.pop(0)
        recent = rewards[-min(10, len(rewards)):]
        if len(recent) >= 3:
            ...
            success_rate = float(successes) / float(len(recent))
            avg_reward = float(np.mean(recent))
            self.adapt_alpha[adapt_idx] = float(np.clip(1.0 + success_rate * 5.0 + avg_reward * 2.0, 0.1, 100.0))
            self.adapt_beta[adapt_idx] = float(np.clip(1.0 + (1.0 - success_rate) * 5.0 - avg_reward * 2.0, 0.1, 100.0))

    def _save_adapt_state(self, idx):
        """Save current parameter state for adaptation strategy idx"""
        state = {
            'F': float(self.F),
            'CR': float(self.CR),
            'p_best_rate': float(self.p_best_rate),
            '_ema_F': getattr(self, '_ema_F', 0.7),
            '_ema_CR': getattr(self, '_ema_CR', 0.5),
            '_ema_p': getattr(self, '_ema_p', 0.1),
            '_F_success_history': getattr(self, '_F_success_history', []).copy(),
            '_CR_success_history': getattr(self, '_CR_success_history', []).copy(),
            '_improvement_streak': int(getattr(self, '_improvement_streak', 0)),
            '_prev_diversity': getattr(self, '_prev_diversity', None),
        }
        self._saved_adapt_states[idx] = state

    def _load_adapt_state(self, idx):
        """Load saved parameter state for adaptation strategy idx"""
        state = self._saved_adapt_states[idx]
        self.F = float(state.get('F', 0.7))
        self.CR = float(state.get('CR', 0.5))
        self.p_best_rate = float(state.get('p_best_rate', 0.1))
        if 'EMA_F' in state:
            self._ema_F = float(state['_ema_F'])
            self._ema_CR = float(state['_ema_CR'])
            self._ema_p = float(state['_ema_p'])
        if '_F_success_history' in state:
            self._F_success_history = list(state['_F_success_history'])
            self._CR_success_history = list(state['_CR_success_history'])
            self._improvement_streak = int(state['_improvement_streak'])
        if '_prev_diversity' in state:
            self._prev_diversity = float(state['_prev_diversity'])
    ...
    def __call__(self, func, stopping_condition):
        ...
        self._select_adapt_strategy_thompson()
        ...
        while not stopping_condition():
            ...
            self._update_adapt_strategy_rewards(self.current_adapt_idx, reward)
            ...
            if self.generation - self.last_adapt_switch_gen >= self.adapt_min_switch_gens:
                new_adapt = self._select_adapt_strategy_thompson()
                if new_adapt != self.current_adapt_idx:
                    self._save_adapt_state(self.current_adapt_idx)
                    self.current_adapt_idx = new_adapt
                    self._load_adapt_state(new_adapt)
                    self.last_adapt_switch_gen = self.generation
                    self.adapt_switches.append((self.generation, new_adapt))
    ...
    def _mutate_original(self, population, fitness):
        ...
        if self.generation > 5 and np.any(self.cultural_weights > 0):
            ...
            similarity = np.array([np.linalg.norm(population[i] - cultural_contribution) for i in range(self.NP)])
    ...
    def _adapt_parameters_batch(self, population, fitness, mutants, trials):
        ...
        if self.current_adapt_idx == 0:
            self._adapt_parameters_original(population, fitness, mutants, trials)
        elif self.current_adapt_idx == 1:
            self._adapt_parameters_variant05(population, fitness, mutants, trials)
        else:
            self._adapt_parameters_variant07(population, fitness, mutants, trials)

    def _adapt_parameters_original(self, population, fitness, mutants, trials):
        """Original parameter adaptation: random sampling of F and CR candidates"""
        F_candidates = np.random.uniform(0.4, 1.0, size=5)
        self.F = float(np.clip(float(np.mean(F_candidates)), 0.3, 1.5))
        CR_candidates = np.random.uniform(0.3, 0.9, size=5)
        self.CR = float(np.clip(float(np.mean(CR_candidates)), 0.1, 0.95))
        if np.random.rand() < 0.2:
            self.p_best_rate = float(np.clip(self.p_best_rate + np.random.uniform(-0.05, 0.05), 0.05, 0.3))

    def _adapt_parameters_variant05(self, population, fitness, mutants, trials):
        """Variant 05: EMA-based adaptation with stagnation handling and diversity-based p_best_rate"""
        if not hasattr(self, '_ema_F'):
            self._ema_F = 0.7
            self._ema_CR = 0.5
            self._ema_p = 0.1
        improvements = fitness - fitness
        if len(trials) == len(fitness) and len(fitness) > 0:
        ...
        if n_improved > 0:
            ...
            alpha = 0.3
            self._ema_F = float(np.clip(alpha * float(np.mean(F_used)) + (1 - alpha) * self._ema_F, 0.1, 2.0))
            self._ema_CR = float(np.clip(alpha * float(np.mean(CR_used)) + (1 - alpha) * self._ema_CR, 0.05, 0.99))
        ...
        current_diversity = self._compute_diversity_batch(population)
        if hasattr(self, '_prev_diversity'):
            diversity_change = current_diversity - self._prev_diversity
    ...
    def _adapt_parameters_variant07(self, population, fitness, mutants, trials):
        """Variant 07: Success history tracking with diversity-aware exploration/exploitation"""
        ...
        range_vals = np.ptp(population, axis=0)
        range_mean = float(np.mean(range_vals)) + 1e-10
        diversity = float(np.mean(distances) / range_mean)
