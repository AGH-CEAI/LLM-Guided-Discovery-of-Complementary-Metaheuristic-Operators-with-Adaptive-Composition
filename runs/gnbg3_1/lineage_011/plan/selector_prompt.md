...
Consider:
- Which operators directly and continuously influence the sampling distribution?
- Which methods can be replaced with fundamentally different strategies (not just tuned)?
- Which components have the highest impact on late-stage convergence?
- Which methods offer large design freedom (sampling, covariance, recombination)?

Important guidance:
- Prefer methods that act every generation.
- Prefer methods whose logic can be fundamentally redesigned.
- Prefer distribution-shaping operators (covariance, sampling, mean, step-size).
- Deprioritize restart, stagnation detection, and simple threshold logic.

CANDIDATE methods (you MUST pick exactly 3 distinct names from this list):
  - _clip_to_bounds  (3 LOC)
  - _handle_budget_truncation  (8 LOC)
  - _select_mutation_operator_batch  (11 LOC)
  - _crossover_batch  (15 LOC)
  - _check_stagnation  (17 LOC)
  - _restart_if_needed  (28 LOC)
  - _mutate_batch  (47 LOC)
  - _apply_local_refinement  (70 LOC)

HISTORY of cycles in this lineage so far (most recent last):
(no prior cycles in this lineage)

  Methods excluded (already picked 2+ times globally): _adapt_covariance, _adapt_covariance_original, _adapt_covariance_variant_01, _adapt_covariance_variant_06, _adapt_covariance_variant_08, _adapt_covariance_variant_09, _adapt_step_size, _compute_diversity, _initialize_population, _sample_trials_batch, _select_operator_thompson, _select_survivors_batch, _update_operator_rewards
  Global pick counts so far (method: count):
    _adapt_covariance: 2
    _adapt_covariance_original: 2
    _adapt_covariance_variant_01: 2
    _adapt_covariance_variant_06: 2
    _adapt_covariance_variant_08: 2
    _adapt_covariance_variant_09: 2
    _adapt_step_size: 2
    _compute_diversity: 2
    _initialize_population: 2
    _sample_trials_batch: 2
    _select_operator_thompson: 2
    _select_survivors_batch: 2
    _update_operator_rewards: 2
    _check_stagnation: 1
    _clip_to_bounds: 1
    _ensure_positive_definite: 1
    _restart_if_needed: 1
...
{"reasoning": "Step-by-step comparison of candidate methods based on influence, replaceability, and precision impact.", "analysis": {"method_scores": {"method_name": {"influence": "0-5", "replaceability": "0-5", "precision_impact": "0-5"}}, "top_candidates": ["method_a", "method_b", "..."]}, "reason": "...", "picks": ["...", "...", "..."]}
