...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - Sliding window credit assignment with exponential weighting
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection parameters - 6 best variants from benchmarks
        self.num_operators = 6
        self.operator_names = ['original', 'variant_01', 'variant_02', 'variant_03', 'variant_06', 'variant_08']
        ...
        self.recent_improvement_window = []
    ...
    def _update_operator_rewards(self):
        """Update operator rewards using sliding window credit assignment with exponential weighting."""
        ...
        # Compute fitness spread as additional reward signal
        fitness_spread = np.max(self.fitness) - np.min(self.fitness)
        spread_reward = float(np.clip(np.log1p(fitness_spread) / 20.0, 0.0, 1.0))
        
        # Base reward from improvement with log scaling
        reward = float(np.log1p(improvement * 1e10) / 10.0)
        reward = float(np.clip(reward, -10.0, 10.0))
        
        # Add diversity bonus
        reward += 0.2 * diversity
        reward += 0.1 * spread_reward
        ...
        if len(self.performance_history[op]) > self.reward_window_size:
            self.performance_history[op].pop(0)
        
        n = len(self.operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.operator_rewards[op]))
            var_reward = float(np.var(self.operator_rewards[op])) + 1e-10
            
            # Compute exponential moving average reward
            ema_reward = 0.0
            decay = 0.9
            weight_sum = 0.0
            for i, r in enumerate(self.operator_rewards[op]):
                w = decay ** (n - 1 - i)
                ema_reward += r * w
                weight_sum += w
            if weight_sum > 0:
                ema_reward /= weight_sum
            
            # Use EMA reward for more stable estimates
            mean_reward = 0.7 * mean_reward + 0.3 * ema_reward
            
            denom = max(var_reward * n, 1e-10)
        ...
        # Apply exploration bonus to less tried operators
        total_selections = float(np.sum(self.selection_counts)) + 1e-10
        exploration_bonus = 0.1 * (1.0 - self.selection_counts[op] / total_selections)
        self.alpha[op] += exploration_bonus
    ...
    def _adapt_covariance_variant_02(self):
        ...
        if not hasattr(self, 'prev_f_opt_variant02'):
            self.prev_f_opt_variant02 = self.f_opt
            self.improvement_ema = 1.0

        improvement = max(1e-10, self.prev_f_opt_variant02 - self.f_opt)
    ...
    def _adapt_covariance_variant_03(self):
        ...
        if not hasattr(self, 'prev_f_opt_variant03'):
            self.prev_f_opt_variant03 = self.f_opt
            self.exploration_temp = 1.0

        delta_f_opt = self.prev_f_opt_variant03 - self.f_opt
        self.prev_f_opt_variant03 = self.f_opt
