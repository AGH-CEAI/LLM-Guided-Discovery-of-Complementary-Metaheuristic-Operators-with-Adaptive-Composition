...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection parameters - 8 operators (variant_04 excluded as it crashed)
        ...
        self.operator_generation = {i: [] for i in range(self.num_operators)}
    ...
    def _reset_operator_state(self):
        ...
        # Reset operator tracking state
        for key in ['step_memory', 'step_memory_fitness', 'recent_improvements', 
                    'archive_positions', 'archive_fitness', 'archive_generations',
                    'cov_momentum', 'pca_cache']:
            if hasattr(self, key):
                delattr(self, key)
    ...
    def _adapt_step_size(self):
        ...
        for i in range(min(self.NP, min(len(self.trial_fitness), len(self.fitness)))):
    ...
    def _select_operator_thompson(self):
        ...
        self.operator_generation[self.current_operator].append(self.generation)
    ...
    def _update_operator_rewards(self):
        ...
        cond_penalty = np.log1p(cond) / (10.0 + np.log1p(self.dim))
        
        # Compute relative fitness progress
        rel_progress = improvement / (max(abs(self.f_opt), 1.0) + 1e-10)
        
        reward = float(np.log1p(improvement * 1e10) / 10.0 + 0.1 * diversity - 0.05 * cond_penalty)
        reward += 0.1 * np.log1p(rel_progress * 100.0)
            ...
            if i < len(self.trial_fitness) and i < len(self.fitness):
                if self.trial_fitness[i] < self.fitness[i]:
                    ...
                    if step_norm > 1e-10:
                        ...
                        self.step_memory_fitness.append(max(self.fitness[i] - self.trial_fitness[i], 1e-10))
