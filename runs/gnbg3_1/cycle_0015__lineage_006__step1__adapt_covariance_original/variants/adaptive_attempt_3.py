...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - Sliding window credit assignment with adaptive reward scaling
    - Performance-weighted operator selection
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = [
            'original', 'variant_01', 'variant_02', 'variant_03', 'variant_05',
            'variant_06', 'variant_07', 'variant_08', 'variant_09', 'variant_restart'
        ]
        ...
        # Adaptive selection hyperparameters
        self.exploration_bonus = 0.1
        self.min_selections_for_confidence = 5
    ...
    def _reset_operator_state(self):
        ...
        # Reset operator-specific state for variants
        for attr in ['step_memory', 'step_memory_fitness', 'recent_improvements', 
                     'cov_momentum', 'exploration_temp', 'archive_positions', 
                     'archive_fitness', 'archive_generations', 'stagnation_counter_var']:
    ...
    def _select_operator_thompson(self):
        """Select operator using Thompson Sampling from Beta distributions with adaptive scaling."""
        # Compute adaptive scaling based on selection confidence
        total_selections = np.sum(self.selection_counts)
        
        if total_selections > 0:
            # Boost operators with fewer selections to ensure exploration
            selection_probs = self.selection_counts / total_selections
            min_prob = np.min(selection_probs)
            exploration_boost = np.ones(self.num_operators) * self.exploration_bonus
            exploration_boost[selection_probs > min_prob * 2] *= 0.5
            
            # Add small random noise to break ties
            noise = np.random.uniform(0, 0.01, self.num_operators)
            
            # Thompson Sampling with adjusted parameters
            adjusted_alpha = self.alpha.copy()
            adjusted_beta = self.beta.copy()
            
            # Increase variance for under-explored operators
            for i in range(self.num_operators):
                if self.selection_counts[i] < self.min_selections_for_confidence:
                    # Use uniform-like sampling for new operators
                    adjusted_alpha[i] = 1.0
                    adjusted_beta[i] = 1.0
            
            samples = np.random.beta(adjusted_alpha, adjusted_beta) + exploration_boost + noise
        else:
    ...
    def _update_operator_rewards(self):
        """Update operator rewards using sliding window credit assignment with adaptive scaling."""
        ...
        rel_improvement = improvement / (fit_scale + 1e-10)
        reward_improvement = float(np.log1p(rel_improvement * 1e10) / 10.0)
        
        # Diversity reward (higher when population is diverse)
        reward_diversity = 0.1 * diversity
        
        # Stagnation penalty (negative when stuck)
        stagnation_penalty = -0.05 * min(self.stagnation_counter / self.max_stagnation, 1.0)
        
        # Combined reward with adaptive scaling
        reward = reward_improvement + reward_diversity + stagnation_penalty
        reward = float(np.clip(reward, -10.0, 10.0))
        
        op = self.current_operator
        self.operator_rewards[op].append(reward)
        
        # Apply reward decay for older rewards
        if len(self.operator_rewards[op]) > 0:
            for i in range(len(self.operator_rewards[op]) - 1):
                self.operator_rewards[op][i] *= self.reward_decay
        ...
        # Track improvements for this operator
        if len(self.operator_fitness_history[op]) >= 2:
            hist = self.operator_fitness_history[op]
            imp = max(0.0, hist[-2] - hist[-1])
            self.operator_improvements[op].append(imp)
        ...
        if n >= 3:
            ...
            temp = mean_reward * (1 - mean_reward) / denom - 1
            alpha_new = max(1.0, mean_reward * (n - 1) / denom + 1)
            beta_new = max(1.0, (1 - mean_reward) * (n - 1) / denom + 1)
            
            # Smooth update to prevent rapid changes
            self.alpha[op] = float(0.7 * self.alpha[op] + 0.3 * alpha_new)
            self.beta[op] = float(0.7 * self.beta[op] + 0.3 * beta_new)
            
            # Ensure valid parameters
            self.alpha[op] = max(1.0, self.alpha[op])
            self.beta[op] = max(1.0, self.beta[op])
        elif n >= 1:
    ...
    def _adapt_covariance(self):
        ...
        if self.current_operator == 0:
            ...
            self._adapt_covariance_variant_restart()
            ...
            if i < len(self.trial_fitness) and i < len(self.fitness):
                if self.trial_fitness[i] < self.fitness[i]:
                    ...
                    if step_norm > 1e-10:
                        ...
                        self.step_memory_fitness.append(float(self.fitness[i] - self.trial_fitness[i]))
    ...
    def _adapt_covariance_variant_05(self):
        ...
        if hasattr(self, 'stagnation_counter_var'):
            self.stagnation_counter_var += 1
        else:
            self.stagnation_counter_var = 0

        if self.stagnation_counter_var > self.max_stagnation:
            should_restart = True
    ...
    def _adapt_covariance_variant_restart(self):
    ...
    def _restart_if_needed(self):
        ...
        if (self.stagnation_counter > self.max_stagnation or 
            ...
            np.any(np.isnan(self.C)) or
            np.any(np.isinf(self.C))):
