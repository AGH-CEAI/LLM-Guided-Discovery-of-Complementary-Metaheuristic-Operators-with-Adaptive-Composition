...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    AND reward update strategy during optimization using multi-level Thompson Sampling.
    
    Features:
    - 5 covariance adaptation strategies
    - 10 reward update strategies (adaptive selection)
    - Multi-level Thompson Sampling for operator and strategy selection
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection parameters (5 operators)
        self.num_operators = 5
        self.operator_names = ['original', 'variant_01', 'variant_06', 'variant_08', 'variant_09']
        
        # Thompson Sampling with Beta distributions for operators
        ...
        # ===== NEW: Adaptive reward strategy selection (10 strategies) =====
        ...
        # Thompson Sampling for reward strategy selection
        ...
        self.reward_strategy_counts = np.zeros(self.num_reward_strategies)
        self.current_reward_strategy = 0
        
        # Meta-level adaptation parameters
        self.strategy_selection_window = 20
        self.recent_strategy_performance = {i: [] for i in range(self.num_reward_strategies)}
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Select reward strategy BEFORE selecting operator (meta-level adaptation)
            ...
            # Update reward strategy performance based on improvement
            self._update_reward_strategy_performance()
    ...
    def _select_reward_strategy_thompson(self):
        """Select reward update strategy using Thompson Sampling with performance weighting."""
        # Compute performance-weighted probabilities for each strategy
        strategy_scores = np.zeros(self.num_reward_strategies)
        for i in range(self.num_reward_strategies):
            if len(self.recent_strategy_performance[i]) > 0:
                recent_mean = float(np.mean(self.recent_strategy_performance[i]))
                recent_count = len(self.recent_strategy_performance[i])
                # Boost score for strategies with more data and better performance
                strategy_scores[i] = recent_mean * np.log1p(recent_count)
            else:
                strategy_scores[i] = 0.0
        
        # Add exploration bonus for under-explored strategies
        min_count = float(np.min(self.reward_strategy_counts))
        for i in range(self.num_reward_strategies):
            exploration_bonus = 0.1 * (min_count + 1) / (self.reward_strategy_counts[i] + 1)
            strategy_scores[i] += exploration_bonus
        
        # Use Thompson Sampling with adjusted parameters
        adjusted_alpha = self.reward_alpha.copy()
        adjusted_beta = self.reward_beta.copy()
        
        # Boost exploration of strategies with low counts
        for i in range(self.num_reward_strategies):
            if self.reward_strategy_counts[i] < 3:
                adjusted_beta[i] *= 2.0  # Increase uncertainty for new strategies
        
        # Sample from Beta distribution
        samples = np.random.beta(adjusted_alpha, adjusted_beta)
        
        # Blend with performance scores (70% Thompson, 30% exploitation)
        normalized_scores = (strategy_scores - np.min(strategy_scores) + 1e-10)
        normalized_scores /= (np.max(normalized_scores) + 1e-10)
        
        combined_scores = 0.7 * samples + 0.3 * normalized_scores
        self.current_reward_strategy = int(np.argmax(combined_scores))
        self.reward_strategy_counts[self.current_reward_strategy] += 1
    ...
    def _update_reward_strategy_performance(self):
        """Update performance tracking for the selected reward strategy."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
        
        # Track improvement for the selected strategy
        strategy = self.current_reward_strategy
        self.reward_strategy_improvements[strategy].append(float(improvement))
        
        # Keep sliding window
        if len(self.reward_strategy_improvements[strategy]) > self.strategy_selection_window:
            self.reward_strategy_improvements[strategy].pop(0)
        
        # Update recent performance for strategy selection
        self.recent_strategy_performance[strategy].append(float(improvement))
        if len(self.recent_strategy_performance[strategy]) > self.strategy_selection_window:
            self.recent_strategy_performance[strategy].pop(0)
        
        # Update Thompson Sampling parameters based on improvement
        # Positive improvement = good strategy
        reward_signal = float(np.clip(np.log1p(improvement * 1e10) / 10.0, -10.0, 10.0))
        
        # Map to [0, 1] for Beta parameters
        reward_mapped = (reward_signal + 10.0) / 20.0
        reward_mapped = float(np.clip(reward_mapped, 0.01, 0.99))
        
        self.reward_alpha[strategy] += reward_mapped
        self.reward_beta[strategy] += (1.0 - reward_mapped)
        
        # Keep bounded
        self.reward_alpha[strategy] = float(np.clip(self.reward_alpha[strategy], 1e-6, 1e6))
        self.reward_beta[strategy] = float(np.clip(self.reward_beta[strategy], 1e-6, 1e6))
    
    def _update_operator_rewards(self):
        """Dispatch to the selected reward update strategy."""
        if self.current_reward_strategy == 0:
            self._update_rewards_original()
        elif self.current_reward_strategy == 1:
            self._update_rewards_variant_01()
        elif self.current_reward_strategy == 2:
            self._update_rewards_variant_02()
        elif self.current_reward_strategy == 3:
            self._update_rewards_variant_03()
        elif self.current_reward_strategy == 4:
            self._update_rewards_variant_04()
        elif self.current_reward_strategy == 5:
            self._update_rewards_variant_05()
        elif self.current_reward_strategy == 6:
            self._update_rewards_variant_06()
        elif self.current_reward_strategy == 7:
            self._update_rewards_variant_07()
        elif self.current_reward_strategy == 8:
            self._update_rewards_variant_08()
        else:
            self._update_rewards_variant_09()
    
    # ========== Reward Update Strategies ==========
    
    def _update_rewards_original(self):
        """Original reward update strategy."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
    ...
    def _update_rewards_variant_01(self):
        """Sigmoid reward with EMA tracking."""
    ...
    def _update_rewards_variant_02(self):
    ...
    def _update_rewards_variant_03(self):
        """Hybrid rank-immediate reward strategy."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
        ...
        immediate_reward = float(np.log1p(improvement * 1e10) / 10.0)
        
        if len(self.trial_fitness) >= self.NP // 2:
            sorted_fitness = np.sort(np.concatenate([self.fitness, self.trial_fitness]))
            rank = np.searchsorted(sorted_fitness, self.f_opt, side='left') / len(sorted_fitness)
            rank_reward = float(1.0 - rank)
        else:
            rank_reward = 0.5
        
        reward = 0.6 * immediate_reward + 0.4 * rank_reward + 0.1 * diversity
        reward = float(np.clip(reward, -10.0, 10.0))
    ...
    def _update_rewards_variant_04(self):
        """Trend-aware multi-signal credit assignment."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
    ...
    def _update_rewards_variant_05(self):
        """Exponentially-weighted cumulative improvement with stagnation awareness."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
    ...
    def _update_rewards_variant_06(self):
        """Max-reward tracking with EMA smoothing."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
    ...
    def _update_rewards_variant_07(self):
        """Per-operator rank-normalized credit assignment."""
    ...
    def _update_rewards_variant_08(self):
        """Inverse-error task-difficulty weighting."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
    ...
    def _update_rewards_variant_09(self):
        """Error-scaled exploration-weighted credit assignment."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
