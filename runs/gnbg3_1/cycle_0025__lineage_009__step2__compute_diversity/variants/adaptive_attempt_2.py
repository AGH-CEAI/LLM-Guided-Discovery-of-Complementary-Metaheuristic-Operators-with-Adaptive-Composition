...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - 6 diversity computation strategies with adaptive selection
    - Thompson Sampling for operator selection (both covariance and diversity)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Diversity computation operators (6 strategies)
        self.num_diversity_operators = 6
        self.diversity_names = ['original', 'variant_02', 'variant_04', 'variant_06', 'variant_08', 'variant_09']
        
        # Thompson Sampling for diversity operators
        self.alpha_div = np.ones(self.num_diversity_operators)
        self.beta_div = np.ones(self.num_diversity_operators)
        
        # Sliding window for credit assignment
        self.reward_window_size = 10
        self.operator_rewards = {i: [] for i in range(self.num_cov_operators)}
        self.diversity_rewards = {i: [] for i in range(self.num_diversity_operators)}
        self.operator_counts = np.zeros(self.num_cov_operators)
        self.diversity_counts = np.zeros(self.num_diversity_operators)
        self.selection_counts = np.zeros(self.num_cov_operators)
        self.diversity_selection_counts = np.zeros(self.num_diversity_operators)
        
        # Runtime state
        self.generation = 0
        self.current_operator = 0
        self.current_diversity_operator = 0
        self.L = None
    ...
    def _select_diversity_operator_thompson(self):
        ...
        self.current_diversity_operator = int(np.argmax(samples))
        self.diversity_counts[self.current_diversity_operator] += 1
        return self.current_diversity_operator
    ...
    def _update_cov_operator_rewards(self):
        ...
        if not hasattr(self, 'operator_cumulative_reward'):
            self.operator_cumulative_reward = np.zeros(self.num_cov_operators)
        if not hasattr(self, 'operator_decay_sum'):
            self.operator_decay_sum = np.zeros(self.num_cov_operators)
    ...
    def _update_diversity_operator_rewards(self):
        """Credit assignment for diversity operators based on correlation with fitness improvement."""
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
        ...
        if not hasattr(self, 'diversity_cumulative_reward'):
            self.diversity_cumulative_reward = np.zeros(self.num_diversity_operators)
        if not hasattr(self, 'diversity_decay_sum'):
            self.diversity_decay_sum = np.zeros(self.num_diversity_operators)

        decay = 0.95
        self.diversity_cumulative_reward *= decay
        self.diversity_decay_sum *= decay

        self.diversity_cumulative_reward[self.current_diversity_operator] += reward
        self.diversity_decay_sum[self.current_diversity_operator] += 1.0

        norm = max(self.diversity_decay_sum[self.current_diversity_operator], 1.0)
        normalized_reward = self.diversity_cumulative_reward[self.current_diversity_operator] / norm
        normalized_reward = float(np.clip(normalized_reward, -10.0, 10.0))

        op = self.current_diversity_operator
        self.diversity_rewards[op].append(normalized_reward)

        if len(self.diversity_rewards[op]) > self.reward_window_size:
            self.diversity_rewards[op].pop(0)

        n = len(self.diversity_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.diversity_rewards[op]))
            var_reward = float(np.var(self.diversity_rewards[op]))
            denom = max(var_reward * n + 1e-10, 1e-10)
            ...
            sum_reward = float(np.sum(self.diversity_rewards[op]))
            if sum_reward > 0:
    ...
    def _compute_diversity(self):
        """Compute diversity using the adaptively selected strategy."""
        if self.current_diversity_operator == 0:
            return self._compute_diversity_original()
        elif self.current_diversity_operator == 1:
            return self._compute_diversity_variant_02()
        elif self.current_diversity_operator == 2:
            return self._compute_diversity_variant_04()
        elif self.current_diversity_operator == 3:
            return self._compute_diversity_variant_06()
        elif self.current_diversity_operator == 4:
