...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 11 covariance adaptation strategies (original + 10 variants from benchmark)
    - Thompson Sampling for operator selection with Beta distributions
    - Sliding window credit assignment with exponential moving average rewards
    - Automatic operator reward tracking and adaptation
    - Robust restart on stagnation
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = [
            ...
            'variant_06', 'variant_07', 'variant_08', 'variant_09', 'variant_10', 'variant_11'
        ]
        ...
        self.ema_decay = 0.9
        self.operator_ema = np.zeros(self.num_operators)
        self.operator_var = np.ones(self.num_operators) * 0.25
    ...
    def _select_operator_thompson(self):
        """Select operator using Thompson Sampling from Beta distributions with EMA smoothing."""
        # Add exploration bonus for under-sampled operators
        min_selections = float(np.min(self.selection_counts[self.selection_counts > 0])) if np.any(self.selection_counts > 0) else 0.0
        exploration_bonus = np.maximum(0.0, min_selections + 5.0 - self.selection_counts) * 0.1
        
        # Thompson sampling with exploration bonus
        samples = np.random.beta(self.alpha, self.beta) + exploration_bonus
        self.current_operator = int(np.argmax(samples))
        self.operator_counts[self.current_operator] += 1
        self.selection_counts[self.current_operator] += 1
        return self.current_operator
    
    def _compute_reward(self, improvement, diversity):
        """Compute reward for operator selection."""
        # Primary reward based on fitness improvement
        reward_improvement = float(np.log1p(max(improvement, 0.0) * 1e10) / 10.0)
        
        # Secondary reward based on diversity contribution
        reward_diversity = float(0.2 * diversity)
        
        # Tertiary reward based on fitness variance (exploration signal)
        fit_var = float(np.var(self.fitness)) if len(self.fitness) > 1 else 0.0
        fit_scale = max(abs(self.f_opt), 1.0)
        rel_var = fit_var / (fit_scale ** 2 + 1e-10)
        reward_variance = float(0.1 * np.log1p(rel_var * 100))
        
        # Combine rewards
        reward = reward_improvement + reward_diversity + reward_variance
    ...
    def _update_operator_rewards(self):
        """Update operator rewards using sliding window credit assignment with EMA."""
        ...
        reward = self._compute_reward(improvement, diversity)
        ...
        n_selections = float(self.operator_counts[op])
        if n_selections > 1:
            self.operator_ema[op] = float(self.ema_decay * self.operator_ema[op] + (1.0 - self.ema_decay) * reward)
            # Update variance estimate
            old_var = self.operator_var[op]
            self.operator_var[op] = float(0.95 * old_var + 0.05 * (reward - self.operator_ema[op]) ** 2)
        else:
            self.operator_ema[op] = float(reward)
        
        # Update Beta distribution parameters based on sliding window
        n = len(self.operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.operator_rewards[op]))
            var_reward = float(np.var(self.operator_rewards[op])) if n > 1 else 0.01
            
            # Use method of moments for Beta distribution
            mean_reward = np.clip(mean_reward, 0.01, 0.99)
            var_reward = np.clip(var_reward, 0.001, 0.25)
            
            denom = max(var_reward * n + 1e-10, 1e-10)
            new_alpha = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            new_beta = float(max(1.0, (1.0 - mean_reward) * ((n - 1) * (1.0 - mean_reward) / denom + 1)))
            
            # Blend with EMA-based estimate for smoother adaptation
            blend_factor = 0.7
            self.alpha[op] = float(blend_factor * self.alpha[op] + (1.0 - blend_factor) * new_alpha)
            self.beta[op] = float(blend_factor * self.beta[op] + (1.0 - blend_factor) * new_beta)
        elif n >= 1:
        ...
        self.alpha[op] = float(np.clip(self.alpha[op], 0.1, 100.0))
        self.beta[op] = float(np.clip(self.beta[op], 0.1, 100.0))
    ...
    def _adapt_covariance(self):
        ...
        if self.current_operator == 0:
            ...
            self._adapt_covariance_variant_11()
    ...
    def _adapt_covariance_variant_01(self):
        ...
        cc_1_adapt = float(np.clip(cc_1_adapt, 1e-10, 0.5))
        cc_mu_adapt = float(np.clip(cc_mu_adapt, 1e-10, 0.5))
    ...
    def _adapt_covariance_variant_02(self):
        ...
        self.C = ((1.0 - ccov_1 - ccov_mu) * self.C + 
                  ccov_1 * rank_one + 
                  ccov_mu * rank_mu * float(np.sum(self.weights[:self.mu])))
        ...
        try:
            ...
            if cond > narrow_threshold:
                ...
                expansion[eigvals < float(np.median(eigvals))] = expansion_factor
    ...
    def _adapt_covariance_variant_04(self):
        ...
        new_cond = float(np.max(eigvals_new) / np.min(eigvals_new))
        if new_cond > 1e+8:
            min_eig = float(np.min(eigvals_new))
    ...
    def _adapt_covariance_variant_05(self):
        ...
        stagnation_threshold = float(max(20, self.dim * 2))
        ...
        if is_stagnant:
            try:
                ...
                for k in range(num_perturb):
                    minor_dir = eigvecs[:, k]
                    scale = float(np.sqrt(eigvals[k]) * self.sigma * 0.3)
                ...
                for i in range(min(10, self.NP)):
                    ...
                    random_dir = random_dir / (float(np.linalg.norm(random_dir)) + 1e-15)
    ...
    def _adapt_covariance_variant_07(self):
        ...
        if self.f_opt < self.best_known_fitness:
            ...
            self.stagnation_depth = float(np.log1p(max(self.best_known_fitness - self.f_opt, 0)) + 1.0)

        stagnation_factor = float(np.clip(self.stagnation_depth / 10.0, 0.0, 1.0))

        ccov_base = (1.0 / self.mueff) * (2.0 / (self.dim + 1.41) ** 2) + \
                    (1.0 - 1.0 / self.mueff) * (2.0 * self.cc - 1.0 / self.mueff)

        ccov = float(ccov_base * (1.0 + 5.0 * stagnation_factor))
        ccov = float(np.clip(ccov, 1e-8, 0.5))

        cc_adaptive = float(self.cc * (1.0 + 2.0 * stagnation_factor))
        cc_adaptive = float(np.clip(cc_adaptive, 0.01, 0.5))
    ...
    def _adapt_covariance_variant_08(self):
        ...
        y_mean_norm = float(np.dot(y_mean, y_mean))
        rank_one = np.outer(y_mean, y_mean) / max(y_mean_norm, 1e-30)

        C_target = (1.0 - ccov_1 - ccov_mu) * self.C
        C_target += ccov_1 * rank_one
        C_target += ccov_mu * rank_mu * float(np.sum(self.weights[:self.mu]))

        # Ensure symmetry and positive definiteness
        C_target = 0.5 * (C_target + C_target.T)
        min_eig = float(np.min(np.linalg.eigvalsh(C_target)))
        if min_eig < 1e-12:
        ...
        try:
            ...
            C_target = np.eye(self.dim) * float(np.mean(np.diag(self.C)))
            eigvals, eigvecs = np.linalg.eigh(C_target)
        ...
        cond = float(np.max(eigvals) / (float(np.min(eigvals)) + 1e-15))

        if cond > 1e6 or diversity < 0.05:
            # Escape mode: boost smallest eigenvalues significantly
            eigvals = eigvals ** 0.5
            min_eig_val = float(np.min(eigvals))
        ...
        if diversity < 0.02:
            for _ in range(3):
                ...
                z = z / max(z_norm, 1e-15)
                random_dir = np.outer(z, z)
                self.C += 0.1 * float(np.mean(eigvals)) * random_dir
    ...
    def _adapt_covariance_variant_09(self):
        ...
        eig_min = float(np.maximum(eigvals.min(), 1e-15))
        eig_max = float(np.maximum(eigvals.max(), 1e-15))
        ...
        ccov_adaptive = float(min(ccov_base * spread_factor, 0.5))
        cc_adaptive = float(min(self.cc * spread_factor, 0.3))
        ...
        if log_cond > 3.0:
            ...
            noise_scale = 0.01 * ccov_adaptive * float(np.mean(eigvals))
    ...
    def _adapt_covariance_variant_10(self):
        ...
        cond = float(np.max(eigvals)) / (float(np.maximum(np.min(eigvals), 1e-10)))
    ...
    def _adapt_covariance_variant_11(self):
        """Hybrid adaptive covariance combining best aspects of multiple variants."""
        y_mean = (self.mean - self.old_mean) / self.sigma
        ...
        # Adaptive learning rates based on multiple signals
        log_cond = float(np.log10(max(cond, 1.0)))
        ...
        cond_factor = 1.0 + max(0.0, log_cond - 2.0) * 0.3
        diversity_factor = 1.0 + 2.0 * (1.0 - diversity)
        stagnation_factor = float(np.clip(self.stagnation_counter / float(max(self.max_stagnation, 1)), 0.0, 1.0))

        combined_factor = cond_factor * diversity_factor * (1.0 + stagnation_factor * 0.5)

        ccov_adaptive = float(np.clip(ccov_base * combined_factor, 1e-8, 0.5))
        cc_adaptive = float(np.clip(self.cc * (1.0 + stagnation_factor * 0.3), 0.01, 0.3))
        ...
        # Stagnation escape: anisotropic expansion
        if stagnation_factor > 0.5:
            escape_strength = stagnation_factor * 0.3
            n_directions = min(self.dim, max(2, int(self.dim * 0.2)))
            ...
            self.sigma *= (1.0 + stagnation_factor * 0.5)

        # Diversity injection when needed
        if diversity < 0.05:
            self.C += 0.1 * float(np.mean(eigvals)) * np.eye(self.dim)
