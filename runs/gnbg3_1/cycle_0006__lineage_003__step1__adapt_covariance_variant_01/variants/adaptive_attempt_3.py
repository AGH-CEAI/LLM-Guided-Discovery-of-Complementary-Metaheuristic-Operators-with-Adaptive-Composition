...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 11 covariance adaptation strategies (original + 10 variants from benchmarks)
    - Thompson Sampling for operator selection with Beta distributions
    - Sliding window credit assignment with diversity-aware rewards
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection parameters - 11 operators
        ...
        self.operator_names = [
            ...
            'variant_11'  # reserved for future use
        ]
        ...
        # Reward history for credit assignment
        ...
        # Stagnation tracking for variants that use it
        ...
        # Diversity tracking
        self.prev_f_opt = float('inf')
        self.improvement_ema = 1.0
        
        # Operator-specific state
        self.pc_weighted = None
        self.p_cross = None
    ...
    def _initialize_population(self):
        ...
        # Initialize diagonal covariance for variants that use it
    ...
    def _adapt_step_size(self):
        ...
        if not hasattr(self, 'improvement_ema') or self.improvement_ema is None:
    ...
    def _update_operator_rewards(self):
        ...
        # Compute condition number as complexity indicator
        try:
            eigvals = np.linalg.eigvalsh(self.C)
            eigvals = np.maximum(eigvals, 1e-15)
            cond = np.max(eigvals) / np.min(eigvals)
            cond_factor = np.log1p(np.clip(cond, 0, 1e10)) / 20.0
        except:
            cond_factor = 0.0
        ...
        # Composite reward: improvement + diversity + condition awareness
        reward = float(np.log1p(improvement * 1e10) / 10.0)
        reward += 0.15 * diversity
        reward += 0.05 * cond_factor
        reward += 0.1 * (1.0 - np.clip(rel_var, 0, 1))
        reward = float(np.clip(reward, -10.0, 10.0))
        
        # Store in reward history
        self.reward_history.append((self.current_operator, reward))
        if len(self.reward_history) > self.reward_history_size:
            self.reward_history.pop(0)
        
        # Update operator-specific sliding window
    ...
    def _adapt_covariance_variant_01(self):
        ...
        self.C = np.diag(self.diag_C.copy())
    ...
    def _adapt_covariance_variant_06(self):
        ...
        if not hasattr(self, 'stagnation_history') or self.stagnation_history is None:
            self.stagnation_history = []
    ...
    def _adapt_covariance_variant_07(self):
        ...
        if not hasattr(self, 'best_known_fitness') or self.best_known_fitness is None:
            self.best_known_fitness = float('inf')
    ...
    def _restart_if_needed(self):
        ...
        has_nan = np.any(np.isnan(self.C)) if hasattr(self, 'C') and self.C is not None else False
