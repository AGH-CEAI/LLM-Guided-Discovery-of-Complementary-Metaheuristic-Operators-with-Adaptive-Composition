...
class AdaptiveCovarianceEvolutionStrategy:
    """
    Adaptive optimizer with automatic selection of the best _clip_to_bounds strategy
    using Thompson Sampling with sliding window credit assignment.
    
    Features:
    - CMA-ES covariance adaptation
    - Adaptive restart strategies
    - Adaptive clip-to-bounds strategy selection (5 variants)
    - Thompson Sampling for all operator selections
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive clip strategy selection parameters
        self.num_clip_strategies = 5
        self.clip_strategy_names = ['simple_clip', 'smooth_reflection', 'reflection_noise', 'simple_reflection', 'cyclic_wrap']
        
        # Thompson Sampling with Beta distributions for clip strategies
        self.alpha_clip = np.ones(self.num_clip_strategies)
        self.beta_clip = np.ones(self.num_clip_strategies)
        
        # Sliding window for clip strategy credit assignment
        self.clip_reward_window_size = 8
        self.clip_strategy_rewards = {i: [] for i in range(self.num_clip_strategies)}
        self.clip_strategy_counts = np.zeros(self.num_clip_strategies)
        
        # Track clip strategy outcomes
        self.last_clip_f_opt = None
        self.last_clip_generation = 0
        self.clip_selection_counts = np.zeros(self.num_clip_strategies)
    ...
    def _clip_to_bounds(self, x):
        """Clip solution to bounds using adaptive strategy selection."""
        strategy = int(self.current_clip_strategy)
        
        if strategy == 0:
            return self._clip_simple(x)
        elif strategy == 1:
            return self._clip_smooth_reflection(x)
        elif strategy == 2:
            return self._clip_reflection_noise(x)
        elif strategy == 3:
            return self._clip_simple_reflection(x)
        else:
            return self._clip_cyclic_wrap(x)
    
    def _clip_simple(self, x):
        """Simple clip to bounds (baseline)."""
        return np.clip(x, self.lb, self.ub)
    
    def _clip_smooth_reflection(self, x):
        """Clip solution to bounds using smooth reflection (variant_02)."""
    ...
    def _clip_reflection_noise(self, x):
        """Reflective boundary handling with adaptive noise injection (variant_04)."""
        x = np.asarray(x, dtype=np.float64)
    ...
    def _clip_simple_reflection(self, x):
        """Reflect out-of-bounds values back into the feasible region (variant_08)."""
        x = np.asarray(x, dtype=np.float64)
    ...
    def _clip_cyclic_wrap(self, x):
        """Clip solution to bounds using cyclic wrapping (variant_09)."""
        bound_range = self.ub - self.lb
    ...
    def _select_clip_strategy_thompson(self):
        """Select clip strategy using Thompson Sampling from Beta distributions."""
        samples = np.random.beta(self.alpha_clip, self.beta_clip)
        self.current_clip_strategy = int(np.argmax(samples))
        self.clip_strategy_counts[self.current_clip_strategy] += 1
        self.clip_selection_counts[self.current_clip_strategy] += 1
        return self.current_clip_strategy
    
    def _update_clip_strategy_rewards(self, f_opt_before, f_opt_after, generations_count):
        """Credit assignment for clip strategies based on post-selection performance.
        
        Args:
            f_opt_before: float, f_opt before clip strategy was selected
            f_opt_after: float, f_opt after clip strategy was selected
            generations_count: int, number of generations since last update
        """
        op = int(self.current_clip_strategy)
        
        # Compute improvement
        improvement = max(0.0, f_opt_before - f_opt_after)
        
        # Compute reward based on improvement
        if improvement > 0:
            reward = float(np.clip(np.log1p(improvement * 1e8) / 8.0, 0.0, 2.0))
        else:
            reward = 0.0
        
        # Bonus for consistency over multiple generations
        if generations_count >= 5:
            reward *= 1.0 + 0.2 * min(generations_count / 20.0, 1.0)
        
        # Initialize cumulative tracking if needed
        if not hasattr(self, 'clip_operator_cumulative_reward'):
            self.clip_operator_cumulative_reward = np.zeros(self.num_clip_strategies)
        if not hasattr(self, 'clip_operator_decay_sum'):
            self.clip_operator_decay_sum = np.zeros(self.num_clip_strategies)
        
        # Apply exponential decay
        decay = 0.9
        self.clip_operator_cumulative_reward *= decay
        self.clip_operator_decay_sum *= decay
        
        # Update cumulative reward
        self.clip_operator_cumulative_reward[op] += reward
        self.clip_operator_decay_sum[op] += 1.0
        
        # Normalize reward
        norm = max(self.clip_operator_decay_sum[op], 1.0)
        normalized_reward = self.clip_operator_cumulative_reward[op] / norm
        normalized_reward = float(np.clip(normalized_reward, -5.0, 5.0))
        
        # Store in sliding window
        self.clip_strategy_rewards[op].append(normalized_reward)
        if len(self.clip_strategy_rewards[op]) > self.clip_reward_window_size:
            self.clip_strategy_rewards[op].pop(0)
        
        # Update Beta distribution parameters
        n = len(self.clip_strategy_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.clip_strategy_rewards[op]))
            var_reward = float(np.var(self.clip_strategy_rewards[op]))
            denom = max(var_reward * n + 1e-10, 1e-10)
            self.alpha_clip[op] = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            self.beta_clip[op] = float(max(1.0, (1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.clip_strategy_rewards[op]))
            if sum_reward > 0:
                self.alpha_clip[op] = 1.0 + sum_reward
            else:
                self.beta_clip[op] = 1.0 - sum_reward
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            # Select clip strategy using Thompson Sampling
            self._select_clip_strategy_thompson()
            
            # Track f_opt before this generation for clip reward
            f_opt_before_clip = float(self.f_opt)
            clip_selection_gen = int(self.generation)
            ...
            # Update clip strategy rewards after enough generations
            generations_since_clip_update = int(self.generation - clip_selection_gen)
            if generations_since_clip_update >= 3:
                f_opt_after_clip = float(self.f_opt)
                self._update_clip_strategy_rewards(f_opt_before_clip, f_opt_after_clip, generations_since_clip_update)
    ...
    def _initialize_population(self):
        ...
        self.last_restart_generation = 0
        self.last_clip_f_opt = self.f_opt
        self.last_clip_generation = 0
