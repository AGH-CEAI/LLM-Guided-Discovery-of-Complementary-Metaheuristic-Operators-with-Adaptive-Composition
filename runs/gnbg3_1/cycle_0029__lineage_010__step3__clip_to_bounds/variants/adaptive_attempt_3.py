...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - 4 adaptive clip-to-bounds strategies (NEW)
    - Thompson Sampling for all operator selections
    - Sliding window credit assignment for restart and clip operators
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # ===== NEW: Adaptive clip-to-bounds operator selection =====
        self.num_clip_operators = 4
        self.clip_operator_names = ['clip_basic', 'clip_reflection', 'clip_reflection_noise', 'clip_cyclic']
        
        # Thompson Sampling with Beta distributions for clip operators
        self.alpha_clip = np.ones(self.num_clip_operators)
        self.beta_clip = np.ones(self.num_clip_operators)
        
        # Sliding window for clip operator credit assignment
        self.clip_reward_window_size = 8
        self.clip_operator_rewards = {i: [] for i in range(self.num_clip_operators)}
        self.clip_operator_counts = np.zeros(self.num_clip_operators)
        
        # Track clip operator performance
        self.clip_operator_cumulative_reward = np.zeros(self.num_clip_operators)
        self.clip_operator_decay_sum = np.zeros(self.num_clip_operators)
        
        # Track current clip operator
        self.current_clip_operator = 0
        self.clip_selection_history = []
    ...
    def _select_clip_operator_thompson(self):
        ...
        self.current_clip_operator = int(np.argmax(samples))
        self.clip_operator_counts[self.current_clip_operator] += 1
        self.clip_selection_history.append(self.current_clip_operator)
        return self.current_clip_operator
    
    def _update_clip_operator_rewards(self, improvement, violation_detected):
        """Credit assignment for clip operators based on optimization progress.
        
        Args:
            improvement: float, improvement in f_opt since last update
            violation_detected: bool, whether any boundary violations were handled
        """
        op = self.current_clip_operator
        
        # Compute reward based on clip operator performance
        if improvement > 0:
            # Positive reward for helping achieve improvement
            reward = float(np.clip(np.log1p(improvement * 1e10) / 15.0, 0.0, 2.0))
        else:
            # Small negative reward for no improvement
            reward = -0.1
        
        # Bonus for handling violations efficiently (not too aggressive)
        if violation_detected:
            # Check if the clip was gentle (small adjustments)
            reward += 0.05
        
        reward = float(np.clip(reward, -1.0, 2.5))
        
        # Apply exponential decay to cumulative tracking
        decay = 0.90
        ...
        normalized_reward = float(np.clip(normalized_reward, -3.0, 3.0))
        
        # Store in sliding window
        self.clip_operator_rewards[op].append(normalized_reward)
        if len(self.clip_operator_rewards[op]) > self.clip_reward_window_size:
            self.clip_operator_rewards[op].pop(0)
        
        # Update Beta distribution parameters
        n = len(self.clip_operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.clip_operator_rewards[op]))
            var_reward = float(np.var(self.clip_operator_rewards[op]))
            ...
            sum_reward = float(np.sum(self.clip_operator_rewards[op]))
            if sum_reward > 0:
                self.alpha_clip[op] = 1.0 + sum_reward
            else:
                self.beta_clip[op] = 1.0 - sum_reward
    
    def _clip_to_bounds_basic(self, x):
        """Basic numpy clip to bounds."""
        x = np.asarray(x, dtype=np.float64)
        return np.clip(x, self.lb, self.ub)
    
    def _clip_to_bounds_reflection(self, x):
        """Clip solution to bounds using smooth reflection (variant_02 style)."""
        x = np.asarray(x, dtype=np.float64)
    ...
    def _clip_to_bounds_reflection_noise(self, x):
        """Reflective boundary handling with adaptive noise injection (variant_04 style)."""
        x = np.asarray(x, dtype=np.float64)
    ...
    def _clip_to_bounds_cyclic(self, x):
        """Clip solution to bounds using cyclic wrapping (variant_09 style)."""
        bound_range = self.ub - self.lb
    ...
    def _clip_to_bounds(self, x):
        """Adaptive clip to bounds - dispatch to selected operator."""
        if self.current_clip_operator == 0:
            return self._clip_to_bounds_basic(x)
        elif self.current_clip_operator == 1:
            return self._clip_to_bounds_reflection(x)
        elif self.current_clip_operator == 2:
            return self._clip_to_bounds_reflection_noise(x)
        else:
            return self._clip_to_bounds_cyclic(x)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            # Select clip operator at the start of each generation
            ...
            # Track if we had improvements and violations for clip reward update
            f_opt_before_clip_update = float(self.f_opt)
            ...
            # Update clip operator rewards based on optimization progress
            improvement = max(0.0, f_opt_before_clip_update - self.f_opt)
            # Check if any boundary violations were handled in this generation
            violation_detected = len(self.clip_selection_history) > 0 and self.clip_selection_history[-1] in [1, 2]
            self._update_clip_operator_rewards(improvement, violation_detected)
