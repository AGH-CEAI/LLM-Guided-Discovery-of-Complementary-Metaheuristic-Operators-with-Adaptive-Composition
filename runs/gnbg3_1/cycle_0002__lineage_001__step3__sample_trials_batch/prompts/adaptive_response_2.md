...
class AdaptiveCovarianceEvolutionStrategy:
    """
    Adaptive optimizer with adaptive sampling strategy selection.
    
    Features:
    - 7 covariance sampling strategies (original + 6 variants)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Sampling operator selection (7 operators)
        self.num_sampling_operators = 7
        self.sampling_operator_names = [
            'original', 'variant_01', 'variant_03', 'variant_04',
            'variant_05', 'variant_06', 'variant_09'
        ]
        
        # Thompson Sampling for sampling operators
        self.sampling_alpha = np.ones(self.num_sampling_operators)
        self.sampling_beta = np.ones(self.num_sampling_operators)
        
        # Sliding window for credit assignment
        self.reward_window_size = 10
        self.sampling_operator_rewards = {i: [] for i in range(self.num_sampling_operators)}
        self.sampling_operator_counts = np.zeros(self.num_sampling_operators)
        self.sampling_selection_counts = np.zeros(self.num_sampling_operators)
        
        # Runtime state
        self.generation = 0
        self.current_sampling_operator = 0
        self.L = None
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            self._select_sampling_operator_thompson()
            ...
            # Update sampling operator rewards based on improvement
            self._update_sampling_operator_rewards()
    ...
    def _initialize_population(self):
        ...
        self._reset_sampling_operator_state()
    
    def _reset_sampling_operator_state(self):
        """Reset all sampling operator-specific state variables."""
        self.L = None
        ...
        if hasattr(self, '_best_at_last_improvement'):
            delattr(self, '_best_at_last_improvement')
        if hasattr(self, '_gen_at_last_improvement'):
            delattr(self, '_gen_at_last_improvement')
    ...
    def _select_sampling_operator_thompson(self):
        """Select sampling operator using Thompson Sampling from Beta distributions."""
        samples = np.random.beta(self.sampling_alpha, self.sampling_beta)
        self.current_sampling_operator = int(np.argmax(samples))
        self.sampling_operator_counts[self.current_sampling_operator] += 1
        return self.current_sampling_operator
    
    def _update_sampling_operator_rewards(self):
        ...
        op = self.current_sampling_operator
        self.sampling_operator_rewards[op].append(reward)
        
        if len(self.sampling_operator_rewards[op]) > self.reward_window_size:
            self.sampling_operator_rewards[op].pop(0)
        
        n = len(self.sampling_operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.sampling_operator_rewards[op]))
            var_reward = float(np.var(self.sampling_operator_rewards[op]))
            
            denom = max(var_reward * n + 1e-10, 1e-10)
            self.sampling_alpha[op] = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            self.sampling_beta[op] = float(max(1.0, (1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.sampling_operator_rewards[op]))
            if sum_reward > 0:
                self.sampling_alpha[op] = 1.0 + sum_reward
            else:
                self.sampling_beta[op] = 1.0 - sum_reward
    
    def _sample_trials_batch(self):
        """Dispatch to the selected sampling strategy."""
        if self.current_sampling_operator == 0:
            self._sample_trials_original()
        elif self.current_sampling_operator == 1:
            self._sample_trials_variant_01()
        elif self.current_sampling_operator == 2:
            self._sample_trials_variant_03()
        elif self.current_sampling_operator == 3:
            self._sample_trials_variant_04()
        elif self.current_sampling_operator == 4:
            self._sample_trials_variant_05()
        elif self.current_sampling_operator == 5:
            self._sample_trials_variant_06()
        else:
            self._sample_trials_variant_09()
    ...
    def _sample_trials_variant_09(self):
    ...
    def _adapt_covariance(self):
        """Default covariance adaptation (original)."""
