...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    AND initialization strategy during optimization using Thompson Sampling with sliding window credit assignment.
    
    Features:
    - 5 covariance adaptation strategies
    - 7 initialization strategies (adaptively selected based on benchmark results)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Sliding window for credit assignment (covariance operators)
        self.reward_window_size = 10
        self.operator_rewards = {i: [] for i in range(self.num_operators)}
        self.operator_counts = np.zeros(self.num_operators)
        self.selection_counts = np.zeros(self.num_operators)
        
        # Adaptive initialization operator selection (7 winning strategies)
        self.num_init_operators = 7
        self.init_operator_names = [
            'init_original', 'init_v01_sobol', 'init_v02_lhs_directional',
            'init_v04_basin_seeking', 'init_v06_multi_scale', 
            'init_v07_sobol_fitness', 'init_v09_archive_guided'
        ]
        ...
        # Initialization operator tracking
        ...
        self.best_fitness_at_init = None
    ...
    def _update_init_operator_rewards(self):
        """Credit assignment for initialization operators based on fitness improvement."""
        if self.best_fitness_at_init is None:
            return
            
        # Compute improvement since initialization
        improvement = self.best_fitness_at_init - self.f_opt
        ...
        # Compute reward: log-scaled improvement normalized by generations since init
        if improvement > 0:
            gens_since_init = max(1, self.generation - self.init_start_generation)
            reward = float(np.log1p(improvement * 1e8) / max(10.0, gens_since_init))
            reward = float(np.clip(reward, 0.0, 5.0))
        else:
            reward = float(np.clip(improvement * 0.1, -1.0, 0.0))
        
        op = self.current_init_operator
        self.init_operator_cumulative_reward[op] += reward
        self.init_operator_decay_sum[op] += 1.0
        
        # Normalize reward
        norm = max(self.init_operator_decay_sum[op], 1.0)
        normalized_reward = self.init_operator_cumulative_reward[op] / norm
        ...
        if n >= 3:
            ...
            mean_reward = float(np.clip(mean_reward, 0.0, 1.0))
            var_reward = float(np.var(self.init_operator_rewards[op]))
            var_reward = float(np.clip(var_reward, 1e-10, 1.0))
            ...
            sum_reward = float(np.clip(sum_reward, -10.0, 10.0))
            if sum_reward > 0:
    ...
    def _initialize_population(self):
        """Initialize population using adaptively selected strategy via Thompson Sampling."""
        # Select initialization operator using Thompson Sampling
        self._select_init_operator_thompson()
        self.best_fitness_at_init = self.f_opt if hasattr(self, 'f_opt') and self.f_opt is not None else float('inf')
        self.init_start_generation = self.generation
        
        # Dispatch to selected initialization method
    ...
    def _initialize_population_variant_02(self):
        ...
        self.population = np.clip(samples, self.lb, self.ub)
        self.mean = np.mean(self.population, axis=0)
        self.old_mean = self.mean.copy()
    ...
    def _initialize_population_variant_04(self):
        ...
        inv_fitness = np.array([1.0 / (max(float(f), 1e-10)) for f in cluster_best_fitness])
    ...
    def _restart_if_needed(self):
        ...
        # Update initialization operator reward before restart
        ...
        if (self.stagnation_counter > self.max_stagnation or 
            ...
            elite_fit = float(np.asarray(self.fitness[best_idx]).flatten()[0])
