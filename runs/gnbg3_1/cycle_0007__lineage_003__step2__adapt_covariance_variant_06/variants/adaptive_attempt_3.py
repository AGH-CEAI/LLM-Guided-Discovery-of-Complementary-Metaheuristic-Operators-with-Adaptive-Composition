...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 8 covariance adaptation strategies (original + 7 benchmarked variants)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_attempts = np.zeros(self.num_operators)
    ...
    def _reset_operator_state(self):
        ...
        # Reset pool-based variant states
    ...
    def _update_operator_rewards(self):
        ...
        pop_variance = np.mean(np.var(self.population, axis=0))
        expected_var = ((self.ub[0] - self.lb[0]) / 6.0) ** 2
        diversity = np.clip(float(pop_variance) / (float(expected_var) + 1e-10), 0.0, 1.0)
        ...
        # Track success for this operator
        if improvement > 1e-12:
            self.operator_successes[op] += 1.0
        self.operator_attempts[op] += 1.0
        ...
        # Boost alpha for operators that showed good benchmark performance
        if self.generation < 5:
            benchmark_wins = [0, 1, 3, 2, 2, 7, 1, 4]  # Based on provided benchmark results
            win_bonus = benchmark_wins[op] * 0.5
            self.alpha[op] += win_bonus
