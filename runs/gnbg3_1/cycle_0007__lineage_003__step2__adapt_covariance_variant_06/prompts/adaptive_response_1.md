...
class AdaptiveCovarianceEvolutionStrategy:
    """
    Adaptive CMA-ES optimizer that automatically selects the best covariance adaptation
    strategy during optimization using Thompson Sampling with sliding window credit assignment.
    
    Features:
    - 8 covariance adaptation strategies (from benchmark analysis)
    - Thompson Sampling for operator selection
    - Sliding window credit assignment with diversity bonus
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.L = None
        self.run_stats = {'improvements': 0, 'restarts': 0}
    ...
    def _select_operator_thompson(self):
        ...
        alpha_safe = np.maximum(self.alpha, 0.1)
        beta_safe = np.maximum(self.beta, 0.1)
    ...
    def _update_operator_rewards(self):
        ...
        # Compute reward with improvement and diversity bonus
        reward = float(np.log1p(improvement * 1e10) / 10.0 + 0.2 * diversity)
        reward = float(np.clip(reward, -10.0, 10.0))
        
        op = self.current_operator
        self.operator_rewards[op].append(reward)
        
        # Track fitness history for each operator
        current_fitness = self.f_opt
        self.operator_fitness_history[op].append(current_fitness)
        ...
        # Apply small boost to operators with recent best fitness
        for i in range(self.num_operators):
            if len(self.operator_fitness_history[i]) >= 3:
                recent_best = min(self.operator_fitness_history[i][-3:])
                if recent_best == self.f_opt and self.f_opt < self.f_opt_prev:
                    self.alpha[i] *= 1.05
                    self.alpha[i] = float(np.clip(self.alpha[i], 0.1, 100.0))
