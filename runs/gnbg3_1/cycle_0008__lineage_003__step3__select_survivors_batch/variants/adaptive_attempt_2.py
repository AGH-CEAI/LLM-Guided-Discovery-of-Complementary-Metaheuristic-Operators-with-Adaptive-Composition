...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 4 survivor selection strategies (original + variants 05, 07, 09)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection for survivor selection
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Select and apply survivor selection operator BEFORE selection
    ...
    def _update_operator_rewards(self):
        ...
        reward = 0.0
        try:
            reward = float(np.log1p(float(improvement) * 1e10) / 10.0)
            if not np.isfinite(reward):
                reward = 0.0
        except (ValueError, OverflowError):
            reward = 0.0
        
        reward = float(reward + 0.1 * float(diversity))
