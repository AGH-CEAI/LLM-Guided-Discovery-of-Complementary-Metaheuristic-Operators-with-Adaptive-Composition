...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 4 survivor selection strategies (original + 3 variants based on benchmark results)
    ...
    def _adapt_step_size(self):
        ...
        success_rate = float(np.clip(float(recent_improved) / float(recent_total), 0.0, 1.0))

        avg_improvement = total_improvement / max(float(recent_improved), 1)
        improvement_magnitude = float(np.log1p(max(float(avg_improvement), 1e-15)))

        if not hasattr(self, 'improvement_ema'):
            self.improvement_ema = float(improvement_magnitude)
        self.improvement_ema = float(0.7 * self.improvement_ema + 0.3 * improvement_magnitude)

        target_rate = 0.25
        adaptation = (success_rate - target_rate) / target_rate
        adaptation += 0.3 * float(np.tanh(float(self.improvement_ema - 1.0)))
        adaptation = float(np.clip(adaptation, -0.8, 0.8))

        pop_variance = float(np.mean(np.var(self.population, axis=0)))
        expected_var = ((float(self.ub[0]) - float(self.lb[0])) / 6.0) ** 2
        ...
        damping_adaptive = float(np.clip(self.damping * (0.5 + 0.5 * float(np.log1p(self.dim))) / diversity_boost, 0.05, 200.0))
    ...
    def _update_operator_rewards(self):
        ...
        reward = float(np.clip(float(np.log1p(improvement * 1e10)) / 10.0 + 0.1 * diversity, -10.0, 10.0))
        ...
        if n >= 3:
            ...
            denom = float(np.clip(var_reward * n + 1e-10, 1e-10, None))
            self.alpha[op] = float(np.clip(mean_reward * (mean_reward * (n - 1) / denom + 1), 1.0, None))
            self.beta[op] = float(np.clip((1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1), 1.0, None))
        elif n >= 1:
            sum_reward = float(np.sum(self.operator_rewards[op]))
            if sum_reward > 0:
                self.alpha[op] = float(np.clip(1.0 + sum_reward, 1.0, None))
            else:
                self.beta[op] = float(np.clip(1.0 - sum_reward, 1.0, None))
