...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - 4 survivor selection strategies (original + 3 winning variants)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Survivor selection operator selection (NEW)
        ...
        # Sliding window for survivor selection credit assignment
        self.survivor_reward_window_size = 10
        self.survivor_operator_rewards = {i: [] for i in range(self.num_survivor_operators)}
        self.survivor_operator_counts = np.zeros(self.num_survivor_operators)
        self.survivor_selection_counts = np.zeros(self.num_survivor_operators)
        self.current_survivor_operator = 0
        
        # Track per-generation improvements for survivor operator reward
        self._prev_gen_best_fitness = None
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Store fitness before survivor selection to measure improvement
            self._prev_gen_best_fitness = self.f_opt
            
            # Select and apply survivor selection operator (ADAPTIVE)
    ...
    def _select_survivor_operator_thompson(self):
        ...
        self.survivor_operator_counts[self.current_survivor_operator] += 1
    ...
    def _update_survivor_operator_rewards(self):
        ...
        if self._prev_gen_best_fitness is None:
            return
            
        improvement = max(0.0, self._prev_gen_best_fitness - self.f_opt)
        ...
        # Compute reward combining improvement and diversity
        reward = float(np.log1p(improvement * 1e10) / 10.0 + 0.1 * diversity)
        reward = float(np.clip(reward, -10.0, 10.0))
        
        op = self.current_survivor_operator
        self.survivor_operator_rewards[op].append(reward)
        
        # Maintain sliding window
        if len(self.survivor_operator_rewards[op]) > self.survivor_reward_window_size:
            self.survivor_operator_rewards[op].pop(0)
        
        # Update Beta distribution parameters
        n = len(self.survivor_operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.survivor_operator_rewards[op]))
            var_reward = float(np.var(self.survivor_operator_rewards[op]))
            
            denom = max(var_reward * n + 1e-10, 1e-10)
            self.survivor_alpha[op] = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            self.survivor_beta[op] = float(max(1.0, (1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.survivor_operator_rewards[op]))
            if sum_reward > 0:
                self.survivor_alpha[op] = 1.0 + sum_reward
            else:
                self.survivor_beta[op] = 1.0 - sum_reward
    ...
    def _select_survivors_original(self):
        """Select survivors via elitist (mu, lambda)-selection (baseline)."""
    ...
    def _select_survivors_variant_03(self):
        """Select survivors via deterministic crowding with adaptive diversity threshold."""
        combined_pop = np.vstack([self.population, self.trials])
        combined_fit = np.concatenate([self.fitness, self.trial_fitness])
    ...
    def _select_survivors_variant_04(self):
        ...
        if num_trials == 0 or len(self.population) == 0:
            ...
            # Sort trials by fitness (best first)
