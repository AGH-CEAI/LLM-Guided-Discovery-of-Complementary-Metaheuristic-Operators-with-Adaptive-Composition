...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    AND survivor selection strategy during optimization using Thompson Sampling
    with sliding window credit assignment.
    
    Features:
    - 5 covariance adaptation strategies
    - 4 survivor selection strategies (adaptive)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Thompson Sampling for survivor selection
        self.s_alpha = np.ones(self.num_survivor_operators)
        self.s_beta = np.ones(self.num_survivor_operators)
        
        # Sliding window for survivor credit assignment
        self.s_reward_window_size = 10
        self.s_operator_rewards = {i: [] for i in range(self.num_survivor_operators)}
        self.s_operator_counts = np.zeros(self.num_survivor_operators)
        self.s_selection_counts = np.zeros(self.num_survivor_operators)
        ...
        # Pre-selection fitness for reward computation
        self._pre_selection_fitness = None
        self._pre_selection_population = None
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            # Store pre-selection state for reward computation
            self._pre_selection_population = self.population.copy()
            self._pre_selection_fitness = self.fitness.copy()
            self._pre_selection_best = self.f_opt
            ...
            # Update survivor operator rewards based on selection quality
    ...
    def _select_survivor_operator_thompson(self):
        ...
        samples = np.random.beta(self.s_alpha, self.s_beta)
        self.current_survivor_operator = int(np.argmax(samples))
        self.s_operator_counts[self.current_survivor_operator] += 1
    ...
    def _update_survivor_operator_rewards(self):
        ...
        # Compute reward based on selection quality
        # Compare best fitness before and after selection
        best_before = self._pre_selection_best
        best_after = self.f_opt
        
        # Improvement in best solution
        improvement = max(0.0, best_before - best_after)
        
        # Compute diversity after selection
        pop_variance = np.mean(np.var(self.population, axis=0))
        expected_var = ((self.ub[0] - self.lb[0]) / 6.0) ** 2
        diversity = float(np.clip(pop_variance / (expected_var + 1e-10), 0.0, 1.0))
        
        # Compute mean fitness improvement
        mean_fit_before = float(np.mean(self._pre_selection_fitness))
        mean_fit_after = float(np.mean(self.fitness))
        mean_improvement = max(0.0, mean_fit_before - mean_fit_after)
        
        # Reward combines best improvement and diversity preservation
        reward = 0.7 * float(np.log1p(improvement * 1e10) / 10.0)
        reward += 0.3 * diversity
        reward += 0.1 * float(np.log1p(mean_improvement * 1e5) / 10.0)
        reward = float(np.clip(reward, -10.0, 10.0))
        
        op = self.current_survivor_operator
        self.s_operator_rewards[op].append(reward)
        
        if len(self.s_operator_rewards[op]) > self.s_reward_window_size:
            self.s_operator_rewards[op].pop(0)
        
        # Update Beta distribution parameters
        n = len(self.s_operator_rewards[op])
        if n >= 3:
            mean_reward = float(np.mean(self.s_operator_rewards[op]))
            var_reward = float(np.var(self.s_operator_rewards[op]))
            
            denom = max(var_reward * n + 1e-10, 1e-10)
            self.s_alpha[op] = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            self.s_beta[op] = float(max(1.0, (1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.s_operator_rewards[op]))
            if sum_reward > 0:
                self.s_alpha[op] = float(max(1.0, 1.0 + sum_reward))
            else:
                self.s_beta[op] = float(max(1.0, 1.0 - sum_reward))
    ...
    def _select_survivors_variant_04(self):
        ...
        if num_trials == 0 or len(self.population) == 0:
            ...
            sorted_indices = np.argsort(combined_fit)
            self.population = combined_pop[sorted_indices[:self.NP]]
            self.fitness = np.array([float(x) for x in combined_fit[sorted_indices[:self.NP]]])
        else:
            # Compute pairwise distances between trials and population
            trials_exp = self.trials[:, np.newaxis, :]  # (num_trials, 1, dim)
            parents_exp = self.population[np.newaxis, :, :]  # (1, pop_size, dim)
            diffs = trials_exp - parents_exp
            dists = np.sum(diffs * diffs, axis=2)  # squared distances
            ...
            if len(candidates_pop) > pop_size:
                ...
                # If fewer candidates, add random individuals from combined population
    ...
    def _select_survivors_variant_06(self):
        ...
        if len(new_pop) < self.NP:
            ...
            new_fit = np.concatenate([new_fit, [float(x) for x in padding_fit]])

        self.population = new_pop[:self.NP]
        self.fitness = np.array([float(x) for x in new_fit[:self.NP]])
    ...
    def _restart_if_needed(self):
        ...
        if (self.stagnation_counter > self.max_stagnation or 
            ...
            self.fitness[0] = float(elite_fit)
