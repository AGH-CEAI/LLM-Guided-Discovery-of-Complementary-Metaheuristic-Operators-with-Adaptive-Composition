...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    - 6 restart strategies (original + 5 best-performing variants)
    - Thompson Sampling for operator selection on both strategy types
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # ==== Adaptive operator selection for restart strategies ====
        self.num_restart_operators = 6
        self.restart_operator_names = ['original', 'variant_02', 'variant_03', 'variant_04', 'variant_05', 'variant_06']
        
        # Thompson Sampling with Beta distributions for restart
        self.restart_alpha = np.ones(self.num_restart_operators)
        self.restart_beta = np.ones(self.num_restart_operators)
        
        # Sliding window for credit assignment (restart)
        self.restart_reward_window_size = 10
        ...
        self.restart_cumulative_reward = np.zeros(self.num_restart_operators)
        self.restart_decay_sum = np.zeros(self.num_restart_operators)
        ...
        # Track restart performance history
        self.restart_history = []  # List of (operator, improvement, success) tuples
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Check if restart is needed and select restart strategy
            if self._should_restart():
                self._select_restart_operator_thompson()
                f_opt_before_restart = self.f_opt
                self._apply_restart_strategy()
                # Update restart operator rewards based on improvement
                self._update_restart_operator_rewards(f_opt_before_restart)
    ...
    def _update_restart_operator_rewards(self, f_opt_before_restart):
        """Credit assignment for restart operators based on post-restart improvement."""
        improvement = max(0.0, f_opt_before_restart - self.f_opt)
        
        # Also check if we escaped a bad state (diversity improvement)
        diversity_before = getattr(self, 'diversity_before_restart', 0.5)
        diversity_after = self._compute_diversity()
        diversity_gain = max(0.0, diversity_after - diversity_before)
        
        # Combined reward: fitness improvement + diversity gain
        if improvement > 0:
            fitness_reward = float(np.log1p(improvement * 1e8) / 8.0)
        else:
            fitness_reward = -0.1  # Small penalty for no improvement
        
        diversity_reward = float(np.tanh(diversity_gain * 10.0) * 0.5)
        reward = fitness_reward + diversity_reward
        
        op = self.current_restart_operator
        
        # Apply decay
        decay = 0.9
        self.restart_cumulative_reward *= decay
        self.restart_decay_sum *= decay
        
        self.restart_cumulative_reward[op] += reward
        self.restart_decay_sum[op] += 1.0
        
        # Normalize reward
        norm = max(self.restart_decay_sum[op], 1.0)
        normalized_reward = self.restart_cumulative_reward[op] / norm
        ...
        if n >= 3:
            ...
            denom = max(var_reward * n + 1e-10, 1e-10)
            # Map reward from [-5, 5] to [0, 1] for Beta
            p = (mean_reward + 5.0) / 10.0
            p = float(np.clip(p, 0.01, 0.99))
            self.restart_alpha[op] = float(max(1.0, p * (p * (1 - p) * (n - 1) / denom + 1)))
            self.restart_beta[op] = float(max(1.0, (1 - p) * ((1 - p) * p * (n - 1) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.restart_operator_rewards[op]))
            if sum_reward > 0:
                self.restart_alpha[op] = 1.0 + sum_reward * 0.5
            else:
                self.restart_beta[op] = 1.0 - sum_reward * 0.5
    ...
    def _apply_restart_strategy(self):
        """Dispatch to the selected restart strategy."""
        # Store diversity before restart for reward calculation
        self.diversity_before_restart = self._compute_diversity()
        self.restart_counter += 1
        
        if self.current_restart_operator == 0:
            self._restart_if_needed_original()
        elif self.current_restart_operator == 1:
            self._restart_if_needed_variant_02()
        elif self.current_restart_operator == 2:
            self._restart_if_needed_variant_03()
        elif self.current_restart_operator == 3:
            self._restart_if_needed_variant_04()
        elif self.current_restart_operator == 4:
            self._restart_if_needed_variant_05()
        else:
            self._restart_if_needed_variant_06()
    ...
    def _restart_if_needed_original(self):
    ...
    def _restart_if_needed_variant_02(self):
    ...
    def _restart_if_needed_variant_03(self):
    ...
    def _restart_if_needed_variant_04(self):
    ...
    def _restart_if_needed_variant_05(self):
    ...
    def _restart_if_needed_variant_06(self):
