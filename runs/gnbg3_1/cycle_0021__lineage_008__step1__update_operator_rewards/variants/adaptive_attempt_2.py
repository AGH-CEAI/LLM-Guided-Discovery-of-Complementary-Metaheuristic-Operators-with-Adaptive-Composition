...
class AdaptiveCovarianceEvolutionStrategy:
    """
    Adaptive optimizer with META-ADAPTIVE operator reward strategy selection.
    
    Features:
    - 5 covariance adaptation strategies
    - 5 operator reward strategies (top performers from benchmarks)
    - Meta-level Thompson Sampling to select best reward strategy per-task
    - Automatic strategy switching during optimization
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # META-ADAPTIVE: Reward strategy selection (5 top performers)
        self.num_reward_strategies = 5
        self.reward_strategy_names = [
            'strategy_02',   # Rank-based multi-dimensional (5 wins)
            'strategy_10',   # Elite-relative progress (3 wins)
            'strategy_07',   # Multi-component credit (3 wins)
            'strategy_04',   # Rank-based generational (2 wins)
            'strategy_08',   # Comparative rank-based (3 wins)
        ]
        
        # Meta-level Beta distributions for reward strategy selection
        ...
        self.reward_strategy_scores = {i: [] for i in range(self.num_reward_strategies)}
        self.reward_strategy_switch_count = np.zeros(self.num_reward_strategies)
        self.current_reward_strategy = 0
        self.meta_generation = 0
        
        # Meta-level adaptation parameters
        self.meta_window_size = 15  # Window for meta-level performance tracking
        self.meta_switch_threshold = 5  # Generations before allowing switch
        self.meta_min_improvement = 1e-10
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Meta-adaptive: Select and apply operator reward strategy
            self._select_reward_strategy_meta()
            self._update_operator_rewards()
            self._update_meta_reward_strategy_performance()
    ...
    def _adapt_step_size(self):
        """Adapt step-size using momentum-enhanced improvement tracking."""
        recent_improved = 0
        recent_total = 0
    ...
    def _select_reward_strategy_meta(self):
        """Meta-level Thompson Sampling to select reward strategy."""
        # Use meta-level Thompson Sampling with slight exploration bonus
        samples = np.random.beta(self.meta_alpha, self.beta)
        
        # Add small exploration bonus to underrepresented strategies
        for i in range(self.num_reward_strategies):
            trials_i = self.meta_alpha[i] + self.beta[i]
            if trials_i < 5.0:
                samples[i] += 0.1 * (5.0 - trials_i) / 5.0
        
        prev_strategy = self.current_reward_strategy
        self.current_reward_strategy = int(np.argmax(samples))
        
        if prev_strategy != self.current_reward_strategy:
            self.reward_strategy_switch_count[self.current_reward_strategy] += 1
    
    def _update_meta_reward_strategy_performance(self):
        """Update meta-level Beta distributions based on reward strategy performance."""
        # Compute improvement signal
        improvement = max(0.0, self.f_opt_prev - self.f_opt)
        
        # Also consider diversity contribution
        pop_variance = np.mean(np.var(self.population, axis=0))
        expected_var = ((self.ub[0] - self.lb[0]) / 6.0) ** 2
        diversity = np.clip(pop_variance / (expected_var + 1e-10), 0.0, 1.0)
        
        # Combined signal: improvement + diversity bonus
        signal = improvement + 0.01 * diversity
        
        # Track score for current strategy
        self.reward_strategy_scores[self.current_reward_strategy].append(float(signal))
        
        # Keep window bounded
        if len(self.reward_strategy_scores[self.current_reward_strategy]) > self.meta_window_size:
            self.reward_strategy_scores[self.current_reward_strategy].pop(0)
        
        # Update meta-level Beta distributions
        if len(self.reward_strategy_scores[self.current_reward_strategy]) >= 3:
            scores = self.reward_strategy_scores[self.current_reward_strategy]
            mean_score = float(np.mean(scores))
            var_score = float(np.var(scores)) + 1e-10
            
            # Normalize to [0, 1] range approximately
            normalized = float(np.clip(mean_score / (np.sqrt(var_score) + 1e-10), 0.0, 1.0))
            
            # Update Beta parameters
            self.meta_alpha[self.current_reward_strategy] = max(1.0, normalized * 10.0)
            self.meta_beta[self.current_reward_strategy] = max(1.0, (1.0 - normalized) * 10.0)
        
        # Penalize strategies that haven't improved in a while
        if self.stagnation_counter > self.max_stagnation // 2:
            # Boost exploration of different strategies
            for i in range(self.num_reward_strategies):
                if i != self.current_reward_strategy:
                    self.meta_alpha[i] += 0.5
                    self.meta_beta[i] += 0.5
    
    def _update_operator_rewards(self):
        """Dispatch to selected reward strategy."""
        if self.current_reward_strategy == 0:
            self._update_reward_strategy_02()
        elif self.current_reward_strategy == 1:
            self._update_reward_strategy_10()
        elif self.current_reward_strategy == 2:
            self._update_reward_strategy_07()
        elif self.current_reward_strategy == 3:
            self._update_reward_strategy_04()
        else:
            self._update_reward_strategy_08()
    
    def _update_reward_strategy_02(self):
        """Rank-based multi-dimensional credit assignment (5 wins)."""
    ...
    def _update_reward_strategy_10(self):
        """Elite-relative progress (3 wins)."""
    ...
    def _update_reward_strategy_07(self):
        """Multi-component credit assignment (3 wins)."""
        if not hasattr(self, 'operator_cumulative_reward'):
            self.operator_cumulative_reward = np.zeros(self.num_operators)
    ...
    def _update_reward_strategy_04(self):
        """Rank-based generational memory (2 wins)."""
        if not hasattr(self, 'operator_cumulative_reward'):
            self.operator_cumulative_reward = np.zeros(self.num_operators)
        if not hasattr(self, 'operator_decay_sum'):
            self.operator_decay_sum = np.zeros(self.num_operators)
    ...
    def _update_reward_strategy_08(self):
        """Comparative rank-based multi-objective (3 wins)."""
        ...
        streak_bonus = float(np.tanh(self.operator_streak[op] / 10.0) * 0.5)
