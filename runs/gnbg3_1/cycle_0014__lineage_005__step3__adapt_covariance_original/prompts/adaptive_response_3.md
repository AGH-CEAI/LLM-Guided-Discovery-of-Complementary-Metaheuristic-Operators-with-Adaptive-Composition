...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 5 winning covariance adaptation strategies (based on benchmark results)
    ...
    def _initialize_population(self):
        ...
        if hasattr(self.fitness, '__len__') and len(self.fitness) > 0:
    ...
    def _reset_operator_state(self):
        ...
        for attr in ['momentum_ema', 'prev_f_opt_trap', 'stagnation_gen', 
                     'archive_positions', 'archive_fitness']:
    ...
    def _evaluate_batch(self):
        ...
        if hasattr(self.trial_fitness, '__len__') and len(self.trial_fitness) > 0:
    ...
    def _select_operator_thompson(self):
        ...
        # Ensure valid alpha/beta values
    ...
    def _update_operator_rewards(self):
        ...
        # Compute stagnation penalty
        stagnation_penalty = float(np.clip(self.stagnation_counter / self.max_stagnation, 0.0, 1.0))
        
        # Combined reward
        reward = float(np.log1p(improvement * 1e10) / 10.0)
        reward += 0.1 * diversity
        reward += 0.2 * cond_score
        reward -= 0.3 * stagnation_penalty
        reward = float(np.clip(reward, -10.0, 10.0))
        
        op = int(self.current_operator)
        rewards_list = self.operator_rewards[op]
        rewards_list.append(reward)
        
        if len(rewards_list) > self.reward_window_size:
            rewards_list.pop(0)
        ...
        if n >= 3:
            ...
            var_reward = float(np.var(rewards_list))
            
            # Normalize mean to [0, 1] range for Beta
            norm_mean = float(np.clip((mean_reward + 10.0) / 20.0, 0.01, 0.99))
            
            denom = max(var_reward * n + 1e-10, 1e-10)
            new_alpha = float(max(0.1, norm_mean * (norm_mean * (1.0 - norm_mean) / denom + 1.0)))
            new_beta = float(max(0.1, (1.0 - norm_mean) * ((1.0 - norm_mean) * (1.0 - norm_mean) / denom + 1.0)))
            
            # Blend with previous values for stability
            self.alpha[op] = 0.7 * self.alpha[op] + 0.3 * new_alpha
            self.beta[op] = 0.7 * self.beta[op] + 0.3 * new_beta
        elif n >= 1:
            sum_reward = float(np.sum(rewards_list))
            norm_sum = float(np.clip((sum_reward + 10.0 * n) / 20.0, 0.1, n))
            self.alpha[op] = float(max(0.1, 1.0 + norm_sum))
            self.beta[op] = float(max(0.1, 1.0 + n - norm_sum))
        
        # Store performance history
        self.performance_history[op].append(float(self.f_opt))
        if len(self.performance_history[op]) > 50:
            self.performance_history[op].pop(0)
        
        # Penalize operators that haven't improved in a while
        if len(self.performance_history[op]) >= 10:
            recent_best = min(self.performance_history[op][:5])
            recent_worst = max(self.performance_history[op][:5])
            if recent_best >= recent_worst - 1e-10:
                self.alpha[op] *= 0.95
    ...
    def _adapt_covariance_variant_08(self):
        ...
        y_momentum_norm = np.linalg.norm(self.momentum_ema)
        if y_momentum_norm > 1e-10:
            y_momentum = self.momentum_ema / y_momentum_norm * np.linalg.norm(y_mean)
        else:
            y_momentum = y_mean
    ...
    def _restart_if_needed(self):
        ...
        if (self.stagnation_counter > self.max_stagnation or 
            ...
            # Decay operator performance on restart
