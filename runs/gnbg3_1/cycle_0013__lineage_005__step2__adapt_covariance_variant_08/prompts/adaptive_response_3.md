...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 8 covariance adaptation strategies (original + 7 best-performing variants from benchmarks)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = ['original', 'variant_01', 'variant_02', 'variant_05', 
    ...
    def _adapt_step_size(self):
        ...
        self.improvement_ema = 0.7 * float(self.improvement_ema) + 0.3 * improvement_magnitude
    ...
    def _update_operator_rewards(self):
        ...
        improvement = float(max(0.0, float(self.f_opt_prev) - float(self.f_opt)))
        ...
        if n >= 3:
            ...
            denom = float(max(var_reward * n + 1e-10, 1e-10))
    ...
    def _adapt_covariance_variant_01(self):
        ...
        pos_sum = max(float(np.sum(self.weights[:self.mu][self.weights[:self.mu] > 0])), 1e-10)
        neg_sum = max(float(np.sum(np.abs(self.weights[:self.mu][self.weights[:self.mu] < 0]))), 1e-10)
    ...
    def _adapt_covariance_variant_02(self):
        ...
        if needs_rescue:
            log_cond = float(np.log10(cond + 1.0))
            rescue_strength = float(np.clip(0.5 + 0.5 * log_cond, 0.5, 5.0))
            ...
            improv_scale = float(np.clip(1.0 + 5.0 * float(np.log1p(improvement * 1e6)), 0.2, 3.0))

            cond_factor = float(np.clip(1.0 / float(np.log10(cond + 10.0)), 0.1, 1.0))

            cc_adapt = float(np.clip(self.cc * improv_scale * cond_factor, 0.001, 0.3))
            ...
            ccov_adapt = float(np.clip(self.ccov * improv_scale * cond_factor, 1e-10, 0.5))
            ...
            cond_check = float(np.max(eigvals_check)) / float(np.min(eigvals_check))
            if cond_check > 1e7:
                self.C *= 0.5
    ...
    def _adapt_covariance_variant_05(self):
        ...
        if is_stagnant:
            ...
            eigvals_check = np.linalg.eigvalsh(self.C)
            cond = float(np.max(eigvals_check)) / float(np.min(eigvals_check))
            if cond > 1e7:
                self.C *= 0.5
    ...
    def _adapt_covariance_variant_06(self):
        ...
        if is_converging:
            ...
            ccov_scale = float(np.clip(ccov_scale, 0.1, 10.0))

            cc_adapt = float(np.clip(self.cc * ccov_scale, 0.001, 0.5))
            self.pc = (1.0 - cc_adapt) * self.pc + np.sqrt(cc_adapt * (2.0 - cc_adapt)) * y_mean
            ...
            ccov_adapt = float(np.clip(self.ccov * ccov_scale, 1e-10, 0.5))
            self.C = ((1.0 - ccov_adapt) * self.C + 
                      ccov_adapt * rank_one +
                      (1.0 - 1.0 / self.mueff) * ccov_adapt * 2.0 * rank_mu)
    ...
    def _adapt_covariance_variant_07(self):
        ...
        multimodality_factor = float(np.clip(rel_var * 10.0, 0.0, 1.0))
        illcond_factor = float(np.clip(1.0 - eig_spread, 0.0, 1.0))
        trap_factor = 1.0 if is_trapped else 0.0

        mix = 0.05 + 0.15 * multimodality_factor + 0.30 * illcond_factor + 0.30 * trap_factor
        mix = float(np.clip(mix, 0.01, 0.7))
    ...
    def _adapt_covariance_variant_09(self):
        ...
        eigvals = np.linalg.eigvalsh(self.C)
        eig_min = max(float(np.min(eigvals)), 1e-15)
        ...
        adaptive_threshold = max(15, self.dim * 1.5) * (1.0 + 0.1 * float(np.log1p(cond)))
        ...
        if is_stagnant or is_converged:
            ...
            improv_scale = float(np.clip(improv_scale, 0.2, 10.0))

            cc_adapt = float(np.clip(self.cc * improv_scale, 0.001, 0.5))
            self.pc = (1.0 - cc_adapt) * self.pc + np.sqrt(cc_adapt * (2.0 - cc_adapt)) * y_mean
            ...
            ccov_adapt = float(np.clip(self.ccov * improv_scale, 1e-10, 0.6))
            self.C = ((1.0 - ccov_adapt) * self.C +
                      ccov_adapt * rank_one +
                      (1.0 - 1.0 / self.mueff) * ccov_adapt * 2.0 * rank_mu)
            ...
            eigvals_check = np.linalg.eigvalsh(self.C)
            cond_check = float(np.max(eigvals_check)) / max(float(np.min(eigvals_check)), 1e-15)
            if cond_check > 1e6:
                self.C *= 0.5
                self.C = self._ensure_positive_definite(self.C)
    ...
    def _adapt_covariance_variant_10(self):
        ...
        eigvals = np.linalg.eigvalsh(self.C)
        eig_min = max(float(np.min(eigvals)), 1e-10)
        eig_max = float(np.max(eigvals))
        cond = eig_max / eig_min
        ...
        restart_prob = 0.0
        if rel_var < 1e-4:
            restart_prob += 0.3 * max(0.0, 1.0 - rel_var / 1e-4)
        if cond > 1e3:
            restart_prob += 0.4 * max(0.0, 1.0 - float(np.log10(cond + 1)) / 7.0)
        if diversity < 1e-3:
            restart_prob += 0.3 * max(0.0, 1.0 - diversity / 1e-3)
        restart_prob *= float(np.clip(1.0 + float(np.log1p(abs(self.f_opt))), 0.1, 10.0))
