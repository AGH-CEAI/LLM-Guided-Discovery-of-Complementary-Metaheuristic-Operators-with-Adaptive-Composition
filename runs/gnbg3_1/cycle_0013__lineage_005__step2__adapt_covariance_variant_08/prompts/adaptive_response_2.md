...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 8 covariance adaptation strategies (original + 7 winning variants from benchmarks)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = [
            'original',      # 0: 3 benchmark wins
            'variant_01',    # 1: Active CMA-ES (4 wins)
            'variant_02',    # 2: Multi-trigger eigenspace (3 wins)
            'variant_05',    # 3: Forced perturbation (7 wins - most wins)
            'variant_06',    # 4: Fitness variance trigger (1 win)
            'variant_07',    # 5: Dual-path multimodal (1 win)
            'variant_09',    # 6: Diversity injection (3 wins)
            'variant_10'     # 7: Anticipatory restart (2 wins)
        ]
        ...
        # Reward history for exponential moving average per operator
    ...
    def _update_operator_rewards(self):
        ...
        if self.operator_ema[op] == 0.0:
            self.operator_ema[op] = reward
        else:
            self.operator_ema[op] = self.ema_alpha * reward + (1 - self.ema_alpha) * self.operator_ema[op]
        ...
        min_selections = np.min(self.selection_counts[self.selection_counts > 0]) if np.any(self.selection_counts > 0) else 0
        if self.selection_counts[op] < max(min_selections, 5):
            self.alpha[op] *= 1.1
            self.beta[op] *= 0.95
