...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 8 covariance adaptation strategies (from benchmark analysis)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = [
            ...
            'variant_05', 'variant_07', 'variant_08', 'variant_09'
        ]
        
        # Prior weights based on benchmark win counts
        benchmark_wins = [6, 1, 1, 4, 1, 5, 1, 5]
        total_wins = sum(benchmark_wins)
        self.prior_probs = [w / total_wins for w in benchmark_wins]
        
        # Thompson Sampling with Beta distributions (initialized with prior knowledge)
        self.alpha = np.array([1.0 + 2.0 * p for p in self.prior_probs])
        self.beta = np.array([1.0 + 1.0 * (1.0 - p) for p in self.prior_probs])
        ...
        self.cumulative_rewards = np.zeros(self.num_operators)
    ...
    def _reset_operator_state(self):
        ...
        for attr in ['recent_improvements', 'stagnation_levels', 'eigenspace_history',
                     'prev_f_opt_es', 'archive_positions', 'archive_fitness', 
                     'archive_generations', 'prev_fitness_improvements']:
    ...
    def _select_operator_thompson(self):
        ...
        # Add small exploration bonus for under-explored operators
        exploration_bonus = 0.1 * (1.0 - self.selection_counts / (np.sum(self.selection_counts) + 1e-10))
        samples = samples + exploration_bonus
    ...
    def _update_operator_rewards(self):
        """Update operator rewards using sliding window credit assignment with multiple metrics."""
        ...
        # Compute condition number for reward
        try:
        ...
        self.cumulative_rewards[op] += reward
