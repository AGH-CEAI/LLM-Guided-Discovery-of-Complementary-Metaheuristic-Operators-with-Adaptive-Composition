...
class AdaptiveCovarianceEvolutionStrategy:
    ...
    Features:
    - 5 covariance adaptation strategies (original + 4 winning variants)
    ...
    def _adapt_covariance_variant_01(self):
        """Active CMA-ES with negative weights for improved exploration (8 wins)."""
        y_mean = (self.mean - self.old_mean) / self.sigma
    ...
    def _adapt_covariance_variant_02(self):
        """Multi-direction ensemble covariance: blend diverse search directions (3 wins)."""
        y_mean = (self.mean - self.old_mean) / self.sigma
    ...
    def _adapt_covariance_variant_03(self):
        """Proactive stagnation-guided escaping with directional perturbations (4 wins)."""
        ...
        cc_adapt = float(np.clip(self.cc * cc_scale, 0.001, 0.5))
        ...
        for i in range(self.mu):
            if i >= len(self.population):
                break
            diff = (self.population[i] - self.old_mean) / self.sigma
            rank_mu += float(self.weights[i]) * np.outer(diff, diff)

        ccov_adapt = float(np.clip(self.ccov * ccov_scale, 1e-10, 0.6))
    ...
    def _adapt_covariance_variant_07(self):
        """Fitness gradient-directed covariance adaptation with stagnation escape (3 wins)."""
        y_mean = (self.mean - self.old_mean) / self.sigma

        fit_diff = np.array(self.fitness) - np.mean(self.fitness)
        fit_std = float(np.std(self.fitness))
        if fit_std > 1e-10:
            fit_diff = fit_diff / fit_std
        else:
        ...
        for i in range(min(self.mu, len(fit_diff))):
            if i >= len(self.population):
                break
            diff = (self.population[i] - self.old_mean) / self.sigma
            grad_matrix += float(fit_diff[i]) * np.outer(diff, diff)
        ...
        for j in range(self.dim):
            ...
            for i in range(min(self.mu, len(fit_diff))):
                if i >= len(self.population):
                    break
                diff = (self.population[i] - self.old_mean) / self.sigma
                proj += float(fit_diff[i]) * np.dot(eigvecs[:, j], diff)
            grad_proj[j] = proj
        ...
        grad_strength = float(np.linalg.norm(grad_proj) / np.sqrt(self.dim))
        cc_adapt = float(self.cc * np.clip(1.0 + 0.5 * np.tanh(grad_strength - 1.0), 0.01, 0.3))
        ...
        ccov_adapt = float(self.ccov * np.clip(1.0 + 0.5 * np.tanh(grad_strength - 0.5), 0.1, 5.0))
        ccov_adapt = float(np.clip(ccov_adapt, 1e-10, 0.5))
