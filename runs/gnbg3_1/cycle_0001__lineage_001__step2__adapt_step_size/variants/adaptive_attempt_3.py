...
class AdaptiveStepSizeEvolutionStrategy:
    """
    Adaptive optimizer that automatically selects the best step-size adaptation
    strategy during optimization using Thompson Sampling with sliding window credit assignment.
    
    Features:
    - 7 step-size adaptation strategies (based on benchmark winners)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.operator_names = ['original', 'variant_02', 'variant_04', 'variant_05', 
    ...
    def _initialize_population(self):
        ...
        if hasattr(self, 'success_history'):
            delattr(self, 'success_history')
        if hasattr(self, 'sigma_history'):
            delattr(self, 'sigma_history')
        if hasattr(self, '_step_history'):
            delattr(self, '_step_history')
    ...
    def _adapt_step_size_variant_02(self):
        ...
        for i in range(self.NP):
            if i < len(self.trial_fitness) and i < len(self.fitness):
                ...
                curr_fit = float(np.asarray(self.fitness[i]).flatten()[0])
    ...
    def _adapt_step_size_variant_08(self):
        """Adapt step-size using mirrored sampling variance ratio (fundamentally different from CSA) - 6 wins."""
        y_mean = (self.mean - self.old_mean) / self.sigma
    ...
    def _adapt_step_size_variant_09(self):
        """Multi-signal adaptive step-size with fitness landscape awareness - 1 win."""
        y_mean = (self.mean - self.old_mean) / self.sigma
