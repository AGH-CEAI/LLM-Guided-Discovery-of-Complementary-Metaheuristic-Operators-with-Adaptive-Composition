...
class AdaptiveStepSizeOptimizer:
    ...
    Features:
    - 6 step-size adaptation strategies (original + 5 variants from benchmark)
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Step-size operator selection
        self.num_step_operators = 6
        self.step_operator_names = ['original', 'variant_02', 'variant_04', 'variant_05', 'variant_06', 'variant_08']
        
        # Thompson Sampling with Beta distributions
        self.alpha_step = np.ones(self.num_step_operators)
        self.beta_step = np.ones(self.num_step_operators)
    ...
    def _ensure_positive_definite(self, C):
        ...
        if min_eigenvalue < 1e-10:
            C += (1e-7 - min_eigenvalue) * np.eye(self.dim)
        return C
    ...
    def _select_step_operator_thompson(self):
        ...
        samples = np.random.beta(self.alpha_step, self.beta_step)
    ...
    def _update_step_operator_rewards(self):
        ...
        if n >= 3:
            ...
            self.alpha_step[op] = float(max(1.0, mean_reward * (mean_reward * (n - 1) / denom + 1)))
            self.beta_step[op] = float(max(1.0, (1 - mean_reward) * ((n - 1) * (1 - mean_reward) / denom + 1)))
        elif n >= 1:
            sum_reward = float(np.sum(self.step_operator_rewards[op]))
            if sum_reward > 0:
                self.alpha_step[op] = 1.0 + sum_reward
            else:
                self.beta_step[op] = 1.0 - sum_reward
    ...
    def _adapt_step_size_variant_04(self):
        ...
        if stagnation:
            adaptive_damping = base_damping * 0.3
        elif diversity_ratio < 0.1:
            adaptive_damping = base_damping * 0.5
        elif diversity_ratio > 0.5:
            adaptive_damping = base_damping * 1.5
        else:
        ...
        if f_improved:
            fitness_adjustment = -0.1 / adaptive_damping
        else:
            fitness_adjustment = 0.05 / adaptive_damping

        if diversity_ratio < 0.05:
            diversity_adjustment = 0.2 / adaptive_damping
        elif diversity_ratio > 2.0:
            diversity_adjustment = -0.1 / adaptive_damping
        else:
    ...
    def _adapt_step_size_variant_06(self):
        ...
        for f in self.trial_fitness:
            if float(f) < self.f_opt_prev:
                n_successful += 1
        current_success_rate = n_successful / max(len(self.trial_fitness), 1)
        self._step_history.append(float(current_success_rate))
