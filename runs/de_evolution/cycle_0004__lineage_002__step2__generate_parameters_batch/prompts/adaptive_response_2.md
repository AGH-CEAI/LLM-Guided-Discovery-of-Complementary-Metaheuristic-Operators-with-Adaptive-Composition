...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.strategy_alpha = np.ones(self.num_strategies, dtype=float)
        self.strategy_beta = np.ones(self.num_strategies, dtype=float)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Compute reward before selection (number of improvements and their magnitude)
            pre_best = self.f_opt
            ...
            # Compute reward for the strategy
            reward = self._compute_strategy_reward(pre_best, success_delta)
            self._update_strategy_scores(self.current_strategy_idx, reward)
    ...
    def _compute_strategy_reward(self, pre_best, success_delta):
        """Compute reward for the current strategy based on improvement."""
        # Reward based on: (1) number of successful updates, (2) total improvement, (3) best improvement
        reward = 0.0
        if len(success_delta) > 0:
            total_delta = float(np.sum(success_delta))
            n_success = len(success_delta)
            # Normalize by population size
            success_rate = n_success / max(1, self.np_size)
            reward = success_rate
            # Bonus for improving global best
            if self.f_opt < pre_best:
                reward += 0.5
        return float(reward)

    def _update_strategy_scores(self, strategy_idx, reward):
        """Update Thompson Sampling parameters based on reward."""
        # Decay old observations slightly to adapt over time
        decay = 0.995
        ...
        # Update: treat reward > threshold as success
        if reward > 0.1:
        ...
        self.strategy_history.append((int(strategy_idx), float(reward)))
    ...
    def _generate_parameters_batch(self, memory_f, memory_cr):
        ...
        if idx == 0:
            ...
            return self._gen_params_variant04(memory_f, memory_cr)
        elif idx == 4:
            return self._gen_params_variant05(memory_f, memory_cr)
        elif idx == 5:
            return self._gen_params_variant06(memory_f, memory_cr)
        elif idx == 6:
    ...
    # Strategy 1: variant_01 (10 wins) - generation-adaptive scaling + exploration injection
    ...
    # Strategy 2: variant_02 (2 wins) - bimodal: aggressive explorers + adaptive exploiters
    ...
    # Strategy 3: variant_04 (1 win) - bimodal aggressive F + high CR
    def _gen_params_variant04(self, memory_f, memory_cr):
        n = self.np_size
        indices = np.random.randint(0, self.memory_size, size=n)
    ...
    # Strategy 4: variant_05 (2 wins) - bimodal distributions for enhanced exploration
    ...
    # Strategy 5: variant_06 (1 win) - bimodal regime mixing with overshooting
    def _gen_params_variant06(self, memory_f, memory_cr):
        n = self.np_size
        indices = np.random.randint(0, self.memory_size, size=n)
    ...
    # Strategy 6: variant_07 (1 win) - bimodal: aggressive explorers + local refiners
