...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.num_strategies = 5  # number of parameter generation strategies
        self.strategy_alpha = np.ones(self.num_strategies)  # Beta distribution alpha
        self.strategy_beta = np.ones(self.num_strategies)   # Beta distribution beta
        self.strategy_window_size = 50  # sliding window for credit assignment
        self.strategy_history = []  # list of (strategy_idx, reward) tuples
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            self.current_strategy_idx = self._select_strategy()
            ...
            # Compute reward before selection (number of improvements / fitness improvement)
            n_check = min(len(population), len(trials))
            improvements = 0
            total_improvement = 0.0
            for i in range(n_check):
                if i < len(valid) and valid[i] and trial_fitness[i] < fitness[i]:
                    improvements += 1
                    total_improvement += float(abs(fitness[i] - trial_fitness[i]))

            # Update Thompson Sampling with reward
            reward = float(improvements) / max(1, n_check)
            self._update_strategy_reward(self.current_strategy_idx, reward)
    ...
    def _select_strategy(self):
        """Select a parameter generation strategy using Thompson Sampling."""
        samples = np.array([
    ...
    def _update_strategy_reward(self, strategy_idx, reward):
        ...
        self.strategy_history.append((int(strategy_idx), reward))

        # Update Beta distribution parameters
        self.strategy_alpha[strategy_idx] += reward
        self.strategy_beta[strategy_idx] += (1.0 - reward)

        # Sliding window decay: if history is too long, remove oldest and decay
        if len(self.strategy_history) > self.strategy_window_size:
            old_idx, old_reward = self.strategy_history.pop(0)
            old_reward = float(old_reward)
            self.strategy_alpha[old_idx] = max(1.0, self.strategy_alpha[old_idx] - old_reward)
            self.strategy_beta[old_idx] = max(1.0, self.strategy_beta[old_idx] - (1.0 - old_reward))
    ...
    def _generate_parameters_batch(self, memory_f, memory_cr):
        """Dispatch to the selected parameter generation strategy."""
        idx = self.current_strategy_idx
        if idx == 0:
            return self._gen_params_variant01(memory_f, memory_cr)
        elif idx == 1:
            return self._gen_params_variant02(memory_f, memory_cr)
        elif idx == 2:
            return self._gen_params_variant05(memory_f, memory_cr)
        elif idx == 3:
            return self._gen_params_variant07(memory_f, memory_cr)
        elif idx == 4:
            return self._gen_params_original(memory_f, memory_cr)
        else:
            return self._gen_params_variant01(memory_f, memory_cr)

    # --- Strategy 0: variant_01 (10 wins - dominant) ---
    def _gen_params_variant01(self, memory_f, memory_cr):
        """Generation-adaptive scaling with exploration injection."""
        n = self.np_size
    ...
    # --- Strategy 1: variant_02 (2 wins) ---
    def _gen_params_variant02(self, memory_f, memory_cr):
        """Bimodal strategy: aggressive explorers + adaptive exploiters."""
        n = self.np_size
        explore_ratio = 0.4
    ...
    # --- Strategy 2: variant_05 (2 wins) ---
    def _gen_params_variant05(self, memory_f, memory_cr):
        """Bimodal distributions for enhanced exploration."""
        n = self.np_size
        gen = self.generation
    ...
    # --- Strategy 3: variant_07 (1 win) ---
    def _gen_params_variant07(self, memory_f, memory_cr):
        """Bimodal distribution: aggressive explorers + local refiners."""
        n = self.np_size
        f_values = np.zeros(n)
        cr_values = np.zeros(n)
    ...
    # --- Strategy 4: original ---
    def _gen_params_original(self, memory_f, memory_cr):
        """Original SHADE-style parameter generation."""
        n = self.np_size
        indices = np.random.randint(0, self.memory_size, size=n)
    ...
    def _restart(self, func, stopping_condition, population, fitness):
        ...
        # Partially reset Thompson Sampling on restart (decay towards uniform)
        self.strategy_alpha = 0.5 * self.strategy_alpha + 0.5 * np.ones(self.num_strategies)
        self.strategy_beta = 0.5 * self.strategy_beta + 0.5 * np.ones(self.num_strategies)
