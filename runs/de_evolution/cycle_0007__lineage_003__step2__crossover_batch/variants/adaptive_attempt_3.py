...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.op_quality = np.ones(self.num_operators)  # Thompson sampling alpha
        self.op_failure = np.ones(self.num_operators)   # Thompson sampling beta
        self.op_min_prob = 0.05  # minimum selection probability per operator
    ...
    def __call__(self, func, stopping_condition):
        ...
        self.op_quality = np.ones(self.num_operators)
        self.op_failure = np.ones(self.num_operators)
        self.op_rewards = [[] for _ in range(self.num_operators)]
        self.op_successes = np.zeros(self.num_operators)
        self.op_trials_count = np.zeros(self.num_operators)
        ...
        while not stopping_condition():
            ...
            # Crossover using selected operator
            trials = self._crossover_batch_adaptive(population, mutants, cr_values, op_idx)
            ...
            self._update_operator_reward(op_idx, success_delta, valid)
    ...
    def _select_crossover_operator(self):
        ...
        samples = np.array([
            np.random.beta(max(self.op_quality[i], 0.01), max(self.op_failure[i], 0.01))
            for i in range(self.num_operators)
        ])
        return int(np.argmax(samples))

    def _update_operator_reward(self, op_idx, success_delta, valid_mask):
        """Update operator statistics based on success."""
        n_valid = int(np.sum(valid_mask))
        n_success = len(success_delta)
        reward = float(np.sum(success_delta)) if n_success > 0 else 0.0

        # Update Thompson Sampling parameters
        if n_valid > 0:
            success_rate = float(n_success) / float(n_valid)
            self.op_quality[op_idx] += success_rate * 2.0 + (reward / (1.0 + reward)) * 0.5
            self.op_failure[op_idx] += (1.0 - success_rate) * 2.0
        else:
            self.op_failure[op_idx] += 1.0

        # Sliding window: decay old stats
        decay = 0.995
        for i in range(self.num_operators):
            self.op_quality[i] = max(1.0, self.op_quality[i] * decay)
            self.op_failure[i] = max(1.0, self.op_failure[i] * decay)

    def _crossover_batch_adaptive(self, population, mutants, cr_values, op_idx):
        """Dispatch to the selected crossover operator."""
        if op_idx == 0:
            ...
            return self._crossover_exponential_eigen(population, mutants, cr_values)
        elif op_idx == 3:
            return self._crossover_eigenvector_boosted(population, mutants, cr_values)
        elif op_idx == 4:
            return self._crossover_eigen_segment(population, mutants, cr_values)
        elif op_idx == 5:
    ...
    # Operator 1: Eigenvector-based crossover (variant_05)
    ...
    # Operator 2: Exponential crossover with eigenvector rotation (variant_06)
    def _crossover_exponential_eigen(self, population, mutants, cr_values):
        n, dim = population.shape
        try:
    ...
    # Operator 3: Eigenvector-rotated crossover with CR boost (variant_08)
    ...
    # Operator 4: Eigenvector-guided segment crossover (variant_10)
    def _crossover_eigen_segment(self, population, mutants, cr_values):
        n, dim = population.shape
        cr_matrix = cr_values[:, np.newaxis]
    ...
    # Operator 5: Exponential crossover with directional perturbation (variant_04)
    ...
    # Keep the original _crossover_batch for compatibility
