...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.n_operators = 6  # number of crossover operators
        self.op_window_size = 50  # sliding window size
        self.op_rewards = [[] for _ in range(self.n_operators)]
        self.op_counts = np.ones(self.n_operators, dtype=float)  # prior counts
        self.op_successes = np.zeros(self.n_operators, dtype=float)
        self.op_total_reward = np.zeros(self.n_operators, dtype=float)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            n_eval = min(len(population), len(trials))
            n_improved = 0
            total_improvement = 0.0
            for i in range(n_eval):
                if valid[i] and trial_fitness[i] < fitness[i]:
                    n_improved += 1
                    total_improvement += float(abs(fitness[i] - trial_fitness[i]))

            # Update operator stats
            self._update_operator_stats(op_idx, n_improved, n_eval, total_improvement)
    ...
    def _update_operator_stats(self, op_idx, n_improved, n_total, total_improvement):
        """Update Thompson Sampling parameters for the selected operator."""
        if n_total == 0:
            return
        # Success rate as reward signal
        success_rate = float(n_improved) / float(n_total)
        # Update Beta distribution parameters with decay
        decay = 0.995
        self.op_alpha *= decay
        self.op_beta *= decay
        # Ensure minimum values
        self.op_alpha = np.maximum(self.op_alpha, 0.5)
        self.op_beta = np.maximum(self.op_beta, 0.5)
        # Update selected operator
        self.op_alpha[op_idx] += float(n_improved)
        self.op_beta[op_idx] += float(n_total - n_improved)

    def _crossover_batch_adaptive(self, population, mutants, cr_values, op_idx):
        """Dispatch to the selected crossover operator."""
        if op_idx == 0:
            return self._crossover_standard(population, mutants, cr_values)
        elif op_idx == 1:
            ...
            return self._crossover_eigenvector_exponential(population, mutants, cr_values)
        elif op_idx == 4:
    ...
    # Operator 0: Original standard binomial crossover
    def _crossover_standard(self, population, mutants, cr_values):
    ...
    # Operator 1: Exponential crossover with directional perturbation (variant_04)
    ...
    # Operator 2: Eigenvector-based crossover (variant_05)
    ...
    # Operator 3: Exponential + eigenvector rotation (variant_06)
    def _crossover_eigenvector_exponential(self, population, mutants, cr_values):
        n, dim = population.shape
        try:
    ...
    # Operator 4: Eigenvector-rotated with boosted CR (variant_08)
    ...
    # Operator 5: Eigenvector-guided segment crossover (variant_10)
    ...
    # Keep the original _crossover_batch for backward compatibility
