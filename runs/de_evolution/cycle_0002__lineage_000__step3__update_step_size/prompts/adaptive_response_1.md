...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    and adaptive step size strategy selection via Thompson Sampling.
    """
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.n_strategies = 3
        self.ts_alpha = np.ones(self.n_strategies, dtype=float)
        self.ts_beta = np.ones(self.n_strategies, dtype=float)
        self.current_strategy = 0
        self._strategy_fitness_start = np.inf
        self._strategy_evals_start = 0
        self._strategy_epoch_length = 0

        # Adaptive step size strategy selection via Thompson Sampling
        # 0 = variant_01 (adaptive damping with condition-aware bounds)
        # 1 = variant_05 (fitness-aware damping with sigma floor and stagnation boost)
        # 2 = variant_08 (two-phase approach: aggressive exploration vs precise convergence)
        # 3 = original (standard CSA)
        # 4 = variant_07 (fitness improvement rate tracking with biphasic behavior)
        # 5 = variant_10 (asymmetric update with periodic perturbation)
        self.n_step_strategies = 6
        self.step_ts_alpha = np.ones(self.n_step_strategies, dtype=float)
        self.step_ts_beta = np.ones(self.n_step_strategies, dtype=float)
        self.current_step_strategy = 0
        self._step_fitness_before = np.inf
        self._step_epoch_counter = 0
        self._step_epoch_length = 10  # evaluate every N generations
    ...
    def _update_step_size_strategy0(self):
        """variant_01: adaptive damping with condition-aware bounds"""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_strategy1(self):
        """variant_05: fitness-aware damping with sigma floor and stagnation boost"""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_strategy2(self):
    ...
    def _update_step_size_strategy3(self):
        """original: standard CSA"""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
        self.sigma *= np.exp((self.c_sigma / self.d_sigma) * (p_sigma_norm / self.chi_n - 1.0))
        self.sigma = np.clip(self.sigma, 1e-20, 1e6)

    def _update_step_size_strategy4(self):
        """variant_07: fitness improvement rate tracking with biphasic behavior"""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_strategy5(self):
        """variant_10: asymmetric update with periodic perturbation"""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _select_step_strategy(self):
        ...
        samples = np.array([
            np.random.beta(max(self.step_ts_alpha[i], 0.01), max(self.step_ts_beta[i], 0.01))
            for i in range(self.n_step_strategies)
        ])
        return int(np.argmax(samples))

    def _update_step_strategy_reward(self, strategy_idx, fitness_before, fitness_after):
        ...
        if rel_improvement > 1e-4:
            self.step_ts_alpha[strategy_idx] += 2.0
        elif rel_improvement > 1e-6:
            self.step_ts_alpha[strategy_idx] += 1.0
        elif rel_improvement > 1e-10:
            self.step_ts_alpha[strategy_idx] += 0.3
            self.step_ts_beta[strategy_idx] += 0.3
        else:
            self.step_ts_beta[strategy_idx] += 1.0

        decay = 0.995
        self.step_ts_alpha *= decay
        self.step_ts_beta *= decay
        self.step_ts_alpha = np.maximum(self.step_ts_alpha, 0.5)
        self.step_ts_beta = np.maximum(self.step_ts_beta, 0.5)

    def _update_step_size(self):
        """Adaptive step size update: delegates to selected strategy."""
        step_strategies = [
            self._update_step_size_strategy0,
            self._update_step_size_strategy1,
            self._update_step_size_strategy2,
            self._update_step_size_strategy3,
            self._update_step_size_strategy4,
            self._update_step_size_strategy5,
        ]

        step_strategies[self.current_step_strategy]()

        # Safety check
        if not np.isfinite(self.sigma):
            self.sigma = 30.0

        self._step_epoch_counter += 1

        # Evaluate and potentially switch strategy
        if self._step_epoch_counter >= self._step_epoch_length:
            self._update_step_strategy_reward(
                self.current_step_strategy,
                self._step_fitness_before,
                float(self.best_fitness)
            )
            self.current_step_strategy = self._select_step_strategy()
            self._step_fitness_before = float(self.best_fitness)
            self._step_epoch_counter = 0
    ...
    def _restart(self):
        ...
        # Update reward for current stagnation strategy
        ...
        # Also finalize step strategy reward before restart
        self._update_step_strategy_reward(
            self.current_step_strategy,
            self._step_fitness_before,
            saved_best_f
        ...
        self.current_step_strategy = self._select_step_strategy()
        self._step_fitness_before = saved_best_f
        self._step_epoch_counter = 0
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize step size strategy tracking
        self.current_step_strategy = self._select_step_strategy()
        self._step_fitness_before = np.inf
        self._step_epoch_counter = 0
