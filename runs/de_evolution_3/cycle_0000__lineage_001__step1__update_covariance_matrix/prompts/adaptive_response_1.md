...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.ts_alpha = np.ones(self.num_operators)
        self.ts_beta = np.ones(self.num_operators)
        # Sliding window for credit assignment
        self.window_size = 20
        self.reward_history = []  # list of (operator_idx, reward)
        self.current_operator = 0
        # Historical rank-mu for variant_07
        self._rank_mu_hist = None
        # Track previous best for reward computation
        self._pre_update_best_f = np.inf
    ...
    def _select_operator(self):
        """Select covariance update operator using Thompson Sampling."""
        samples = np.array([
    ...
    def _update_operator_rewards(self, operator_idx, reward):
        """Update Thompson Sampling parameters with reward."""
        reward = float(reward)
        self.reward_history.append((int(operator_idx), reward))
        
        # Sliding window: decay old observations
        if len(self.reward_history) > self.window_size:
            # Remove oldest and adjust
            old_op, old_reward = self.reward_history.pop(0)
            # Decay towards prior
            decay = 0.95
            self.ts_alpha[old_op] = max(1.0, self.ts_alpha[old_op] * decay)
            self.ts_beta[old_op] = max(1.0, self.ts_beta[old_op] * decay)
        
        # Update current operator
        if reward > 0:
            self.ts_alpha[operator_idx] += reward
        else:
            self.ts_beta[operator_idx] += abs(reward) if reward < 0 else 0.1

    # ===================== COVARIANCE UPDATE STRATEGIES =====================

    def _update_cov_original(self, old_mean, sorted_population, h_sigma):
        """Strategy 0: Original rank-1 + rank-mu update."""
        dim = self.dim
        selected = sorted_population[:self.mu]
    ...
    def _update_cov_variant01(self, old_mean, sorted_population, h_sigma):
        """Strategy 1: Population-wide covariance blend."""
        dim = self.dim
        lam = self.pop_size
        selected = sorted_population[:self.mu]
        rank1 = np.outer(self.p_c, self.p_c)
    ...
    def _update_cov_variant02(self, old_mean, sorted_population, h_sigma):
        """Strategy 2: Active CMA with negative weights."""
        dim = self.dim
        lam = self.pop_size
        rank1 = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
        ...
        try:
            ...
            self.C += 1e-10 * np.eye(dim)

    def _update_cov_variant06(self, old_mean, sorted_population, h_sigma):
        """Strategy 3: Exponential fitness-weighted rank-mu."""
        dim = self.dim
        selected = sorted_population[:self.mu]
        rank1 = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
    ...
    def _update_cov_variant07(self, old_mean, sorted_population, h_sigma):
        """Strategy 4: Exponentially weighted rank-mu history."""
        dim = self.dim
        selected = sorted_population[:self.mu]
        rank1 = np.outer(self.p_c, self.p_c)
        delta_h = (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c)
    ...
    def _update_covariance_matrix(self, old_mean, sorted_population, h_sigma):
        """Adaptively select and apply the best covariance update strategy."""
        # Save state in case we need to recover
        C_backup = self.C.copy()
        
        # Select operator
        op_idx = self._select_operator()
        self.current_operator = op_idx
        
        try:
            if op_idx == 0:
                self._update_cov_original(old_mean, sorted_population, h_sigma)
            elif op_idx == 1:
                self._update_cov_variant01(old_mean, sorted_population, h_sigma)
            elif op_idx == 2:
                self._update_cov_variant02(old_mean, sorted_population, h_sigma)
            elif op_idx == 3:
                self._update_cov_variant06(old_mean, sorted_population, h_sigma)
            elif op_idx == 4:
                self._update_cov_variant07(old_mean, sorted_population, h_sigma)
            
            # Validate the result
            if not np.all(np.isfinite(self.C)):
                self.C = C_backup
                self._update_operator_rewards(op_idx, -1.0)
                return
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Record pre-update best for reward computation
            self._pre_update_best_f = float(self.best_f)
            ...
            # Compute reward for the operator that was just used
            post_best = float(self.best_f)
            improvement = self._pre_update_best_f - post_best
            
            # Reward: positive if improvement, small positive otherwise (exploration credit)
            if improvement > 0:
                # Normalize improvement relative to current best magnitude
                scale = max(1e-10, abs(self._pre_update_best_f))
                reward = float(min(1.0, improvement / scale * 100.0))
                reward = max(reward, 0.1)  # at least some reward for improvement
            else:
                # Small penalty for no improvement, but not too harsh
                reward = 0.0
            
            # Also give credit based on fitness of best in current generation
            valid_fit = sorted_fit[np.isfinite(sorted_fit)]
            if len(valid_fit) > 0:
                gen_best = float(valid_fit[0])
                if gen_best < self._pre_update_best_f:
                    reward = max(reward, 0.5)
            
            self._update_operator_rewards(self.current_operator, reward)
