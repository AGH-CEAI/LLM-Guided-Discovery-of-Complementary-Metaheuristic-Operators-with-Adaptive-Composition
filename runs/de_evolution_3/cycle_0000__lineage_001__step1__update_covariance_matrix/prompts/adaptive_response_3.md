...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.alpha_ts = np.ones(self.n_operators)  # successes + 1
        self.beta_ts = np.ones(self.n_operators)   # failures + 1
        
        # Sliding window for credit assignment
        self.window_size = 20
        self.reward_history = [[] for _ in range(self.n_operators)]
        self.current_operator = 0
        self.prev_best_for_operator = np.inf
        
        # History for variant_07
    ...
    def _select_operator(self):
        ...
        samples = np.array([
            np.random.beta(self.alpha_ts[i], self.beta_ts[i])
            for i in range(self.n_operators)
        ])
        return int(np.argmax(samples))

    def _assign_credit(self, operator_idx, fitness_before, fitness_after):
        """Assign credit to the operator based on improvement."""
        ...
        self.alpha_ts[operator_idx] += reward
        self.beta_ts[operator_idx] += (1.0 - reward)
        
        # Decay to allow adaptation (sliding window effect)
        decay = 0.995
        self.alpha_ts = np.maximum(1.0, self.alpha_ts * decay)
        self.beta_ts = np.maximum(1.0, self.beta_ts * decay)
    ...
    # ---- Operator 1: variant_01 (population-wide covariance blend) ----
    def _update_cov_pop_blend(self, old_mean, sorted_population, h_sigma):
        dim = self.dim
        lam = self.pop_size
        selected = sorted_population[:self.mu]
    ...
    # ---- Operator 2: variant_02 (active CMA with negative weights) ----
    ...
    # ---- Operator 3: variant_06 (exponential fitness-weighted) ----
    def _update_cov_exponential(self, old_mean, sorted_population, h_sigma):
        dim = self.dim
        selected = sorted_population[:self.mu]
    ...
    # ---- Operator 4: variant_07 (exponentially weighted rank-mu history) ----
    def _update_cov_history(self, old_mean, sorted_population, h_sigma):
        dim = self.dim
        selected = sorted_population[:self.mu]
    ...
    def _update_covariance_matrix(self, old_mean, sorted_population, h_sigma):
        """Dispatch to the selected covariance update operator."""
        op = self.current_operator
        
        # Save C in case operator crashes
        C_backup = self.C.copy()
        rank_mu_hist_backup = self._rank_mu_hist.copy() if self._rank_mu_hist is not None else None
        
        try:
            if op == 0:
                self._update_cov_original(old_mean, sorted_population, h_sigma)
            elif op == 1:
                self._update_cov_pop_blend(old_mean, sorted_population, h_sigma)
            elif op == 2:
                self._update_cov_active(old_mean, sorted_population, h_sigma)
            elif op == 3:
                self._update_cov_exponential(old_mean, sorted_population, h_sigma)
            elif op == 4:
                self._update_cov_history(old_mean, sorted_population, h_sigma)
            else:
                self._update_cov_original(old_mean, sorted_population, h_sigma)
            ...
            self.C = C_backup
            self._rank_mu_hist = rank_mu_hist_backup
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            # Select operator for this generation
            ...
            # Credit assignment for the operator
            fitness_after_gen = float(self.best_f)
            self._assign_credit(self.current_operator, fitness_before_gen, fitness_after_gen)
