...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self._stag_strategy_evals_start = 0
        ...
        # 2 = variant_05 (fitness-aware damping + sigma floor + stagnation boost)
        # 3 = variant_08 (two-phase approach)
        # 4 = variant_07 (fitness improvement rate tracking)
        ...
        self._step_strategy_epoch_length = 0
        self._step_strategy_eval_window = 20  # evaluate every N generations
    ...
    def _update_step_size_0(self):
        """Strategy 0: Original standard CSA."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_1(self):
        """Strategy 1: variant_01 - adaptive damping + condition-aware."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_2(self):
        """Strategy 2: variant_05 - fitness-aware damping + sigma floor + stagnation boost."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_3(self):
        """Strategy 3: variant_08 - two-phase approach."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_4(self):
        """Strategy 4: variant_07 - fitness improvement rate tracking."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
    ...
    def _update_step_size_5(self):
        """Strategy 5: variant_10 - asymmetric update."""
        p_sigma_norm = np.linalg.norm(self.p_sigma)
        ...
        if np.isfinite(bf) and hasattr(self, 'generation'):
            gen = self.generation
    ...
    def _update_step_size(self):
        ...
        idx = int(self.current_step_strategy)
        idx = max(0, min(idx, len(step_strategies) - 1))
        step_strategies[idx]()
        self._step_strategy_epoch_length += 1

        # Periodically evaluate and potentially switch step strategy
        if self._step_strategy_epoch_length >= self._step_strategy_eval_window:
            self._update_step_strategy_reward()
            self.current_step_strategy = self._select_step_strategy()
            self._step_strategy_fitness_start = float(self.best_fitness) if np.isfinite(self.best_fitness) else 1e10
            self._step_strategy_epoch_length = 0
    ...
    def _update_step_strategy_reward(self):
        """Credit assignment for step size strategy."""
        strategy_idx = int(self.current_step_strategy)
        fitness_before = float(self._step_strategy_fitness_start)
        fitness_after = float(self.best_fitness) if np.isfinite(self.best_fitness) else 1e10

        if not np.isfinite(fitness_before) or not np.isfinite(fitness_after):
            return
        if self._step_strategy_epoch_length <= 0:
            return

        improvement = fitness_before - fitness_after
        rel_improvement = improvement / (abs(fitness_before) + 1e-30)
    ...
    def _restart(self):
        ...
        # Also finalize step strategy reward
        self._update_step_strategy_reward()
