...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # Adaptive operator selection: 3 mutation strategies
        # 0: original (current-to-pbest/1 + rand-to-pbest/1)
        # 1: variant_01 (rank-weighted current-to-pbest with adaptive p)
        # 2: variant_09 (eigenvector-guided multi-donor)
        self.num_operators = 3
        self.op_successes = np.ones(self.num_operators, dtype=float)
        self.op_failures = np.ones(self.num_operators, dtype=float)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Update operator reward tracking
            self._update_operator_reward(success_delta, len(trials), valid)
    ...
    def _select_operator(self):
        ...
        for i in range(self.num_operators):
            alpha = max(1.0, self.op_successes[i])
            beta = max(1.0, self.op_failures[i])
            try:
    ...
    def _update_operator_reward(self, success_delta, n_trials, valid_mask):
        """Update operator tracking based on success."""
        op = self.current_operator
        n_valid = int(np.sum(valid_mask))
        n_success = len(success_delta)
        n_fail = max(0, n_valid - n_success)

        if n_success > 0:
            total_reward = float(np.sum(success_delta))
            self.op_successes[op] += float(n_success)
            self.op_rewards[op].append(total_reward)
            # Keep sliding window
            if len(self.op_rewards[op]) > self.sliding_window_size:
                self.op_rewards[op] = self.op_rewards[op][-self.sliding_window_size:]
        
        if n_fail > 0:
            self.op_failures[op] += float(n_fail)

        # Decay to allow adaptation over time
        decay = 0.995
        self.op_successes *= decay
        self.op_failures *= decay
        # Ensure minimums
        self.op_successes = np.maximum(self.op_successes, 1.0)
        self.op_failures = np.maximum(self.op_failures, 1.0)
    ...
    def _mutate_batch(self, population, fitness, archive, f_values, strategy_mask):
        """Dispatch to selected operator's mutation strategy."""
        op = self.current_operator
        if op == 0:
    ...
    def _mutate_original(self, population, fitness, archive, f_values, strategy_mask):
        """Original: current-to-pbest/1 + rand-to-pbest/1."""
        n = len(population)
        dim = self.dim
    ...
    def _mutate_variant01(self, population, fitness, archive, f_values, strategy_mask):
        """Variant 01: rank-weighted current-to-pbest with adaptive p."""
        n = len(population)
        dim = self.dim
    ...
    def _mutate_variant09(self, population, fitness, archive, f_values, strategy_mask):
        """Variant 09: eigenvector-guided multi-donor."""
        n = len(population)
        dim = self.dim
        f_col = f_values[:, np.newaxis]
