...
class AdaptiveDEWithRestarts:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        # 3=variant06 (SA boltzmann), 4=variant07 (SA+crowding replace)
    ...
    def __call__(self, func, stopping_condition):
        ...
        while not stopping_condition():
            ...
            # Record fitness before selection
            fitness_before = self.f_opt

            # Selection using chosen strategy
            ...
            # Compute reward for strategy selection
            fitness_after = self.f_opt
            improvement = float(fitness_before - fitness_after)
            n_successes = len(success_f)
            
            # Reward based on improvement and number of successful mutations
    ...
    def _select_strategy(self):
        """Thompson Sampling to select survivor strategy."""
        samples = np.array([
    ...
    def _update_strategy_reward(self, strategy_idx, reward):
        """Update Thompson Sampling parameters with sliding window."""
        reward = float(reward)
        ...
        # Recompute alpha/beta from recent rewards
        rewards = self.strategy_rewards[strategy_idx]
        if len(rewards) > 0:
            successes = sum(1 for r in rewards if r > 0.5)
            failures = len(rewards) - successes
            self.strategy_alpha[strategy_idx] = 1.0 + successes
            self.strategy_beta[strategy_idx] = 1.0 + failures
    ...
    def _survivors_variant02(self, population, fitness, trials, trial_fitness,
                             archive, f_values, cr_values, valid_mask):
        """SA+crowding distance based acceptance."""
        n = min(len(population), len(trials))
        success_f = []
        success_cr = []
        success_delta = []
    ...
    def _survivors_variant04(self, population, fitness, trials, trial_fitness,
                             archive, f_values, cr_values, valid_mask):
        """Crowding-based selection with probabilistic worse-acceptance."""
        n = min(len(population), len(trials))
        success_f = []
        success_cr = []
        success_delta = []
    ...
    def _survivors_variant06(self, population, fitness, trials, trial_fitness,
                             archive, f_values, cr_values, valid_mask):
        """SA-like acceptance with Boltzmann probability."""
        n = min(len(population), len(trials))
        success_f = []
        success_cr = []
        success_delta = []
