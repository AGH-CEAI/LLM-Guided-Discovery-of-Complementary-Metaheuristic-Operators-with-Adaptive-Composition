...
class SimplifiedCovarianceAdaptiveSearch:
    ...
    def __init__(self, dim, *args, **kwargs):
        ...
        self.strategy_alpha = np.ones(self.num_strategies, dtype=float)  # Beta prior successes
        self.strategy_beta = np.ones(self.num_strategies, dtype=float)   # Beta prior failures
        self.current_strategy_idx = 0
        self._strategy_fitness_before_restart = np.inf
        self._strategy_evals_before_restart = 0
        self._strategy_window_rewards = [[] for _ in range(self.num_strategies)]
    ...
    def _detect_stagnation_variant05(self):
        """Adaptive stagnation detection with fitness history tracking."""
    ...
    def _select_strategy(self):
        ...
        return self.current_strategy_idx

    def _detect_stagnation(self):
        """Adaptive stagnation detection using selected strategy."""
        strategies = [
    ...
    def _update_strategy_reward(self, fitness_before, fitness_after):
        ...
        # Normalize: any positive improvement is a success
        if improvement > 0:
            # Scale reward by relative improvement
            rel_imp = improvement / (abs(float(fitness_before)) + 1e-30)
            reward = min(float(rel_imp) * 100.0, 5.0)  # cap reward
            reward = max(reward, 0.5)  # minimum reward for any improvement
            self.strategy_alpha[idx] += reward
        else:
            # No improvement or worse: penalize mildly
            self.strategy_beta[idx] += 0.5

        # Decay to avoid locking in early decisions
        decay = 0.995
    ...
    def __call__(self, func, stopping_condition):
        ...
        self._strategy_fitness_before_restart = float(self.best_fitness)
        self._generations_since_strategy_select = 0
        ...
        while not stopping_condition():
            ...
            self.generation += 1
            self._generations_since_strategy_select += 1

            if self._detect_stagnation():
                # Compute reward for current strategy
                fitness_after = float(self.best_fitness)
                self._update_strategy_reward(
                    self._strategy_fitness_before_restart,
                    fitness_after
                )
                
                # Select new strategy for next phase
                self._select_strategy()
                
                self._restart()
                
                # Record fitness at start of new phase
                self._strategy_fitness_before_restart = float(self.best_fitness)
                self._generations_since_strategy_select = 0
