...
- Decompose the algorithm into MANY SMALL METHODS, each with a single
  ...
  on the whole population at once, not one individual at a time):
      ...
      _compute_diversity, ...
  Each method should be short (ideally < 25 lines) so it can later be
...
- Avoid making methods that are too short (below 3 lines)
...
- SimplifiedCovarianceAdaptiveSearch
- TopologyAdaptivePSO
