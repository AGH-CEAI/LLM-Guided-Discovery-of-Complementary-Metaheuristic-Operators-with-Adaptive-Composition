...
class ReflectiveDEWithStrategyPortfolio:
    ...
    def __init__(self, dim, np_factor=6, seed=None):
        ...
        # Per-operator tracking for dim-success
        ...
        self._dim_attempt_count = None
        self._dim_history_fitness = None
        
        # For variant_10
        self.cx_elite_memory = None
        
        # Generation counter for warm-up
    ...
    def _initialize_population(self):
        ...
        self._dim_attempt_count = np.zeros(self.dim)
    ...
    def _crossover_variant03(self, population, trial_population):
        """Op 1: Success-rate guided crossover with adaptive mixing."""
        n_trials = self.NP
        dim = self.dim
        ...
        diversity = float(np.mean(pop_std / (max_spread + 1e-10)))
    ...
    def _crossover_variant04(self, population, trial_population):
        """Op 2: Success-guided adaptive crossover."""
        n_trials = self.NP
        dim = self.dim
        ...
        if self.prev_trial_pop is not None and self.prev_trial_pop.shape == (n_trials, dim):
        ...
        max_spread = self.upper - self.lower
        diversity_factor = float(np.clip(np.mean(pop_std) / (max_spread + 1e-10), 0.05, 0.95))
        ...
        for i in range(n_trials):
            ...
            cross_mask[i, list(selected_dims)] = True
    ...
    def _crossover_variant06(self, population, trial_population):
        """Op 3: Confidence-weighted parent preservation crossover."""
        n_trials = self.NP
        dim = self.dim
    ...
    def _crossover_variant08(self, population, trial_population):
        """Op 4: Ultra-conservative crossover for sensitive tasks."""
        n_trials = self.NP
        dim = self.dim
    ...
    def _crossover_variant10(self, population, trial_population):
        """Op 5: Coordinate-wise elite-preserving conservative crossover."""
        n_trials = self.NP
        dim = self.dim
        ...
        avg_distance = float(np.mean(np.linalg.norm(population - pop_centroid, axis=1)))
        max_spread = self.upper - self.lower
        ...
        alpha_blend = 0.2
        blended_trial = (
            alpha_blend * trial_population +
            (1.0 - alpha_blend) * population[:n_trials]
        )
    ...
    def _select_crossover_operator(self):
        ...
        samples = np.array([
            self.rng.beta(float(self.cx_alpha[i]), float(self.cx_beta[i]))
    ...
    def _crossover_block(self, population, trial_population):
        """Adaptive crossover: selects among operators using Thompson Sampling."""
        # During warm-up, cycle through operators to gather info
        if self.generation_count < self.n_crossover_ops * 3:
            self.current_cx_op = self.generation_count % self.n_crossover_ops
        else:
    ...
    def _crossover_batch(self, population, trial_population):
        """Wrapper that calls _crossover_block (the adaptive selector)."""
