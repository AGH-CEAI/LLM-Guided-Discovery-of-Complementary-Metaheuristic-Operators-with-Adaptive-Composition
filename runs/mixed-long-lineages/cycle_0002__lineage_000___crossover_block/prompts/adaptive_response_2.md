...
class ReflectiveDEWithStrategyPortfolio:
    ...
    def __init__(self, dim, np_factor=6, seed=None):
        ...
        # 0: original (binomial)
        # 1: variant_03 (success-rate guided crossover with adaptive mixing) - 4 wins
        # 2: variant_04 (success-guided adaptive crossover) - 2 wins
        # 3: variant_06 (confidence-weighted parent preservation) - 4 wins
        # 4: variant_08 (ultra-conservative crossover) - 3 wins
        # 5: variant_10 (coordinate-wise elite-preserving conservative) - 2 wins
        self.n_crossover_ops = 6
        # Thompson Sampling parameters (Beta distribution)
        self.cx_alpha = np.ones(self.n_crossover_ops)
        self.cx_beta = np.ones(self.n_crossover_ops)
        # Give initial priors based on win counts
        # original: 6 wins, variant_03: 4, variant_04: 2, variant_06: 4, variant_08: 3, variant_10: 2
        self.cx_alpha[0] += 6.0
        self.cx_alpha[1] += 4.0
        self.cx_alpha[2] += 2.0
        self.cx_alpha[3] += 4.0
        self.cx_alpha[4] += 3.0
        self.cx_alpha[5] += 2.0
        # Add some beta to balance
        total_tasks = 21  # non-trivial tasks
        self.cx_beta[0] += float(total_tasks - 6)
        self.cx_beta[1] += float(total_tasks - 4)
        self.cx_beta[2] += float(total_tasks - 2)
        self.cx_beta[3] += float(total_tasks - 4)
        self.cx_beta[4] += float(total_tasks - 3)
        self.cx_beta[5] += float(total_tasks - 2)

        # Sliding window for credit assignment
        self.cx_window_size = 80
        self.cx_history = []  # list of (op_idx, reward)
        # Current operator index
        self.current_cx_op = 0  # start with original
        ...
        # For variant_04
        self.dim_improve_count_v4 = None
        self.dim_attempt_count_v4 = None
        self.prev_trial_pop_v4 = None
        
        # For variant_06
        self._dim_success_count_v6 = None
        self._dim_attempt_count_v6 = None
        self._dim_history_fitness_v6 = None
        
        # For variant_10
        self.cx_elite_memory = None
        
        # Generation counter for exploration schedule
        self.gen_count = 0
    ...
    def _initialize_population(self):
        ...
        # Reset crossover operator per-operator state
        ...
        self.dim_improve_count_v4 = np.zeros(self.dim)
        self.dim_attempt_count_v4 = np.zeros(self.dim)
        self.prev_trial_pop_v4 = None
        
        self._dim_success_count_v6 = np.zeros(self.dim)
        self._dim_attempt_count_v6 = np.zeros(self.dim)
        self._dim_history_fitness_v6 = np.full(self.dim, np.inf)
    ...
    def _crossover_original(self, population, trial_population):
        """Op 0: Standard binomial crossover (original, 6 wins)."""
    ...
    def _crossover_variant03(self, population, trial_population):
        """Op 1: Success-rate guided crossover with adaptive mixing (variant_03, 4 wins)."""
        n_trials = self.NP
        dim = self.dim
        ...
        best_fit = float(np.min(valid_fitness)) if len(valid_fitness) > 0 else 0.0
        worst_fit = float(np.max(valid_fitness)) if len(valid_fitness) > 0 else 1.0
        fit_range = worst_fit - best_fit + 1e-10

        median_fit = float(np.median(valid_fitness)) if len(valid_fitness) > 0 else best_fit
        mix_factor = np.clip(
    ...
    def _crossover_variant04(self, population, trial_population):
        """Op 2: Success-guided adaptive crossover (variant_04, 2 wins)."""
        n_trials = self.NP
        dim = self.dim

        if self.dim_improve_count_v4 is None or len(self.dim_improve_count_v4) != dim:
            self.dim_improve_count_v4 = np.zeros(dim)
            self.dim_attempt_count_v4 = np.zeros(dim)
            self.prev_trial_pop_v4 = np.zeros((n_trials, dim))

        if self.prev_trial_pop_v4 is not None and self.prev_trial_pop_v4.shape[0] == n_trials:
            improved_mask = trial_population < self.prev_trial_pop_v4
            self.dim_improve_count_v4 += np.sum(improved_mask, axis=0)
            self.dim_attempt_count_v4 += n_trials

        self.prev_trial_pop_v4 = trial_population.copy()

        dim_success_rate = np.zeros(dim)
        valid_dims = self.dim_attempt_count_v4 > 10
        dim_success_rate[valid_dims] = (
            self.dim_improve_count_v4[valid_dims] / self.dim_attempt_count_v4[valid_dims]
        )
    ...
    def _crossover_variant06(self, population, trial_population):
        """Op 3: Confidence-weighted parent preservation crossover (variant_06, 4 wins)."""
        n_trials = self.NP
        dim = self.dim

        if self._dim_success_count_v6 is None or len(self._dim_success_count_v6) != dim:
            self._dim_success_count_v6 = np.zeros(dim)
            self._dim_attempt_count_v6 = np.zeros(dim)
            self._dim_history_fitness_v6 = np.full(dim, np.inf)

        trial_better = (trial_population < population[:n_trials]).astype(float)
        dim_signal = np.mean(trial_better, axis=0)

        self._dim_attempt_count_v6 += 1
        alpha = 0.1
        self._dim_success_count_v6 = (
            alpha * dim_signal * float(n_trials) +
            (1 - alpha) * self._dim_success_count_v6
        )

        success_rate = np.zeros(dim)
        valid = self._dim_attempt_count_v6 > 5
        success_rate[valid] = self._dim_success_count_v6[valid] / self._dim_attempt_count_v6[valid]
    ...
    def _crossover_variant08(self, population, trial_population):
        """Op 4: Ultra-conservative crossover (variant_08, 3 wins)."""
        n_trials = self.NP
        dim = self.dim
    ...
    def _crossover_variant10(self, population, trial_population):
        """Op 5: Coordinate-wise elite-preserving conservative crossover (variant_10, 2 wins)."""
        n_trials = self.NP
        dim = self.dim
    ...
    def _select_crossover_operator(self):
        ...
        samples = np.array([
            self.rng.beta(max(self.cx_alpha[i], 0.01), max(self.cx_beta[i], 0.01))
    ...
    def _update_crossover_reward(self, op_idx, n_improved, n_total, fitness_improvement):
        ...
        # Recompute alpha/beta from sliding window + priors
        # Start with weak priors based on benchmark wins
        prior_alpha = np.array([1.0 + 3.0, 1.0 + 2.0, 1.0 + 1.0, 1.0 + 2.0, 1.0 + 1.5, 1.0 + 1.0])
        prior_beta = np.array([1.0 + 1.0, 1.0 + 1.5, 1.0 + 2.0, 1.0 + 1.5, 1.0 + 1.5, 1.0 + 2.0])
        
        self.cx_alpha = prior_alpha.copy()
        self.cx_beta = prior_beta.copy()
    ...
    def _adapt_strategy_probabilities(self):
        ...
        best_fitness = float(np.min(self.population_fitness)) if hasattr(self, 'population_fitness') else 0.0
