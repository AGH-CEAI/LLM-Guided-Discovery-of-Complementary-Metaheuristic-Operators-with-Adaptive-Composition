...
class ReflectiveDEWithStrategyPortfolio:
    ...
    This version uses a meta-level Multi-Armed Bandit (Thompson Sampling)
    to adaptively select among multiple operator selection strategies during
    optimization.
    """
    ...
    def __init__(self, dim, np_factor=6, seed=None):
        ...
        # Meta-level: define the pool of adapt strategies
        # Each is identified by an integer index
        self.n_meta_strategies = 7
        # Meta-level Thompson Sampling parameters
        self.meta_alpha = np.ones(self.n_meta_strategies)  # successes + prior
        self.meta_beta = np.ones(self.n_meta_strategies)   # failures + prior
        self.current_meta_strategy = 0
        self.meta_window_size = 5
        self.meta_generation_counter = 0
        self.meta_best_before = np.inf
        # Sliding window for credit assignment
        self.meta_rewards = [[] for _ in range(self.n_meta_strategies)]
    ...
    # =========================================================================
    # Meta-level: select which adapt strategy to use via Thompson Sampling
    # =========================================================================
    def _select_meta_strategy(self):
        """Use Thompson Sampling to select among the meta-strategies."""
        samples = np.array([
            self.rng.beta(max(self.meta_alpha[i], 0.01), max(self.meta_beta[i], 0.01))
            for i in range(self.n_meta_strategies)
        ])
        self.current_meta_strategy = int(np.argmax(samples))

    def _update_meta_reward(self, current_best):
        """Credit assignment for the meta-level bandit."""
        improvement = float(self.meta_best_before - current_best)
        if improvement > 1e-15:
            # Success: improvement happened
            self.meta_alpha[self.current_meta_strategy] += 1.0
            # Decay others slightly to allow adaptation
            for i in range(self.n_meta_strategies):
                if i != self.current_meta_strategy:
                    self.meta_beta[i] += 0.1
        else:
            # No improvement
            self.meta_beta[self.current_meta_strategy] += 0.5

        # Gentle decay to forget old information (sliding window effect)
        decay = 0.995
        self.meta_alpha = np.maximum(self.meta_alpha * decay, 1.0)
        self.meta_beta = np.maximum(self.meta_beta * decay, 1.0)

        self.meta_best_before = current_best

    # =========================================================================
    # Seven adapt strategy implementations
    # =========================================================================

    def _adapt_strategy_0_original(self):
        """Original: simple fitness-proportional."""
    ...
    def _adapt_strategy_1_thompson(self):
        """Thompson sampling for operator selection (variant_01)."""
        for i in range(self.NP):
    ...
    def _adapt_strategy_2_entropy(self):
        """Entropy-balanced with success rate + log fitness (variant_02)."""
    ...
    def _adapt_strategy_3_gen_blend(self):
        """Generation-adaptive blending of rate and count (variant_03, 6 wins)."""
    ...
    def _adapt_strategy_4_rank_softmax(self):
    ...
    def _adapt_strategy_5_diversity_rank(self):
        """Diversity-sensitive rank-based (variant_05, 3 wins)."""
        ...
        if len(self.diversity_history) >= 10:
            ...
            hist_window = self.diversity_history[-50:]
            div_range = max(np.max(hist_window) - np.min(hist_window), 1e-10)
            diversity_signal = np.clip((recent_div - np.min(hist_window)) / div_range, 0.0, 1.0)
        else:
    ...
    def _adapt_strategy_6_entropy_softmax(self):
        """Entropy-based temperature softmax on fitness (variant_07)."""
    ...
    def _adapt_strategy_probabilities(self):
        """Meta-adaptive: dispatch to the currently selected meta-strategy."""
        meta = self.current_meta_strategy
        if meta == 0:
            self._adapt_strategy_0_original()
        elif meta == 1:
            self._adapt_strategy_1_thompson()
        elif meta == 2:
            self._adapt_strategy_2_entropy()
        elif meta == 3:
            self._adapt_strategy_3_gen_blend()
        elif meta == 4:
            self._adapt_strategy_4_rank_softmax()
        elif meta == 5:
            self._adapt_strategy_5_diversity_rank()
        elif meta == 6:
            self._adapt_strategy_6_entropy_softmax()
        else:
            self._adapt_strategy_3_gen_blend()  # fallback to best
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize meta-level tracking
        self.meta_best_before = f_opt
        self.meta_generation_counter = 0

        # Initial meta-strategy selection
        ...
        while not stopping_condition():
            ...
            # Meta-level: update reward for current meta-strategy and select next
            self.meta_generation_counter += 1
            if self.meta_generation_counter >= self.meta_window_size:
                self._update_meta_reward(f_opt)
                self._select_meta_strategy()
                self.meta_generation_counter = 0
