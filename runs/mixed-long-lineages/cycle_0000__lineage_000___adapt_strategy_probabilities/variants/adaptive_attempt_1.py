...
class ReflectiveDEWithStrategyPortfolio:
    ...
    adaptively select among multiple strategy-probability adaptation methods
    ...
    def __init__(self, dim, np_factor=6, seed=None):
        ...
        # Meta-level MAB for selecting among adaptation methods
        # We have 7 adaptation operator variants
        self.n_adapt_operators = 7
        self.adapt_op_alpha = np.ones(self.n_adapt_operators)  # Beta distribution alpha
        self.adapt_op_beta = np.ones(self.n_adapt_operators)   # Beta distribution beta
        self.current_adapt_op = 0
        self.prev_best_fitness = np.inf
        
        # Sliding window for credit assignment
        self.adapt_window_size = 5
        self.adapt_op_history = []  # (operator_idx, fitness_improvement)
    ...
    # =========================================================================
    # Seven adaptation operator variants
    # =========================================================================

    def _adapt_op0_original(self):
    ...
    def _adapt_op2_entropy_balanced(self):
    ...
    def _adapt_op3_generation_blend(self):
        """Generation-dependent blend (variant_03 - 6 wins)."""
    ...
    def _adapt_op4_rank_softmax(self):
        """Rank-based softmax with adaptive temperature (variant_04)."""
    ...
    def _adapt_op5_diversity_aware(self):
        """Diversity-aware rank-based (variant_05 - 3 wins)."""
    ...
    def _adapt_op6_entropy_softmax(self):
    ...
    def _adapt_strategy_probabilities(self):
        """Meta-level: use Thompson Sampling to select which adaptation operator to use."""
        # Thompson sampling over adaptation operators
        samples = np.empty(self.n_adapt_operators)
        for op in range(self.n_adapt_operators):
            a = float(self.adapt_op_alpha[op])
            b = float(self.adapt_op_beta[op])
            # Clamp to prevent extreme values
            a = np.clip(a, 0.01, 1000.0)
            b = np.clip(b, 0.01, 1000.0)
            samples[op] = self.rng.beta(a, b)
        
        self.current_adapt_op = int(np.argmax(samples))
        
        # Execute the chosen adaptation operator
        adapt_methods = [
            self._adapt_op0_original,
            self._adapt_op1_thompson,
            self._adapt_op2_entropy_balanced,
            self._adapt_op3_generation_blend,
            self._adapt_op4_rank_softmax,
            self._adapt_op5_diversity_aware,
            self._adapt_op6_entropy_softmax,
        ]
        
        try:
            adapt_methods[self.current_adapt_op]()
        except Exception:
            # Fallback to uniform
            self.strategy_idx[:] = self.rng.integers(0, self.n_strategies, size=self.NP)

    def _update_meta_bandit(self, fitness):
        """Update the meta-level bandit based on whether fitness improved."""
        current_best = float(np.min(fitness))
        op = self.current_adapt_op
        
        if current_best < self.prev_best_fitness - 1e-12:
            # Improvement: reward the current adaptation operator
            improvement = float(self.prev_best_fitness - current_best)
            # Scale reward: use a sigmoid-like mapping
            reward = min(1.0, improvement / (abs(self.prev_best_fitness) + 1e-10))
            reward = np.clip(reward, 0.01, 1.0)
            self.adapt_op_alpha[op] += reward
        else:
            # No improvement: penalize slightly
            self.adapt_op_beta[op] += 0.3
        
        # Decay all parameters slowly to allow adaptation over time
        decay = 0.995
        self.adapt_op_alpha *= decay
        self.adapt_op_beta *= decay
        # Ensure minimum values
        self.adapt_op_alpha = np.maximum(self.adapt_op_alpha, 1.0)
        self.adapt_op_beta = np.maximum(self.adapt_op_beta, 1.0)
    ...
    def __call__(self, func, stopping_condition):
        ...
        # Initialize meta-bandit tracking
        ...
        while not stopping_condition():
            ...
            self._update_meta_bandit(fitness)
